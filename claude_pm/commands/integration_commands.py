...
class CMPMAIOpsManager(CMPMCommandBase):
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "overall_health": "healthy" if config_health == "healthy" and env_health == "complete" else "degraded",
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                integrations["memory_service"] = "operational" if memory_health.get("status") == "healthy" else "degraded"
            except Exception:
...
@click.option("--action", type=click.Choice(["status", "test", "reset"]), default="status", 
...
@click.option("--action", type=click.Choice(["status", "config", "health", "providers"]), default="status",
              help="Action to perform")
@click.option("--provider", type=str, help="Specific provider to target (openai|anthropic|google|openrouter|vercel)")
@click.option("--config-level", type=click.Choice(["system", "user", "project", "runtime"]), default="project",
              help="Configuration level")
@click.option("--output-format", type=click.Choice(["dashboard", "json", "table"]), default="dashboard",
              help="Output format")
    ...
    async def run_ai_ops():
        ...
        if action == "status":
            ...
            if output_format == "json":
                ...
                table.add_row("Providers", f"{providers.get('enabled', 0)} enabled", f"Total: {providers.get('total', 0)}")
                ...
                env_status = "✅ Complete" if env.get("missing", 0) == 0 else f"⚠️ Missing {env.get('missing', 0)}"
                ...
                table.add_row("Cost Tracking", cost_status, f"Budget: ${cost.get('daily_budget', 0)}/day")
                
                # Security
                security = status.get("security", {})
                security_status = "✅ Enabled" if security.get("audit_logging", False) else "⚠️ Basic"
                table.add_row("Security", security_status, f"Mode: {security.get('compliance_mode', 'unknown')}")
            ...
            if output_format == "json":
                ...
                if provider:
                    ...
                    for provider_id, provider_config in config.get('providers', {}).items():
                        ...
                        console.print(f"{status} {provider_id}: {provider_config.get('api_key_env', 'N/A')}")
            ...
            if output_format == "json":
                ...
                console.print(f"Configuration: [{config_color}]{config_health.upper()}[/{config_color}]")
                ...
                for provider_id, provider_status in provider_health.items():
                    if provider_status.get("enabled", False):
                        api_key_status = "✅ API Key Set" if provider_status.get("api_key_set", False) else "❌ No API Key"
            ...
            if output_format == "json":
                ...
                for provider_id, provider_config in config.get('providers', {}).items():
                    status = "✅ Enabled" if provider_config.get('enabled', False) else "❌ Disabled"
                    api_key_status = "✅ Set" if provider_config.get('api_key_set', False) else "❌ Missing"
                    ...
                    if provider_config.get('rate_limits'):
                        rpm = provider_config.get('rate_limits', {}).get('requests_per_minute', 'N/A')
