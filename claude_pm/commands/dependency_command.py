...
class DependencyCommand(CLICommand):
    ...
    def _create_status_command(self) -> click.Command:
        ...
        @click.option("--type", type=click.Choice([t.value for t in DependencyType]), help="Filter by dependency type")
    ...
    def _create_install_command(self) -> click.Command:
        ...
        @click.option("--method", type=click.Choice([m.value for m in InstallationMethod]), help="Installation method")
    ...
    def _create_report_command(self) -> click.Command:
        ...
        @click.option("--format", type=click.Choice(["json", "detailed", "summary"]), default="detailed", help="Report format")
    ...
    async def _handle_status(self, ctx, format: str, type_filter: Optional[str], missing_only: bool) -> None:
        ...
        if type_filter:
            dependencies = {
                ...
                if info.type.value == type_filter
            }
        ...
        if format == "json":
            ...
            for name, info in dependencies.items():
                output[name] = {
                    ...
                    "installation_method": info.installation_method.value if info.installation_method else None,
        ...
        if verbose:
            ...
            for name, info in dependencies.items():
                ...
                if info.is_installed:
                    ...
                    self.console.print(f"    Method: {info.installation_method.value if info.installation_method else 'unknown'}")
    ...
    async def _handle_install(self, ctx, dependency_name: str, method: Optional[str], dry_run: bool) -> None:
            ...
            if dep_info:
                if dep_info.is_installed:
                    ...
                    if not install_method:
                        # Get best method
                        dep_config = self.dependency_manager.CORE_DEPENDENCIES.get(dependency_name, {})
                        install_method = self.dependency_manager._select_best_installation_method(
                            dep_config.get("type", DependencyType.SYSTEM_BINARY), 
                            dep_config
                        )
                    
                    self.console.print(f"  📦 Would install {dependency_name} using {install_method.value}")
            else:
        ...
        with Progress(
            ...
            result = await self.dependency_manager.install_dependency(dependency_name, install_method)
    ...
    async def _handle_ai_trackdown(self, ctx, install: bool, verify: bool, status: bool) -> None:
        ...
        if install:
            ...
            if dep_info:
                ...
                self.console.print(f"  Method: {dep_info.installation_method.value if dep_info.installation_method else 'unknown'}")
