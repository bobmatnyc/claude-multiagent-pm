...
async def status(ctx, deployment_info, template_sources, validation):
    ...
    try:
        ...
        if validation:
            ...
            console.print(
                f"   • Accessible Templates: {validation_results['accessible_templates']}"
            )
            console.print(
                f"   • Inaccessible Templates: {validation_results['inaccessible_templates']}"
            )
...
@click.option(
    ...
    "template_type",
    type=click.Choice(
        ["project", "agent", "ticket", "scaffolding", "documentation", "configuration"]
    ),
    default="project",
    help="Template type",
)
@click.option(
    "--source",
    type=click.Choice(["system", "framework", "user", "project"]),
    default="project",
    help="Template source",
)
@click.option("--content", help="Template content (or provide via stdin)")
@click.option(
    "--file", "content_file", type=click.Path(exists=True), help="File containing template content"
)
@click.option("--variables", help="Template variables as JSON string")
@click.option("--metadata", help="Template metadata as JSON string")
@click.pass_context
async def create(
    ctx, template_id, template_type, source, content, content_file, variables, metadata
):
...
@click.option(
    ...
    help="File containing new template content",
)
...
@click.option(
    "--conflict-resolution",
    type=click.Choice(["backup_and_replace", "merge", "skip", "prompt_user"]),
...
@click.option("--variables", help="Template variables as JSON string")
@click.option(
    "--variables-file", type=click.Path(exists=True), help="File containing variables as JSON"
)
@click.option("--output", type=click.Path(), help="Output file path")
...
async def render(ctx, template_id, variables, variables_file, output, version):
    ...
    try:
        ...
        rendered_content = await integration.render_template(
            template_id=template_id, variables=template_variables, version=version
        )
...
async def restore(ctx, template_id, version, confirm):
    ...
    try:
        ...
        if not confirm:
            if not Confirm.ask(
                f"Are you sure you want to restore {template_id} to version {version}?"
            ):
...
@click.option(
    "--force",
    is_flag=True,
    help="Force deployment even if version is current (overrides version checking)",
)
@click.option(
    "--show-version-check", is_flag=True, help="Show detailed version checking information"
)
@click.pass_context
def deploy_claude_md(ctx, target_dir, backup, force, show_version_check):
    ...
    async def _deploy_claude_md_async():
        try:
            from ..services.parent_directory_manager import (
            ...
            if framework_template_path.exists():
                ...
                console.print(
                    f"   • Framework Template Version: {framework_version or 'Not found'}"
                )
            else:
            ...
            claude_md_version = manager._generate_next_claude_md_version(
                target_file, framework_version
            )
            ...
            if target_file.exists():
                ...
                if show_version_check or existing_version != template_version:
                    ...
                    console.print(
                        f"   • Template: {template_version} (from {framework_template_path})"
                    )
                    ...
                    if existing_version and template_version:
                        ...
                        if comparison == 0:
                            console.print(
                                f"   • Result: ✅ Versions are equal - will skip unless forced"
                            )
                        elif comparison > 0:
                            console.print(
                                f"   • Result: ⚠️  Existing version is newer - will skip unless forced"
                            )
                        else:
                            console.print(
                                f"   • Result: 🆙 Template version is newer - will deploy"
                            )

                    # Test skip logic without modifying files
                    should_skip, reason = manager._should_skip_deployment(
                        target_file, f"CLAUDE_MD_VERSION: {claude_md_version}", force
                    )
                    console.print(
                        f"   • Action: {'Skip deployment' if should_skip else 'Proceed with deployment'}"
                    )
                    if reason:
            ...
            operation = await manager.install_template_to_parent_directory(
                target_directory, "parent_directory_claude_md", template_variables, force=force
            )
            ...
            if operation.success:
                if operation.warnings and "skipped" in operation.warnings[0].lower():
                    console.print(
                        f"⏭️  [bold yellow]Deployment Skipped - Version Current[/bold yellow]"
                    )
