...
class PolicyCondition:
    ...
    def __post_init__(self):
        ...
        if self.type == PolicyConditionType.BETWEEN and (
            self.min_value is None or self.max_value is None
        ):
            raise ValueError("min_value and max_value required for BETWEEN condition")

        if (
            self.type in [PolicyConditionType.IN_LIST, PolicyConditionType.NOT_IN_LIST]
            and not self.values
        ):
            raise ValueError("values required for IN_LIST/NOT_IN_LIST conditions")

        if self.type == PolicyConditionType.TIME_RANGE and (
            not self.start_time or not self.end_time
        ):
            raise ValueError("start_time and end_time required for TIME_RANGE condition")

        if self.type == PolicyConditionType.RATE_LIMIT and (
            not self.rate_limit or not self.time_window
        ):
            raise ValueError("rate_limit and time_window required for RATE_LIMIT condition")

        if self.type == PolicyConditionType.PROBABILITY and (
            self.probability is None or self.probability < 0 or self.probability > 1
        ):
            raise ValueError("probability must be between 0 and 1 for PROBABILITY condition")

        if (
            self.type in [PolicyConditionType.COMPOSITE_AND, PolicyConditionType.COMPOSITE_OR]
            and not self.sub_conditions
        ):
...
class PolicyRule:
    ...
    def can_execute(self) -> bool:
        ...
        if (
            self.cooldown_period is not None
            and self.last_execution is not None
            and datetime.now() - self.last_execution < self.cooldown_period
        ):
            return False
...
class PolicyConditionEvaluator:
    ...
    def _evaluate_condition_internal(
        ...
        if condition.type == PolicyConditionType.ALWAYS:
            ...
            return all(
                self.evaluate_condition(sub_cond, context) for sub_cond in condition.sub_conditions
            )
...
class PolicyEngine:
    ...
    def list_rules(
        self, scope: Optional[PolicyScope] = None, enabled_only: bool = True
    ) -> List[PolicyRule]:
    ...
    def evaluate_rules(
        self, context: Dict[str, Any], scope: Optional[PolicyScope] = None
    ) -> List[PolicyRule]:
    ...
    def _evaluate_rule_conditions(self, rule: PolicyRule, context: Dict[str, Any]) -> bool:
        ...
        if rule.condition_operator == "AND":
            return all(
                self.evaluator.evaluate_condition(condition, context)
    ...
    def _execute_rule(self, rule: PolicyRule, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for action in sorted_actions:
            try:
                ...
                if action.condition and not self.evaluator.evaluate_condition(
                ...
                result["action_results"].append(
                    {"action_type": action.type.value, "success": False, "error": str(e)}
                )
    ...
    def _execute_action(self, action: PolicyAction, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {"action_type": action.type.value, "success": True, "result": None, "error": None}
    ...
    def export_rules(self) -> Dict[str, Any]:
        ...
        for name, rule in self.rules.items():
            rules_data[name] = {
                ...
                "cooldown_period": (
                    rule.cooldown_period.total_seconds() if rule.cooldown_period else None
                ),
                "tags": list(rule.tags),
    ...
    def _serialize_condition(self, condition: PolicyCondition) -> Dict[str, Any]:
        ...
        data = {"type": condition.type.value, "negate": condition.negate}
        ...
        if condition.sub_conditions:
            data["sub_conditions"] = [
                self._serialize_condition(sub) for sub in condition.sub_conditions
            ]
    ...
    def _deserialize_rule(self, rule_config: Dict[str, Any]) -> PolicyRule:
        ...
        conditions = [
            self._deserialize_condition(cond) for cond in rule_config.get("conditions", [])
        ]
    ...
    def _deserialize_condition(self, cond_config: Dict[str, Any]) -> PolicyCondition:
        ...
        if "sub_conditions" in cond_config:
            sub_conditions = [
                self._deserialize_condition(sub) for sub in cond_config["sub_conditions"]
            ]
...
class PolicyEngineConfig:
    ...
    def save_to_file(self, file_path: Optional[str] = None) -> None:
        ...
        try:
            config_data = {"rules": self.engine.export_rules()}
