...
class ConfigurationSchemaValidator:
    ...
    def validate_schema(self, config_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            result.add_issue(ValidationSeverity.INFO, "Configuration passes schema validation")
            ...
            result.schema_valid = False
            result.add_issue(
                ValidationSeverity.ERROR, f"Schema validation error: {e}", code="SCHEMA_ERROR"
            )
...
class RuntimeConfigurationValidator:
    ...
    def _validate_performance_consistency(
        self, config: MemoryTriggerConfig, result: ValidationResult
    ) -> None:
    ...
    def _validate_lifecycle_policies(
        self, config: MemoryTriggerConfig, result: ValidationResult
    ) -> None:
    ...
    def _validate_monitoring_settings(
        self, config: MemoryTriggerConfig, result: ValidationResult
    ) -> None:
    ...
    def _validate_trigger_policies(
        self, config: MemoryTriggerConfig, result: ValidationResult
    ) -> None:
    ...
    def _validate_single_trigger_policy(
        self, name: str, policy: TriggerPolicyConfig, result: ValidationResult
    ) -> None:
...
class PolicyValidator:
    ...
    def _validate_single_condition(
        self, condition: PolicyCondition, result: ValidationResult, path: str
    ) -> None:
        ...
        if condition.sub_conditions:
            for i, sub_condition in enumerate(condition.sub_conditions):
                self._validate_single_condition(
                    sub_condition, result, f"{path}.sub_conditions[{i}]"
                )
    ...
    def _validate_single_action(
        self, action: PolicyAction, result: ValidationResult, path: str
    ) -> None:
    ...
    def _validate_rule_performance(self, rule: PolicyRule, result: ValidationResult) -> None:
        ...
        if any(action.type == PolicyActionType.CHAIN_POLICY for action in rule.actions):
            chain_targets = [
                action.parameters.get("policy_name")
...
class ComprehensiveValidator:
    ...
    def validate_configuration(self, config: MemoryTriggerConfig) -> ValidationResult:
        ...
        combined_result.valid = (
            schema_result.valid and runtime_result.valid and len(combined_result.errors) == 0
        )
    ...
    def validate_configuration_file(self, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            result.add_issue(
                ValidationSeverity.ERROR, f"Failed to load configuration: {e}", code="LOAD_ERROR"
            )
            return result
