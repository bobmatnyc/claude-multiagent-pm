...
class MemoryTriggerConfig:
    ...
    def validate(self) -> List[str]:
        ...
        # Cross-validation
        if (self.performance.max_concurrent_operations > 
            self.max_memory_operations_per_second):
            errors.append("max_concurrent_operations cannot exceed max_memory_operations_per_second")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'MemoryTriggerConfig':
        ...
        if 'backend' in data:
            if 'backend_type' in data['backend'] and isinstance(data['backend']['backend_type'], str):
