...
class ProjectService(BaseService):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.compliance_check_interval = self.get_config("compliance_check_interval", 1800)  # 30 minutes
    ...
    async def _health_check(self) -> Dict[str, bool]:
        ...
        try:
            ...
            checks["compliance_cache_healthy"] = len(self._compliance_cache) >= len(self._projects) * 0.8
        ...
        if self.base_path.exists():
            for project_path in self.base_path.iterdir():
                if (project_path.is_dir() and 
                    not project_path.name.startswith('.') and 
                    project_path.name not in excluded_dirs):
    ...
    async def _analyze_project(self, project_path: Path, project_type: str) -> Optional[ProjectInfo]:
    ...
    async def _save_project_registry(self) -> None:
        ...
        try:
            ...
            registry_data = {
                ...
                "compliance": {name: asdict(check) for name, check in self._compliance_cache.items()}
            }
    ...
    async def get_project_stats(self) -> Dict[str, Any]:
        ...
        if self._compliance_cache:
            ...
            stats["compliance_summary"]["average_score"] = total_score // len(self._compliance_cache)
