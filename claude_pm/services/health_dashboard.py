...
from ..models.health import (
    HealthReport, HealthDashboard, ServiceHealthReport, SubsystemHealth, 
    HealthStatus, create_health_report, create_health_dashboard
)
...
class HealthDashboardOrchestrator:
    ...
    def add_collector(self, collector: HealthCollector) -> None:
        ...
        self._logger.info(f"Added health collector: {collector.name} ({collector.get_subsystem_name()})")
    ...
    async def get_health_dashboard(self, force_refresh: bool = False) -> HealthDashboard:
        ...
        if not force_refresh:
            ...
            if cached_report is not None:
                ...
                self._logger.debug(f"Health dashboard served from cache ({dashboard.total_response_time_ms:.1f}ms)")
                return dashboard
        ...
        try:
            ...
            self._logger.info(f"Health dashboard generated ({dashboard.total_response_time_ms:.1f}ms)")
    ...
    async def _collect_fresh_health(self) -> HealthReport:
        ...
        try:
            results = await asyncio.wait_for(
                asyncio.gather(*[limited_execute(ct) for ct in collector_tasks], return_exceptions=True),
                timeout=self.global_timeout_seconds
            )
        except asyncio.TimeoutError:
            self._logger.warning(f"Health collection timed out after {self.global_timeout_seconds}s")
            results = [(collector, TimeoutError("Global timeout")) for collector, _ in collector_tasks]
        ...
        self._add_recommendations_and_alerts(health_report, successful_collectors, failed_collectors)
    ...
    async def _execute_collector_with_circuit_breaker(self, collector: HealthCollector) -> List[ServiceHealthReport]:
        """
    ...
    def _create_subsystem_health(self, name: str, services: List[ServiceHealthReport]) -> SubsystemHealth:
        ...
        unhealthy_services = status_counts[HealthStatus.UNHEALTHY] + status_counts[HealthStatus.ERROR]
    ...
    def get_orchestrator_stats(self) -> Dict[str, Any]:
        ...
        return {
            "orchestrator": {
                ...
                "success_rate": (self._successful_collections / max(self._successful_collections + self._failed_collections, 1)) * 100
            },
            "cache": cache_stats,
