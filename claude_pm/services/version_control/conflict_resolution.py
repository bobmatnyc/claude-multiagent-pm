...
class ConflictResolutionManager:
    ...
    def _get_conflicted_files(self) -> List[str]:
        ...
        try:
            ...
            for line in result.stdout.strip().split('\n'):
                if line.strip() and (line.startswith('UU') or line.startswith('AA') or line.startswith('DD')):
                    # Extract filename
    ...
    def _generate_resolution_suggestion(self, file_path: str, markers: List[ConflictMarker]) -> str:
        ...
        for i, marker in enumerate(markers):
            if not marker.ours_content and marker.theirs_content:
                ...
                suggestions.append(f"Conflict {i+1}: Whitespace differences only - can auto-resolve")
            else:
    ...
    def _calculate_severity(self, file_conflicts: List[FileConflict]) -> str:
        ...
        if binary_conflicts > 0 or (total_markers > 10 and auto_resolvable < len(file_conflicts) * 0.5):
            return "high"
