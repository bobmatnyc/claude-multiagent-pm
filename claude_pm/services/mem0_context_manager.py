...
class Mem0ContextManager:
    ...
    async def prepare_context(self, request: ContextRequest) -> ContextBundle:
        ...
        try:
            ...
            bundle = ContextBundle(request=request, context_id=context_id, prepared_at=start_time)
            ...
            for category in search_categories:
                for query in search_queries:
                    memories = await self._retrieve_category_memories(category, query, request)
            ...
            logger.info(
                f"Prepared context {context_id} with {bundle.total_memories} memories in {bundle.preparation_time_ms}ms"
            )
            return bundle
    ...
    def _determine_search_categories(self, request: ContextRequest) -> List[MemoryCategory]:
        ...
        if request.context_type == ContextType.AGENT_TASK and request.agent_type:
            ...
            return agent_filter.get(
                "primary_categories", [MemoryCategory.PATTERN, MemoryCategory.PROJECT]
            )
    ...
    async def _retrieve_category_memories(
        self, category: MemoryCategory, query: str, request: ContextRequest
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve memories for category {category.value}, query '{query}': {e}"
            )
            return []

    def _apply_agent_filters(
        self, memories: List[Dict[str, Any]], agent_type: str
    ) -> List[Dict[str, Any]]:
            ...
            for category, memories in bundle.memories_by_category.items():
                ...
                for memory in memories:
                    ...
                    if memory_tags.intersection(success_tags):
                        memory["pattern_boost"] = recognizers["success_indicators"][
                            "weight_multiplier"
                        ]
                    ...
                    if memory_tags.intersection(team_tags):
                        memory["pattern_boost"] = (
                            memory.get("pattern_boost", 1.0)
                            * recognizers["team_preferences"]["weight_multiplier"]
                        )
                    ...
                    if memory_tags.intersection(error_tags):
                        memory["pattern_boost"] = (
                            memory.get("pattern_boost", 1.0)
                            * recognizers["error_prevention"]["weight_multiplier"]
                        )
                        memory["pattern_type"] = (
                            memory.get("pattern_type", "") + "_error_prevention"
                        )
    ...
    async def _load_project_context_history(self, bundle: ContextBundle) -> ContextBundle:
        ...
        try:
            ...
            logger.info(
                f"Loaded project context history for {project_name}: {len(arch_decisions + team_patterns + error_history + success_patterns)} memories"
            )
            return bundle
    ...
    async def _retrieve_project_memories(
        self, project_name: str, query: str, category: MemoryCategory, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """Retrieve project-specific memories for a given query and category."""
        try:
            response = await self.memory.retrieve_memories(
                category=category, query=query, project_filter=project_name, limit=limit
            )
    ...
    def _memory_passes_filters(
        self, memory: Dict[str, Any], security_level: str, team_access: str
    ) -> bool:
    ...
    def _apply_project_scope_filter(self, bundle: ContextBundle) -> ContextBundle:
        ...
        cross_project_limit = self.team_knowledge_filters["context_scopes"]["project_focused"][
            "cross_project_limit"
        ]
    ...
    def _apply_global_patterns_filter(self, bundle: ContextBundle) -> ContextBundle:
        ...
        insight_threshold = self.team_knowledge_filters["context_scopes"]["global_insights"][
        ...
        if total_keywords > 0:
            for keyword in request.keywords:
                if keyword.lower() in memory_content:
                    keyword_matches += 1
            keyword_score = (keyword_matches / total_keywords) * self.pattern_weights[
                "keyword_match"
            ]
            score += keyword_score
    ...
    def _generate_context_summary(self, bundle: ContextBundle) -> str:
        ...
        if bundle.patterns:
            top_patterns = [
                p.get("metadata", {}).get("pattern_type", "pattern") for p in bundle.patterns[:3]
            ]
            if any(top_patterns):
