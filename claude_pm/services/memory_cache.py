...
class MemoryCache:
    ...
    async def _ensure_cache_fresh(self):
        ...
        if self._last_loaded is None or datetime.now() - self._last_loaded > self._cache_ttl:
    ...
    async def store_project_memory(
        ...
        try:
            ...
            entry = CacheEntry(
                data={"content": content, "metadata": metadata, "project_name": project_name},
    ...
    async def search_projects(
        self, query: str, category: str = None, tags: List[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for project_name, entry in self._project_cache.items():
                ...
                if query:
                    ...
                    if (
                        query_lower not in content
                        and query_lower not in metadata_text
                        and query_lower not in project_name_lower
                    ):
                        continue
