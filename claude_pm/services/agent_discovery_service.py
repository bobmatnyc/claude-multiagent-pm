...
class AgentFileWatcher(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if not event.is_directory and event.src_path.endswith(".py"):
            ...
            asyncio.create_task(
                self.discovery_service.handle_agent_file_change(event.src_path, "created")
            )
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory and event.src_path.endswith(".py"):
            ...
            asyncio.create_task(
                self.discovery_service.handle_agent_file_change(event.src_path, "modified")
            )
    ...
    def on_deleted(self, event):
        ...
        if not event.is_directory and event.src_path.endswith(".py"):
            ...
            asyncio.create_task(
                self.discovery_service.handle_agent_file_change(event.src_path, "deleted")
            )
...
class AgentDiscoveryService(BaseService):
    ...
    async def discover_all_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Agent discovery completed in {discovery_results['discovery_time']:.2f}s"
            )
    ...
    async def start_file_monitoring(self) -> None:
        ...
        try:
            ...
            for directory in directories_to_monitor:
                if directory.exists():
                    self.file_observer.schedule(self.file_watcher, str(directory), recursive=True)
    ...
    async def perform_agent_health_checks(self) -> Dict[str, Any]:
        ...
        for agent_type, agent_instance in loaded_agents.items():
            try:
                ...
                health_results["agent_health"][agent_type] = {"healthy": False, "error": str(e)}
                health_results["unhealthy_agents"] += 1
            ...
            registry_data = {
                ...
                "agent_types": {
                    agent_type: {
                        "available": True,
                        "tier": (
                            hierarchy.get_agent_by_type(agent_type).tier
                            if hierarchy.get_agent_by_type(agent_type)
                            else "unknown"
                        ),
                    }
                ...
                "health_status": {
                    ...
                    "unloaded_agents": len(hierarchy.get_all_agents())
                    - len(self.agent_loader.get_loaded_agents()),
                },
            }
    ...
    async def _health_check(self) -> Dict[str, bool]:
        ...
        checks["recent_discovery"] = (
            self.last_discovery_time is not None
            and (datetime.now() - self.last_discovery_time).total_seconds()
