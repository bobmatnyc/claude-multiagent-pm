...
class ProjectMemoryManager:
    ...
    async def get_project_info(
        self, project_name: str, use_cache: bool = True
    ...
    async def _execute_search(self, search_query: SearchQuery) -> List[SearchResult]:
        ...
        if search_query.mode == SearchMode.EXACT:
            memories_response = await self._exact_search(
                expanded_query, search_tags, search_query.limit
            )
        elif search_query.mode == SearchMode.FUZZY:
            memories_response = await self._fuzzy_search(
                expanded_query, search_tags, search_query.limit
            )
        elif search_query.mode == SearchMode.SEMANTIC:
            memories_response = await self._semantic_search(
                expanded_query, search_tags, search_query.limit
            )
        else:  # HYBRID
            memories_response = await self._hybrid_search(
                expanded_query, search_tags, search_query.limit
            )

        # Process results
        if not (
            memories_response.success
            and memories_response.data
            and memories_response.data.get("memories")
        ):
            return []
    ...
    async def _fuzzy_search(self, query: str, tags: List[str], limit: int):
        ...
        query_variations = [query, query.lower(), query.replace("-", " "), query.replace("_", " ")]
        ...
        return type(response)(success=True, data={"memories": unique_results[:limit]})
    ...
    async def _hybrid_search(self, query: str, tags: List[str], limit: int):
        ...
        return type(exact_results)(success=True, data={"memories": unique_memories[:limit]})
    ...
    def _calculate_relevance_score(
        self, project_data: Dict[str, Any], original_query: str, expanded_query: str
    ...
    def _rank_results(
        self, results: List[SearchResult], ranking: ProjectRanking
    ) -> List[SearchResult]:
        ...
        if ranking == ProjectRanking.RELEVANCE:
            ...
            return sorted(
                results,
                key=lambda r: r.project_data.get("last_modified", "1900-01-01"),
                reverse=True,
            )
            ...
            return sorted(
                results, key=lambda r: len(r.project_data.get("features", [])), reverse=True
            )
    ...
    async def get_project_recommendations(
        self, current_project: str, limit: int = 5
    ) -> List[SearchResult]:
        """
    ...
    async def get_project_summary(self, project_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            summary = {
                ...
                "description": (
                    project_info.get("description", "")[:200] + "..."
                    if len(project_info.get("description", "")) > 200
                    else project_info.get("description", "")
                ),
    ...
    async def refresh_project(self, project_name: str) -> bool:
        ...
        try:
            ...
            keys_to_remove = [
                key for key in self.query_cache.keys() if project_name.lower() in key.lower()
            ]
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        try:
            ...
            for result in results:
                print(
                    f"  - {result.project_name}: {result.relevance_score:.2f} ({', '.join(result.match_reasons)})"
                )
            ...
            print(
                f"📈 Performance: {stats['avg_response_time_ms']:.1f}ms avg, {stats['cache_hit_rate']:.1f}% cache hit rate"
            )
