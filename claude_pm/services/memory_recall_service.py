...
from .memory.memory_context_enhancer import (
    MemoryContextEnhancer, MemoryContext, RecallTrigger, RecallConfig
)
...
from .memory.context_builder import (
    ContextBuilder, EnrichedContext, ContextType, ContextTemplate
)
from .memory.recommendation_engine import (
    RecommendationEngine, RecommendationSet, RecommendationConfig, RecommendationType
)
...
class MemoryRecallService:
    ...
    def __init__(
        ...
        self.memory_enhancer = MemoryContextEnhancer(
            self.memory_service, self.config.recall_config
        )
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    self._perform_complete_recall(
                        project_name, operation_type, operation_context,
                        context_type, trigger, include_recommendations
                    ),
    ...
    async def get_service_health(self) -> Dict[str, Any]:
        ...
        try:
            health_data["component_health"]["memory_service"] = await self.memory_service.get_service_health()
        except Exception as e:
    ...
    def _update_recall_stats(self, processing_time_ms: float, success: bool):
        ...
        self.service_stats["average_recall_time_ms"] = (
            (current_avg * (total_recalls - 1) + processing_time_ms) / total_recalls
        )
