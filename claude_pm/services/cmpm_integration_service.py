...
class CMPMIntegrationService(BaseService):
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"CMPM Integration Service initialized at {self.integration_level.value} level")
    ...
    async def _initialize_all_services(self):
        ...
        if initialization_tasks:
            try:
                ...
                self.logger.warning(f"Service initialization timed out after {self.initialization_timeout}s")
    ...
    def _determine_integration_level(self):
        ...
        operational_services = sum(1 for service in self.services.values() 
        ...
        self.logger.info(f"Integration level: {self.integration_level.value} ({operational_services}/{total_available} services)")
    ...
    async def _health_check_parent_directory_manager(self):
        ...
        try:
            ...
            self.services["parent_directory_manager"].status = CMPMServiceStatus.OPERATIONAL
            self.services["parent_directory_manager"].last_health_check = datetime.now()
            self.services["parent_directory_manager"].metadata["managed_directories"] = len(directories)
    ...
    async def get_integration_status(self) -> CMPMIntegrationStatus:
        ...
        operational_services = sum(1 for service in self.services.values() 
                                 if service.status == CMPMServiceStatus.OPERATIONAL)
        error_services = sum(1 for service in self.services.values() 
                           if service.status == CMPMServiceStatus.ERROR)
        total_services = len([s for s in self.services.values() if s.available])
        
        # Calculate health score
        health_score = int((operational_services / total_services) * 100) if total_services > 0 else 0
    ...
    def _generate_troubleshooting_recommendations(self) -> List[str]:
        ...
        for service_id, service_info in self.services.items():
            if service_info.status == CMPMServiceStatus.ERROR:
                ...
                recommendations.append(f"{service_info.name} is degraded: {service_info.error_message}")
