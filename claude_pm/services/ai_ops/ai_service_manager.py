...
class AIServiceManager:
        ...
        try:
            ...
            if self.auth_service.is_provider_configured("openai"):
                self.providers["openai"] = OpenAIProvider(
                    auth_service=self.auth_service, config=self.config.get("openai", {})
                )
            ...
            if self.auth_service.is_provider_configured("anthropic"):
                self.providers["anthropic"] = AnthropicProvider(
                    auth_service=self.auth_service, config=self.config.get("anthropic", {})
                )
            ...
            if self.auth_service.is_provider_configured("google"):
                self.providers["google"] = GoogleProvider(
                    auth_service=self.auth_service, config=self.config.get("google", {})
                )
            ...
            if self.auth_service.is_provider_configured("openrouter"):
                self.providers["openrouter"] = OpenRouterProvider(
                    auth_service=self.auth_service, config=self.config.get("openrouter", {})
                )
            ...
            if self.auth_service.is_provider_configured("vercel"):
                self.providers["vercel"] = VercelProvider(
                    auth_service=self.auth_service, config=self.config.get("vercel", {})
                )
    ...
    async def execute_request(self, request) -> Any:
        ...
        try:
            ...
            response = await circuit_breaker.call(provider.execute_request, request)
    ...
    async def get_optimal_provider(self, criteria: Dict[str, Any]) -> Optional[str]:
        """
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for provider, provider_metrics in self.provider_metrics.items():
            metrics["providers"][provider] = {
                ...
                "last_request_time": (
                    provider_metrics.last_request_time.isoformat()
                    if provider_metrics.last_request_time
                    else None
                ),
            }
