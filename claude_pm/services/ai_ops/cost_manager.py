...
class CostManager:
    ...
    def _calculate_usage_for_period(self, provider: str, start_date: datetime, end_date: datetime) -> float:
    ...
    def get_all_budgets(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            key: self.get_budget_status(key)
            for key in self.budgets.keys()
        }
    ...
    async def get_comprehensive_analytics(self) -> Dict[str, Any]:
        ...
        analytics = {
            ...
            "average_cost_per_request": self.total_cost_tracked / max(self.total_requests_tracked, 1),
            "providers": {},
            "models": {},
            "time_periods": {},
        ...
        for entry in self.cost_entries:
            ...
            provider_costs[entry.provider]["models"][model_key]["total_tokens"] += entry.total_tokens
        ...
        for provider, data in provider_costs.items():
            ...
            for model, model_data in data["models"].items():
                model_data["average_cost_per_request"] = model_data["total_cost"] / max(model_data["total_requests"], 1)
                model_data["cost_per_token"] = model_data["total_cost"] / max(model_data["total_tokens"], 1)
        ...
        for model, data in model_analytics.items():
            if data["total_requests"] > 10 and data["cost_per_token"] > 0.001:
                ...
                cheaper_alternatives = [
                    alt for alt, alt_data in model_analytics.items()
                    if alt != model and alt_data["cost_per_token"] < data["cost_per_token"] * 0.8
                ]
                
                if cheaper_alternatives:
                    cheapest_alt = min(cheaper_alternatives, key=lambda x: model_analytics[x]["cost_per_token"])
                    potential_savings = (
                        data["total_cost"] -
