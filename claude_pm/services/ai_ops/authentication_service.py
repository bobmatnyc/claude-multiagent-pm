...
class AuthenticationService:
            ...
            if secure_storage_path.exists():
                ...
                for provider, info_data in stored_data.get("key_info", {}).items():
                    self.key_info[provider] = APIKeyInfo(
                        ...
                        last_used=(
                            datetime.fromisoformat(info_data["last_used"])
                            if info_data.get("last_used")
                            else None
                        ),
                        expires_at=(
                            datetime.fromisoformat(info_data["expires_at"])
                            if info_data.get("expires_at")
                            else None
                        ),
    ...
    def _save_to_secure_storage(self):
        ...
        try:
            ...
            storage_data = {"keys": self.api_keys.copy(), "key_info": {}}
    ...
    async def validate_all_authentications(self) -> Dict[str, Any]:
        ...
        Returns:
            Validation results
        """
        results = {"all_valid": True, "providers": {}, "issues": []}
