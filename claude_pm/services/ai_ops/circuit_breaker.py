...
class CircuitBreaker:
    ...
    async def _on_failure(self, execution_time: float, exception: Exception):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.consecutive_failures >= self.config.failure_threshold:
                should_open = True
                self.logger.warning(f"Circuit breaker opening due to {self.consecutive_failures} consecutive failures")
            ...
            # Check failure rate
                ...
                if failure_rate >= self.config.max_failure_rate:
                    should_open = True
                    self.logger.warning(f"Circuit breaker opening due to failure rate: {failure_rate:.2%}")
                
                # Check slow call rate
                slow_call_rate = self.slow_call_count / self.total_calls
                if slow_call_rate >= self.config.slow_call_rate_threshold:
                    should_open = True
                    self.logger.warning(f"Circuit breaker opening due to slow call rate: {slow_call_rate:.2%}")
    ...
    async def get_state(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            slow_call_rate = (self.slow_call_count / self.total_calls) if self.total_calls > 0 else 0
