...
class ProjectIndexer:
    ...
    async def _index_project(
        self, project_dir: Path, force_refresh: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        """
        result = {"project": project_dir.name, "action": "skipped", "error": None, "metadata": None}
    ...
    async def _project_needs_indexing(self, project_dir: Path) -> bool:
        ...
        try:
            ...
            last_indexed = datetime.fromisoformat(
                existing_metadata.get("last_indexed", "1900-01-01")
            )
    ...
    async def _analyze_package_json(self, file_path: Path, metadata: ProjectMetadata) -> None:
        ...
        try:
            ...
            metadata.development_workflow.extend([f"npm run {script}" for script in scripts.keys()])
    ...
    def _extract_from_markdown_content(self, content: str, metadata: ProjectMetadata) -> None:
        ...
        if "architecture" in content_lower or "decision" in content_lower:
            lines = content.split("\n")
            for line in lines:
                if ("architecture" in line.lower() or "decision" in line.lower()) and len(
                    line
                ) < 200:
    ...
    async def _extract_architecture_decisions(
        self, project_dir: Path, metadata: ProjectMetadata
    ) -> None:
    ...
    async def _analyze_development_workflow(
        self, project_dir: Path, metadata: ProjectMetadata
    ) -> None:
...
""".strip()
            ...
            if self.memory and self.memory.is_connected():
                try:
                    ...
                    if response.success:
                        ...
                        logger.warning(
                            f"Failed to store {metadata.name} in mem0AI: {response.error}"
                        )
                except Exception as e:
            ...
            if mem0ai_success or local_cache_success:
                ...
                logger.error(
                    f"Failed to store metadata in both mem0AI and local cache for {metadata.name}"
                )
                return False
    ...
    def get_indexer_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            * 100,
            "success_rate": (
                (self.stats["projects_indexed"] / max(1, self.stats["projects_scanned"])) * 100
                if self.stats["projects_scanned"] > 0
                else 0
            ),
            "avg_index_time": (
                self.stats["total_index_time"] / max(1, self.stats["projects_indexed"])
                if self.stats["projects_indexed"] > 0
                else 0
            ),
        }
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        try:
            ...
            print(
                f"📊 Scan Results: {results['projects_found']} found, {results['projects_indexed']} indexed"
            )
            ...
            if project_info:
                print(
                    f"📋 Project Info: {project_info['name']} - {project_info['description'][:100]}..."
                )
