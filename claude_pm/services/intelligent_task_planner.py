...
class IntelligentTaskPlanner:
    ...
    async def decompose_task(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Decomposition completed: {len(subtasks)} subtasks, "
    ...
    async def _prepare_decomposition_context(
        self, task_description: str, project_name: str = None, metadata: TaskMetadata = None
    ) -> Any:
    ...
    def _assign_agent_type(
        self, task_title: str, task_description: str, skills_required: List[str] = None
    ) -> str:
        """
        ...
        if any(
            word in combined_text for word in ["implement", "code", "develop", "build", "create"]
        ):
            return "engineer"
    ...
    def _parse_similar_task(
        self, memory: Dict[str, Any], similarity_score: float
    ) -> Optional[SimilarTask]:
    ...
    async def _analyze_task_complexity(
        self, task_description: str, similar_tasks: List[SimilarTask], metadata: TaskMetadata = None
    ) -> TaskComplexity:
        ...
        if similar_tasks:
            ...
            most_common_complexity = max(
                set(historical_complexities), key=historical_complexities.count
            )
    ...
    async def _generate_adaptive_decomposition(
        self,
        task_description: str,
        complexity: TaskComplexity,
        strategy: DecompositionStrategy,
        ...
        if similar_tasks:
            ...
            subtasks = self._generate_from_base_patterns(task_description, base_patterns, metadata)
    ...
    def _get_base_decomposition_patterns(
        self, complexity: TaskComplexity, strategy: DecompositionStrategy
    ) -> Dict[str, Any]:
    ...
    async def _adapt_from_similar_tasks(
        self,
        task_description: str,
    ...
    def _adapt_subtask_from_history(
        self,
        historical_subtask: Dict[str, Any],
        current_task: str,
        index: int,
        similar_task: SimilarTask,
    ) -> SubTask:
        ...
        adapted_title = original_title.replace(
            similar_task.task_description.split()[0], current_task.split()[0]
        )

        # Assign agent type based on adapted content
        agent_type = self._assign_agent_type(
            adapted_title, original_desc, historical_subtask.get("skills_required", [])
        )
    ...
    def _generate_from_base_patterns(
        self, task_description: str, base_patterns: Dict[str, Any], metadata: TaskMetadata = None
    ) -> List[SubTask]:
    ...
    def _generate_subtask_description(
        self, title: str, task_description: str, metadata: TaskMetadata = None
    ) -> str:
    ...
    def _apply_strategy_adjustments(
        self, subtasks: List[SubTask], strategy: DecompositionStrategy
    ) -> List[SubTask]:
        ...
        if strategy == DecompositionStrategy.PARALLEL:
            ...
            for subtask in subtasks:
                subtask.dependencies = [
                    dep for dep in subtask.dependencies if "critical" in dep.lower()
                ]
        ...
        for subtask in subtasks:
            ...
            if relevant_patterns:
                ...
                subtask.validation_steps.extend(
                    ["Review relevant patterns from team knowledge", "Apply learned best practices"]
                )
    ...
    def _calculate_decomposition_metrics(
        self, subtasks: List[SubTask], similar_tasks: List[SimilarTask], complexity: TaskComplexity
    ) -> Tuple[float, float]:
    ...
    def _extract_pattern_matches(
        self, context: Any, similar_tasks: List[SimilarTask]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_adaptation_notes(
        self, similar_tasks: List[SimilarTask], strategy: DecompositionStrategy
    ) -> str:
        ...
        if similar_tasks:
            ...
            notes.append(
                f"Adapted from similar task (similarity: {best_similar.similarity_score:.2f})"
            )
    ...
    async def _store_decomposition_in_memory(
        self, decomposition: TaskDecomposition, project_name: str = None
    ) -> None:
    ...
    def _create_fallback_decomposition(
        self, task_description: str, decomposition_id: str, start_time: datetime
    ) -> TaskDecomposition:
...
# Factory function
def create_intelligent_task_planner(
    memory: ClaudePMMemory, context_manager: Mem0ContextManager
