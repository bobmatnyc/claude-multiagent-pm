...
class AgentHierarchyValidator(BaseService):
    ...
    def _initialize_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            "tier_structure": {
                "required_directories": ["system_agents", "user_agents", "project_agents"],
                "required_files": ["hierarchy.yaml", "registry.json"],
            },
            "agent_requirements": {
                "required_methods": ["__init__", "initialize", "cleanup"],
                "required_attributes": ["name", "agent_type"],
            },
    ...
    async def validate_agent_availability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "validation_time": time.time() - validation_start}
    ...
    async def validate_hierarchy_comprehensive(self) -> HierarchyValidationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Comprehensive validation completed in {report.validation_duration:.2f}s"
            )
    ...
    async def validate_hierarchy_deep(self) -> Dict[str, Any]:
        ...
        try:
            ...
            deep_analysis["optimization_recommendations"] = (
                self._generate_optimization_recommendations(deep_analysis)
            )
    ...
    async def _generate_agent_health_report(
        self, agent_type: str, agent_instance: Any
    ) -> AgentHealthReport:
            ...
            if hasattr(agent_instance, "health_check"):
                ...
                try:
                    ...
                    if isinstance(health_result, dict):
                        health_status = (
                            "healthy" if health_result.get("healthy", True) else "unhealthy"
                        )
                    else:
    ...
    def _generate_tier_summary(self) -> Dict[str, Any]:
        ...
        return {
            "system": {
                ...
                "agent_types": list(
                    set(agent.agent_type for agent in hierarchy.system_agents.values())
                ),
                "loaded_agents": len(
                    [agent for agent in hierarchy.system_agents.values() if agent.loaded]
                ),
            },
            "user": {
                "total_agents": len(hierarchy.user_agents),
                "agent_types": list(
                    set(agent.agent_type for agent in hierarchy.user_agents.values())
                ),
                "loaded_agents": len(
                    [agent for agent in hierarchy.user_agents.values() if agent.loaded]
                ),
            },
            "project": {
                "total_agents": len(hierarchy.project_agents),
                "agent_types": list(
                    set(agent.agent_type for agent in hierarchy.project_agents.values())
                ),
                "loaded_agents": len(
                    [agent for agent in hierarchy.project_agents.values() if agent.loaded]
                ),
            },
        }
    ...
    def _calculate_overall_health(
        self, issues: List[ValidationIssue], agent_reports: List[AgentHealthReport]
    ) -> str:
    ...
    async def collect_agent_health_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for agent_type, agent_instance in loaded_agents.items():
                try:
                    health_report = await self._generate_agent_health_report(
    ...
    async def save_health_reports(self) -> None:
        ...
        try:
            ...
            for agent_type, reports in self.health_history.items():
                health_data[agent_type] = [
                    report.to_dict() for report in reports[-10:]
                ]  # Keep last 10
    ...
    async def get_validation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_validation": (
                self.last_validation_time.isoformat() if self.last_validation_time else None
            ),
