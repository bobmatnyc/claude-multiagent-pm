...
class TemplateDeploymentIntegration(BaseService):
    ...
    async def _get_deployment_config_via_cli(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                [cli_command, "--deployment-info"], capture_output=True, text=True, timeout=10
            )
    ...
    def _build_template_sources(
        self, deployment_type: DeploymentType, framework_path: Path
    ...
    async def get_templates_by_deployment_context(
        self, template_type: Optional[TemplateType] = None, include_development: bool = False
    ) -> List[Dict[str, Any]]:
                    ...
                    try:
                        ...
                        if path.is_dir():
                            ...
                            for pattern in template_patterns:
                                source_info["template_count"] += len(
                                    list(path.glob(f"**/{pattern}"))
                                )

                            validation_results["accessible_templates"] += source_info[
                                "template_count"
                            ]
                        else:
                            validation_results["warnings"].append(
                                f"Template source {source.value} is not a directory: {path}"
                            )
                    except PermissionError:
                        validation_results["warnings"].append(
                            f"Permission denied accessing template source {source.value}: {path}"
                        )
                else:
                    validation_results["inaccessible_templates"] += 1
                    validation_results["warnings"].append(
                        f"Template source {source.value} not found: {path}"
                    )
    ...
    async def get_deployment_specific_template_recommendations(
        self, project_type: str, requirements: List[str] = None
    ) -> List[Dict[str, Any]]:
