...
class LearningIntegrationService:
    ...
    async def create_learning_enhanced_decomposition(
        self, task_description: str, project_name: Optional[str] = None, **kwargs
    ) -> LearningEnhancedDecomposition:
        """
        ...
        try:
            logger.info(
                f"Creating learning-enhanced decomposition for: {task_description[:100]}..."
            )
            ...
            logger.info(
                f"Created learning-enhanced decomposition with {learning_confidence:.2f} confidence"
            )
    ...
    async def get_learning_recommendations(
        self, task_description: str, project_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if similar_patterns:
                ...
                failure_patterns = [
                    p for p in similar_patterns if p.pattern_type == PatternType.FAILURE_PATTERN
                ]
                if failure_patterns:
                    ...
                    recommendations["prevention_strategies"] = self._extract_prevention_strategies(
                        failure_patterns
                    )

                # Analyze success factors
                success_patterns = [
                    p for p in similar_patterns if p.pattern_type == PatternType.SUCCESS_PATTERN
                ]
                if success_patterns:
                    recommendations["success_factors"] = self._extract_success_factors(
                        success_patterns
                    )
                    recommendations["optimization_opportunities"] = self._extract_optimizations(
                        success_patterns
                    )

                # Calculate overall confidence
                avg_confidence = sum(p.confidence_score for p in similar_patterns) / len(
                    similar_patterns
                )
                recommendations["confidence"] = avg_confidence

                # Predict accuracy
                recommendations["estimated_accuracy"] = self._predict_task_accuracy(
                    similar_patterns
                )
    ...
    async def analyze_learning_effectiveness(self, period_days: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            learning_analysis = await self.learning_engine.analyze_learning_effectiveness(
                analysis_period
            )
    ...
    async def _find_relevant_learning_patterns(
        self, task_description: str, project_name: Optional[str]
    ...
    async def _assess_learning_based_risk(
        self,
        task_description: str,
        ...
        if failure_patterns:
            failure_rate = sum((1.0 - p.success_rate) for p in failure_patterns) / len(
                failure_patterns
            )
            risk_assessment["failure_risk"] = min(0.9, failure_rate)
            risk_assessment["overall_risk"] = max(
                risk_assessment["overall_risk"], failure_rate * 0.8
            )

        # Analyze complexity patterns
        if (
            decomposition.complexity == TaskComplexity.COMPLEX
            or decomposition.complexity == TaskComplexity.EPIC
        ):
    ...
    async def _generate_learning_insights(
        ...
        if not patterns:
            insights.append(
                "No similar historical patterns found - proceeding with standard approach"
            )
        ...
        if success_patterns:
            ...
            insights.append(
                f"Similar successful pattern found: {best_pattern.name} ({best_pattern.success_rate:.1%} success)"
            )
        ...
        if failure_patterns:
            ...
            insights.append(
                f"Risk pattern identified: {worst_pattern.name} - implement prevention strategies"
            )
        ...
        if risk_assessment["overall_risk"] > 0.6:
            insights.append(
                "High risk detected - recommend additional planning and risk mitigation"
            )
            adjustments.append("Add risk mitigation phase to decomposition")

        # Efficiency insights
        efficiency_patterns = [
            p for p in patterns if p.pattern_type == PatternType.EFFICIENCY_PATTERN
        ]
    ...
    def _calculate_learning_confidence(
        self, patterns: List[LearningPattern], decomposition: TaskDecomposition
    ) -> float:
    ...
    def _predict_decomposition_accuracy(
        self, decomposition: TaskDecomposition, patterns: List[LearningPattern]
    ) -> float:
    ...
    async def _apply_learning_adjustments(
        ...
        patterns: List[LearningPattern],
        risk_assessment: Dict[str, float],
    ) -> Tuple[float, Optional[DecompositionStrategy]]:
    ...
    def _calculate_risk_adjusted_hours(
        self, base_hours: float, risk_assessment: Dict[str, float]
    ) -> float:
    ...
    def _calculate_risk_level(self, failure_patterns: List[LearningPattern]) -> str:
        ...
        avg_failure_rate = sum((1.0 - p.success_rate) for p in failure_patterns) / len(
            failure_patterns
        )
    ...
    async def _analyze_decomposition_accuracy(
        self, decomposition: TaskDecomposition, outcome: TaskOutcome
    ) -> None:
        """Analyze accuracy of decomposition vs actual outcome."""
        try:
            accuracy = (
                1.0
                - abs(outcome.actual_hours - decomposition.total_estimated_hours)
                / decomposition.total_estimated_hours
            )
            ...
            await self.memory.store_memory(
                ...
                metadata={"type": "decomposition_accuracy", "accuracy_data": accuracy_data},
    ...
    async def _generate_learning_feedback(self, outcome: TaskOutcome) -> Optional[LearningFeedback]:
        ...
        try:
            ...
            accuracy = (
                1.0 - abs(outcome.actual_hours - outcome.estimated_hours) / outcome.estimated_hours
            )
...
# Factory function
def create_learning_integration_service(
    memory: ClaudePMMemory,
    context_manager: Mem0ContextManager,
    learning_mode: LearningMode = LearningMode.ADAPTIVE,
