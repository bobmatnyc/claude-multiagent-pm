...
class ContextBuilder:
    ...
    def build_context(
        ...
        try:
            ...
            if not is_valid:
                self.logger.warning(
                    f"Context validation failed for {context_type}: missing {missing_fields}"
                )
    ...
    def _build_enriched_context(
        ...
        # Generate warnings
        warnings = self._generate_warnings(operation_context, memory_context, analysis_summary)
    ...
    def _analyze_memories(
        self, memory_context: MemoryContext, template: ContextTemplate
    ) -> Dict[str, Any]:
        """
        ...
        for memory in all_memories:
            ...
            if any(
                ...
                for indicator in self.analysis_patterns["success_indicators"]
            ):
            ...
            if any(
                indicator in content_lower
                for indicator in self.analysis_patterns["error_indicators"]
            ):
            ...
            if any(
                indicator in content_lower
                for indicator in self.analysis_patterns["warning_indicators"]
            ):
            ...
            if any(
                indicator in content_lower
                for indicator in self.analysis_patterns["performance_indicators"]
            ):
    ...
    def _build_enriched_data(
        ...
        if memory_context.error_memories:
            enriched_data["error_prevention"] = {
                ...
                "critical_errors": len(
                    [
                        memory
                        for memory in memory_context.error_memories
                        if any(
                            word in memory.content.lower()
                            for word in ["critical", "fatal", "severe"]
                        )
                    ]
                ),
                "recent_errors": len(
        ...
        enriched_data["temporal_context"] = {
            ...
            "experience_age_distribution": {
                ...
                "moderate": analysis_summary["total_memories"]
                - analysis_summary["recent_memories"]
                - analysis_summary["old_memories"],
        ...
        if operation_type == "push":
            if analysis_summary["error_patterns"] > 0:
                recommendations.append(
                    "Run thorough tests before pushing - errors detected in history"
                )
            ...
            if analysis_summary["performance_patterns"] > 0:
                recommendations.append(
                    "Monitor performance during deployment - optimization history available"
                )
            ...
            if analysis_summary["error_patterns"] > 0:
                recommendations.append(
                    "Double-check package integrity - publication errors detected in history"
                )
    ...
    def _get_decision_support_recommendations(
        ...
        if memory_context.decision_memories:
            ...
            positive_decisions = [
                memory
                for memory in memory_context.decision_memories
                if any(
                    word in memory.content.lower() for word in ["successful", "effective", "good"]
                )
            ]

            if positive_decisions:
                recommendations.append(
                    "Favor approaches similar to historically successful decisions"
                )
    ...
    def _get_performance_optimization_recommendations(
        ...
        if memory_context.performance_memories:
            ...
            for memory in memory_context.performance_memories[:3]:
                if any(
                    word in memory.content.lower() for word in ["faster", "optimized", "improved"]
                ):
                    recommendations.append(f"Consider optimization: {memory.content[:50]}...")

        if analysis_summary["performance_patterns"] > 2:
            recommendations.append(
                "Extensive performance history available - leverage optimization insights"
            )
    ...
    def _generate_warnings(
        ...
        if operation_type in ["push", "deploy", "publish"]:
            if any(
                "rollback" in memory.content.lower() for memory in memory_context.error_memories
            ):
                warnings.append(
                    f"Previous {operation_type} operations required rollbacks - ensure safety measures"
                )

        return warnings[:5]  # Limit to 5 warnings

    def _calculate_confidence_score(
        self, memory_context: MemoryContext, analysis_summary: Dict[str, Any], warning_count: int
    ) -> float:
        """
        ...
        if memory_context.similarity_scores:
            avg_similarity = sum(memory_context.similarity_scores.values()) / len(
                memory_context.similarity_scores
            )
    ...
    def _update_build_stats(self, processing_time_ms: float, success: bool, confidence: float):
        ...
        self.build_stats["average_build_time_ms"] = (
            current_avg * (total_builds - 1) + processing_time_ms
        ) / total_builds
