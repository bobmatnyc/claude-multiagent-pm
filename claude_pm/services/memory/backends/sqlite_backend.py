...
class SQLiteBackend(MemoryBackend):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.pragma_settings = self.get_config(
            "pragma_settings",
            {
                "synchronous": "NORMAL",
                "cache_size": 10000,
    ...
    async def _configure_database(self):
        ...
        try:
            ...
            raise BackendInitializationError(
                f"Database configuration failed: {e}", "sqlite", "configuration"
            )
    ...
    async def _verify_schema(self):
        ...
        try:
            ...
            if not await cursor.fetchone():
                raise BackendInitializationError(
                    "memories table not found", "sqlite", "verification"
                )
            ...
            if self.enable_fts:
                ...
                if not await cursor.fetchone():
                    raise BackendInitializationError(
                        "memories_fts table not found", "sqlite", "verification"
                    )
            ...
            if not version_row or version_row[0] != self._schema_version:
                self.logger.warning(
                    f"Schema version mismatch: expected {self._schema_version}, got {version_row[0] if version_row else 'none'}"
                )

        except Exception as e:
            raise BackendInitializationError(
                f"Schema verification failed: {e}", "sqlite", "verification"
            )
    ...
    def _build_standard_query(
        self, query: MemoryQuery, sql_params: List[Any]
    ) -> tuple[str, List[Any]]:
    ...
    async def get_project_memories(
        ...
        try:
            ...
            raise BackendError(
                f"Failed to get project memories: {e}", "sqlite", "get_project_memories", e
            )
    ...
    async def restore_backup(self, backup_path: str) -> bool:
        ...
        try:
            if not Path(backup_path).exists():
                raise BackendError(
                    f"Backup file not found: {backup_path}", "sqlite", "restore_backup"
                )
