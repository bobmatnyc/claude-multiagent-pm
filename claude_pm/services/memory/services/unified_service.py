...
from ..interfaces.models import (
    MemoryItem, MemoryQuery, MemoryCategory, HealthStatus, 
    OperationResult, MemoryStatistics
)
from ..interfaces.exceptions import (
    MemoryServiceError, BackendNotAvailableError, CircuitBreakerOpenError,
...
class FlexibleMemoryService:
    ...
    def _initialize_backends(self):
        ...
        try:
            ...
            self.fallback_chain = [
                backend for backend in self.fallback_chain 
                if backend in self.backends
            ]
            
            self.logger.info(f"Initialized {len(self.backends)} backends: {list(self.backends.keys())}")
        ...
        for backend_name in self.fallback_chain:
            ...
            try:
                ...
                result = await circuit_breaker.call(
                    self._execute_backend_operation,
                    backend, operation, **kwargs
                )
                ...
                if self.active_backend_name != backend_name:
                    self.logger.info(f"Switching active backend from {self.active_backend_name} to {backend_name}")
