...
class AutoDetectionEngine:
    ...
    async def detect_best_backend(
        self, backends: Dict[str, MemoryBackend], priority_order: List[str] = None
        ...
        self.logger.info(
            f"Selected backend: {best_candidate.backend_name} (score: {best_candidate.score:.2f})"
        )
        ...
        for candidate in candidates[1:]:
            self.logger.debug(
                f"Alternative: {candidate.backend_name} (score: {candidate.score:.2f})"
            )
    ...
    async def _check_all_backends_health(
        ...
        backends_to_check = {
            name: backend for name, backend in backends.items() if name not in health_results
        }
    ...
    async def _check_backend_health_with_retry(
        self, name: str, backend: MemoryBackend
    ) -> BackendHealth:
    ...
    async def _check_backend_health(self, name: str, backend: MemoryBackend) -> BackendHealth:
        ...
        try:
            ...
            if not backend.is_initialized():
                init_success = await asyncio.wait_for(backend.initialize(), timeout=self.timeout)
                if not init_success:
                    raise Exception("Backend initialization failed")

            # Perform health check
            health_check = asyncio.wait_for(backend.health_check(), timeout=self.timeout)
    ...
    def _calculate_backend_score(
        self, backend_name: str, health: BackendHealth, backend: MemoryBackend
    ) -> float:
