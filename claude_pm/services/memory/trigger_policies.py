...
class TriggerPolicyEngine:
    ...
    def _setup_default_policies(self):
        ...
        workflow_policy = PolicyConfig(
            ...
            rate_limits={"per_project_per_hour": 10, "per_workflow_per_hour": 3},
            batch_settings={"batch_size": 5, "batch_timeout": 30},
        )

        # High priority for successful workflows
        workflow_policy.add_rule(
            PolicyRule(
        ...
        issue_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_project_per_hour": 15, "per_issue_per_day": 1},
        )

        # Critical issues always captured
        issue_policy.add_rule(
            PolicyRule(
                name="critical_issue",
        ...
        agent_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_agent_per_hour": 20, "per_project_per_hour": 50},
            batch_settings={"batch_size": 10, "batch_timeout": 60},
        )

        # QA operations are important
        agent_policy.add_rule(
            PolicyRule(
                name="qa_operations",
        ...
        error_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_error_type_per_hour": 5, "per_project_per_hour": 20},
        )

        # All error resolutions are high priority
        error_policy.add_rule(
            PolicyRule(
        ...
        milestone_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_project_per_day": 5},
        )

        # All milestones are important
        milestone_policy.add_rule(
            PolicyRule(
        ...
        knowledge_policy = PolicyConfig(
            ...
            rate_limits={"per_project_per_hour": 100},
            batch_settings={"batch_size": 20, "batch_timeout": 120},
        )

        # Important knowledge should be processed immediately
        knowledge_policy.add_rule(
            PolicyRule(
        ...
        pattern_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_pattern_per_hour": 10, "per_project_per_hour": 30},
        )
        ...
        decision_policy = PolicyConfig(
            enabled=True,
            default_decision=PolicyDecision.ALLOW,
            rate_limits={"per_project_per_hour": 25},
        )

        # Critical decisions are always captured
        decision_policy.add_rule(
            PolicyRule(
    ...
    def remove_policy_rule(self, trigger_type: TriggerType, rule_name: str) -> bool:
        ...
        if removed:
            self.logger.info(
                f"Removed policy rule '{rule_name}' from trigger type: {trigger_type.value}"
            )
