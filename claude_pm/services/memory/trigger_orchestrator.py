...
class MemoryTriggerOrchestrator:
    ...
    async def trigger_memory_creation(self, event: TriggerEvent) -> TriggerResult:
        ...
        if not self._enabled:
            return TriggerResult(success=False, skipped_reason="Memory triggers disabled")

        if not self._initialized:
            return TriggerResult(success=False, error="Memory trigger orchestrator not initialized")
        ...
        try:
            ...
            if not await self._should_process_trigger(event):
                return TriggerResult(success=False, skipped_reason="Trigger filtered by policy")
            ...
            return TriggerResult(
                success=False, error=str(e), processing_time_ms=(time.time() - start_time) * 1000
            )
            ...
            try:
                ...
                try:
                    for _ in range(batch_size):
                        event = await asyncio.wait_for(self._trigger_queue.get(), timeout=1.0)
                        batch.append(event)
        ...
        for event in batch:
            try:
                ...
                if result.success:
                    ...
                    self.logger.warning(
                        f"Failed to process trigger {event.event_id}: {result.error}"
                    )
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {
            "queue_size": self._trigger_queue.qsize(),
            "active_triggers": len(self._active_triggers),
            "processing_task_running": (self._processing_task and not self._processing_task.done()),
            "enabled": self._enabled,
        }
