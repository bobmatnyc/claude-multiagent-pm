...
async def _execute_with_memory_trigger(
    func: Callable, config: MemoryTriggerConfig, args: tuple, kwargs: dict
) -> Any:
    ...
    try:
        ...
        if config.capture_on_success and config.hook_type:
            hook_kwargs = {"success": True, "result": result, **context.metadata}
        ...
        if config.capture_on_error:
            error_hook_type = (
                config.hook_type + "_error" if config.hook_type else "error_resolution"
            )
            hook_kwargs = {
                "success": False,
                "error": str(e),
...
def _sanitize_args(args: tuple, kwargs: dict) -> dict:
    ...
    if kwargs:
        sanitized["kwargs"] = {key: _sanitize_value(value) for key, value in kwargs.items()}
...
def _sanitize_value(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {key: _sanitize_value(val) for key, val in list(value.items())[:10]}  # Limit size
    ...
    if hasattr(value, "__dict__"):
        return {
            key: _sanitize_value(val) for key, val in list(value.__dict__.items())[:5]  # Limit size
        }
...
class MemoryTriggerContext:
    ...
    def __init__(
        self, operation_name: str, project_name: str, source: str, hook_type: str, **kwargs
    ):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.hooks and self.context:
            ...
            await self.hooks.execute_hook(
                complete_hook_type, self.context, success=success, **self.kwargs
            )
...
def workflow_trigger_context(
    operation_name: str, project_name: str, workflow_type: Optional[str] = None, **kwargs
...
def agent_trigger_context(
    operation_name: str, project_name: str, agent_type: str, **kwargs
) -> MemoryTriggerContext:
...
def issue_trigger_context(
    operation_name: str, project_name: str, issue_id: str, **kwargs
) -> MemoryTriggerContext:
...
def trigger_immediate_memory(
    ...
    async def execute_hook():
        return await hooks.knowledge_capture(
            context, content=content, knowledge_type="manual", category=category.value
        )
...
def get_memory_trigger_metrics() -> Dict[str, Any]:
    ...
    return {"enabled": True, "hooks_available": True, **hooks.get_metrics()}
