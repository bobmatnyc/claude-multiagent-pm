...
class SimilarityMatcher:
    ...
    def rank_memories_by_similarity(
        ...
        for memory in memories:
            ...
            # Apply thresholds
            if (self.config.min_similarity_threshold <= 
    ...
    def _calculate_cosine_similarity(
        ...
        for token in all_tokens:
            doc_freq = sum([
                1 for tokens in [query_tokens, memory_tokens] 
                if token in tokens
            ])
        ...
        for query_token in query_tokens:
            for memory_token in memory_tokens:
                ...
                for group_key, group_terms in conceptual_groups.items():
                    if (query_token in group_terms and memory_token in group_terms) or \
                       (query_token == group_key and memory_token in group_terms) or \
                       (query_token in group_terms and memory_token == group_key):
                        matches += 1
                        break
    ...
    def _extract_structure_features(self, text: str) -> Dict[str, float]:
        ...
        features['punctuation_ratio'] = sum(1 for c in text if not c.isalnum() and not c.isspace()) / len(text)
    ...
    def _compare_metadata_patterns(self, metadata1: Dict[str, Any], metadata2: Dict[str, Any]) -> float:
