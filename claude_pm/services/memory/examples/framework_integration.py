...
async def complex_workflow_with_context_manager():
    ...
    async with workflow_trigger_context(
        ...
        # Set final result
        ctx.set_result({"status": "success", "duration": "0.4 seconds", "steps_completed": 3})
...
async def agent_operation_with_context_manager():
    ...
    async with agent_trigger_context(
        operation_name="comprehensive_qa", project_name="claude-multiagent-pm", agent_type="qa"
    ) as ctx:
        ...
        # Execute tests
        test_results = {"total_tests": 50, "passed": 48, "failed": 2, "coverage": 85}
...
class MemoryIntegratedErrorHandler:
    ...
    @error_memory_trigger(error_type="memory_backend_failure", project_name="claude-multiagent-pm")
    async def handle_memory_backend_failure(
        self, backend_name: str, error: Exception
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            fallback_result = await self.memory_service.get_memory_service().switch_backend(
                "sqlite"
            )
...
async def run_memory_trigger_demo():
    ...
    async with create_memory_trigger_service(config) as service:
        ...
        issue_result = await resolve_memory_integration_issue(
            "ISS-001", "Added memory trigger hooks"
        )
        ...
        knowledge_result = await capture_solution_pattern(
            "async_memory_integration", "Use decorators and context managers"
        )
        ...
        decision_result = await record_architecture_decision(
            "Use trigger-based memory system", "Provides automatic, policy-driven memory creation"
        )
