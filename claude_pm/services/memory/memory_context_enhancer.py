...
class MemoryContextEnhancer:
    ...
    async def _perform_intelligent_recall(
        ...
        if self.config.enable_pattern_matching:
            ...
            search_tasks.append(("pattern", self.memory_service.search_memories(project_name, pattern_query)))
        ...
        search_tasks.append(("error", self.memory_service.search_memories(project_name, error_query)))
        ...
        search_tasks.append(("project", self.memory_service.search_memories(project_name, project_query)))
        ...
        performance_memories = [
            memory for memory in pattern_memories 
    ...
    def _is_decision_memory(self, memory: MemoryItem, operation_context: Dict[str, Any]) -> bool:
        ...
        """
        decision_keywords = [
            "decision", "choice", "approach", "strategy", "architecture",
            "design", "requirement", "specification", "mandate"
        ]
    ...
    def _is_performance_memory(self, memory: MemoryItem, operation_context: Dict[str, Any]) -> bool:
        ...
        performance_keywords = [
            ...
            "bottleneck", "latency", "throughput", "efficiency"
        ]
    ...
    def _rank_memories_by_relevance(
        ...
        return [memory for memory, _ in scored_memories[:self.config.max_memories_per_category * 2]]
    ...
    def _cache_context(self, cache_key: str, context: MemoryContext):
        ...
        if len(self._recall_cache) >= self.config.performance_cache_size:
            # Remove oldest entry
            oldest_key = min(self._recall_cache.keys(), 
    ...
    def _update_performance_stats(self, recall_time_ms: float, success: bool):
        ...
        self.recall_stats["average_recall_time_ms"] = (
            (current_avg * (total_recalls - 1) + recall_time_ms) / total_recalls
        )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        # Add success rate
        stats["success_rate"] = (
            self.recall_stats["successful_recalls"] / 
            max(self.recall_stats["total_recalls"], 1)
        ) * 100
