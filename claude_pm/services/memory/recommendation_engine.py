    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "by_type": {
                    rec_type.value: len(self.get_by_type(rec_type))
...
class RecommendationEngine:
    ...
    def __init__(
        self, similarity_matcher: SimilarityMatcher, config: Optional[RecommendationConfig] = None
    ):
        """
    ...
    def generate_recommendations(self, enriched_context: EnrichedContext) -> RecommendationSet:
        """
    ...
    def _generate_error_prevention_recommendations(
        self, enriched_context: EnrichedContext
    ) -> List[Recommendation]:
        ...
        for pattern in error_patterns:
            if (
                pattern["frequency"] >= 2
                and pattern["confidence"] >= self.config.min_confidence_threshold
            ):
    ...
    def _generate_workflow_recommendations(
        ...
        team_memories = [
            m for m in memory_context.relevant_memories if m.category == MemoryCategory.TEAM
        ]
    ...
    def _generate_context_specific_recommendations(
        ...
        if enriched_context.context_type == ContextType.AGENT_OPERATION:
            ...
            recommendations.extend(
                self._generate_workflow_command_recommendations(enriched_context)
            )
        elif enriched_context.context_type == ContextType.ERROR_RESOLUTION:
            recommendations.extend(
                self._generate_error_resolution_recommendations(enriched_context)
            )
        elif enriched_context.context_type == ContextType.DECISION_SUPPORT:
            recommendations.extend(
                self._generate_decision_support_recommendations(enriched_context)
            )
        elif enriched_context.context_type == ContextType.PERFORMANCE_OPT:
            recommendations.extend(
                self._generate_performance_context_recommendations(enriched_context)
            )
    ...
    def _generate_performance_context_recommendations(
        ...
        if memory_context.performance_memories:
            optimization_approaches = [
                memory
                for memory in memory_context.performance_memories
                if any(
    ...
    def _analyze_performance_patterns(
        self, perf_memories: List[MemoryItem]
    ) -> List[Dict[str, Any]]:
    ...
    def _create_error_prevention_recommendation(
        self, pattern: Dict[str, Any], memory_context: MemoryContext
    ...
    def _filter_and_rank_recommendations(
        ...
        filtered = [
            rec for rec in recommendations if rec.confidence >= self.config.min_confidence_threshold
        ]
    ...
    def _update_generation_stats(self, processing_time_ms: float, success: bool, rec_count: int):
        ...
        self.generation_stats["average_generation_time_ms"] = (
            current_avg * (total_generations - 1) + processing_time_ms
        ) / total_generations
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if total_generations > 0:
            stats["success_rate"] = (
                self.generation_stats["successful_generations"] / total_generations
            ) * 100
