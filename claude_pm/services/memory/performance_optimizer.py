...
class PerformanceMetrics:
    ...
    def meets_performance_targets(self) -> bool:
        ...
        return (
            self.average_recall_time_ms <= 100.0
            and self.p95_recall_time_ms <= 200.0
            and self.success_rate >= 0.95
            and self.cache_hit_rate >= 0.3
        )
...
class PerformanceOptimizer:
    ...
    def _calculate_success_metrics(self, service_stats: Dict[str, Any]) -> Dict[str, float]:
        ...
        if total_recalls == 0:
            return {"success_rate": 0.0, "error_rate": 0.0, "timeout_rate": 0.0}
    ...
    def _calculate_throughput_metrics(self, service_stats: Dict[str, Any]) -> Dict[str, float]:
        ...
        concurrent_recalls_avg = sum(self.concurrent_recalls_history) / max(
            len(self.concurrent_recalls_history), 1
        )
    ...
    def _calculate_quality_metrics(self, service_stats: Dict[str, Any]) -> Dict[str, float]:
        ...
        # These would need to be tracked by the service
        return {"avg_confidence_score": 0.0, "recommendations_per_recall": 0.0}
        ...
        if metrics.avg_memories_per_recall > 20:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    implementation_effort="low",
                    config_changes={"recall_config.max_memory_age_days": 60},
    ...
    def configure_optimization(
        ...
        if optimization_aggressiveness is not None:
            if optimization_aggressiveness in ["conservative", "moderate", "aggressive"]:
                ...
                raise ValueError(
                    "optimization_aggressiveness must be 'conservative', 'moderate', or 'aggressive'"
                )
