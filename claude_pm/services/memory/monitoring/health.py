...
class HealthMonitor:
    ...
    async def start_monitoring(self, backends: Dict[str, MemoryBackend]):
        ...
        for name, backend in backends.items():
            self.current_health[name] = BackendHealth(
                backend_name=name, is_healthy=False, response_time=0.0
            )
    ...
    async def _check_all_backends(self):
        ...
        for name, backend in self.backends.items():
            task = asyncio.create_task(
                self._check_backend_health(name, backend), name=f"health_check_{name}"
            )
            tasks.append(task)
    ...
    def _record_health_check(
        ...
        self.health_history[backend_name] = [
            record
            for record in self.health_history[backend_name]
    ...
    def _generate_alert(self, backend_name: str, alert_type: str, message: str, severity: str):
        """Generate a health alert."""
        # Check if similar alert already exists
        for alert in self.alerts:
            if (
                alert.backend_name == backend_name
    ...
    def _resolve_alerts(self, backend_name: str):
        ...
        for alert in self.alerts:
            if alert.backend_name == backend_name and not alert.resolved:
                alert.resolved = True
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "backends": {name: health.to_dict() for name, health in self.current_health.items()},
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_check": max(
                (health.last_checked for health in self.current_health.values()), default=0
            ),
        }
    ...
    def get_alerts(
        self, backend_name: Optional[str] = None, resolved: Optional[bool] = None
    ) -> List[Dict[str, Any]]:
        """
        Get alerts.
    ...
    def export_health_report(self, format: str = "json") -> str:
        ...
        report_data = {
            ...
            "backend_health": {
                name: self.get_backend_health(name) for name in self.backends.keys()
            },
