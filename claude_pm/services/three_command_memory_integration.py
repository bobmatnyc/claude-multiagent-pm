...
class ThreeCommandMemoryIntegration:
    ...
    def __init__(self, memory_service: MemoryTriggerService):
        ...
        self.command_configs = {
            ...
            "deploy": {
                ...
                "quality_gates": [
                    QualityGate.DEPLOYMENT_VALIDATION,
    ...
    async def advance_workflow_phase(
        ...
        if phase in [CommandPhase.DOCUMENTATION_VALIDATION, CommandPhase.QUALITY_ASSURANCE, CommandPhase.OPERATIONS]:
    ...
    async def complete_command_workflow(
        ...
        self.logger.info(f"Completed {workflow.command} workflow: {workflow_id} (success: {success})")
        return summary
    ...
    async def _recall_command_memories(self, command: str, project_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recommendations = []
            if success_rate < 0.8:
                recommendations.append(f"Command has {success_rate:.1%} success rate - review common issues")
            if avg_duration > 300:  # 5 minutes
                recommendations.append(f"Average duration is {avg_duration:.1f}s - consider optimization")
            if common_failures:
    ...
    def _calculate_performance_metrics(self, workflow: CommandWorkflowState) -> Dict[str, float]:
        ...
        metrics["quality_gates_rate"] = (
            ...
            if (workflow.quality_gates_passed or workflow.quality_gates_failed) else 1.0
        )
        ...
        if workflow.command in self.performance_baselines:
            ...
            metrics["duration_vs_baseline"] = (
                metrics["duration"] / baseline.get("duration", metrics["duration"])
            )
            metrics["success_vs_baseline"] = (
                metrics["success_rate"] / baseline.get("success_rate", 1.0)
            )
    ...
    def _update_success_rates(self, command: str, success: bool):
        ...
        self.success_rates[command] = current_rate * (1 - weight) + (1.0 if success else 0.0) * weight
    ...
    async def _create_workflow_memory(self, workflow: CommandWorkflowState, phase: str):
        ...
        try:
            ...
            trigger_event = TriggerEvent(
                trigger_type=TriggerType.WORKFLOW_COMPLETION if phase == "completion" else TriggerType.WORKFLOW_TRANSITION,
    ...
    async def get_command_analytics(self, command: str, project_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics["quality_gates_success_rate"] = (
                total_passed / (total_passed + total_failed) if (total_passed + total_failed) > 0 else 0
            )
            ...
            recent_memories = sorted(memories, key=lambda m: m.metadata.get("timestamp", ""), reverse=True)[:10]
            recent_successes = sum(1 for m in recent_memories if m.metadata.get("success", False))
            analytics["recent_success_rate"] = recent_successes / len(recent_memories) if recent_memories else 0
...
def create_three_command_memory_integration(memory_service: MemoryTriggerService) -> ThreeCommandMemoryIntegration:
    """
