...
class AITrackdownHealthCollector(HealthCollector):
    ...
    async def _check_task_system(self) -> ServiceHealthReport:
        ...
        try:
            ...
            epics_count = (
                len(list((self.tasks_path / "epics").glob("*.md")))
                if (self.tasks_path / "epics").exists()
                else 0
            )
            issues_count = (
                len(list((self.tasks_path / "issues").glob("*.md")))
                if (self.tasks_path / "issues").exists()
                else 0
            )
            tasks_count = (
                len(list((self.tasks_path / "tasks").glob("*.md")))
                if (self.tasks_path / "tasks").exists()
                else 0
            )
    ...
    async def _check_migration_progress(self) -> ServiceHealthReport:
        ...
        try:
            ...
            migration_task = (
                self.tasks_path / "tasks" / "TSK-0001-complete-ticket-data-migration.md"
            )
            ...
            if migration_task.exists():
                ...
                return create_service_health_report(
                    ...
                    metrics={"migration_task_exists": False, "expected_path": str(migration_task)},
                )
        ...
        try:
            ...
            for command_args, command_name in test_commands:
                try:
                    ...
                    command_results[command_name] = {"success": False, "error": "Command timed out"}
                except Exception as e:
    ...
    async def _check_ticket_system_integrity(self) -> ServiceHealthReport:
        ...
        try:
            ...
            current_issue = (
                self.tasks_path / "issues" / "ISS-0002-comprehensive-health-slash-command.md"
            )
