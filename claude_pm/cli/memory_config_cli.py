...
Examples:
  ...
  %(prog)s stats --config config.yaml
        ...
        generate_parser.add_argument(
            "--minimal", action="store_true", help="Generate minimal configuration"
        )
        ...
        policy_subparsers = policy_parser.add_subparsers(
            dest="policy_command", help="Policy operations"
        )
        ...
        policy_list_parser.add_argument(
            "--scope",
            choices=["global", "agent", "workflow", "task", "user", "session"],
        ...
        policy_add_parser.add_argument(
            "--validate", action="store_true", help="Validate before adding"
        )
        ...
        policy_remove_parser.add_argument(
        ...
        monitor_parser.add_argument(
            "--alerts", action="store_true", help="Enable alert notifications"
        )
        ...
        backup_parser.add_argument(
            "--include-policies", action="store_true", help="Include policy rules"
        )
    ...
    def cmd_generate(self, args) -> int:
        ...
        try:
            ...
            if args.minimal:
                ...
                template_path = (
                    Path(__file__).parent.parent / "config" / "environments" / f"{args.env}.yaml"
                )
    ...
    def cmd_deploy(self, args) -> int:
        ...
        try:
            ...
            if config.environment != target_env:
                if not args.dry_run:
                    logger.warning(
                        f"Configuration environment ({config.environment.value}) "
    ...
    def cmd_policy_list(self, args) -> int:
        ...
        try:
            ...
            for rule in rules:
                ...
                print(
                    f"{status} {rule.name} (priority: {rule.priority}, scope: {rule.scope.value})"
                )
                if rule.description:
    ...
    def cmd_policy_remove(self, args) -> int:
        ...
        try:
            ...
            if not args.force:
                response = input(
                    f"Are you sure you want to remove policy rule '{args.name}'? [y/N]: "
                )
                if response.lower() != "y":
                    print("Cancelled")
                    return 0
        ...
        try:
            ...
            stats = {
                ...
                "policies": {
                    ...
                    "types": list(
                        set(p.trigger_type.value for p in config.trigger_policies.values())
                    ),
                },
                "features": {
            ...
            if args.format == "json":
                ...
                print(
                    f"  Pool: {stats['backend']['pool_enabled']} (size: {stats['backend']['pool_size']})"
                )
                ...
                try:
                    ...
                    if errors:
                        ...
                        if args.alerts:
                            ...
                            logger.warning(
                                f"Configuration health alert: {len(errors)} errors detected"
                            )
                    else:
                        status = "✅ HEALTHY"
