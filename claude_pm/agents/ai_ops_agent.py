...
class AIOpsAgent(BaseAgent):
        ...
        try:
            ...
            if request.cost_budget:
                ...
                if remaining_budget < request.cost_budget:
                    raise ValueError(f"Insufficient budget: {remaining_budget} < {request.cost_budget}")
    ...
    async def optimize_provider_selection(
        ...
        self.logger.info(f"Selected optimal provider: {optimal_provider} (score: {provider_scores[optimal_provider]})")
    ...
    async def manage_enterprise_security(
        ...
        try:
            ...
            recent_events = await self.security_framework.get_recent_security_events(
                hours=24
            )
            ...
            if recent_events:
                ...
                if any(event.get("severity") == "high" for event in recent_events):
                    ...
                    security_status["recommendations"].append("Review high-severity security events")
    ...
    async def monitor_performance(
        ...
        try:
            ...
            performance_metrics["service_performance"]["ai_service_manager"] = await self.ai_service_manager.get_performance_metrics()
            performance_metrics["service_performance"]["cost_manager"] = await self.cost_manager.get_performance_metrics()
            performance_metrics["service_performance"]["tools_manager"] = await self.tools_manager.get_performance_metrics()
    ...
    async def get_agent_status(self) -> Dict[str, Any]:
        ...
        ai_ops_status = {
            "providers_available": await self.ai_service_manager.get_available_providers() if hasattr(self, 'ai_service_manager') else [],
            "active_requests": len(self.active_requests),
            "total_requests": self.total_requests,
            "success_rate": (self.successful_requests / self.total_requests) if self.total_requests > 0 else 0,
            "total_cost": self.total_cost,
