...
class MemoryEnhancedAgent:
    ...
    async def execute_with_memory(
        ...
        try:
            ...
            result = await self.wrapped_agent.execute(
                operation=operation, context=enhanced_context, **kwargs
            )
    ...
    def _analyze_memory_patterns(self, memories: List[MemoryItem]) -> List[str]:
        ...
        error_types = [
            m.metadata.get("error_type") for m in memories if m.metadata.get("error_type")
        ]
        if error_types:
    ...
    def _generate_memory_recommendations(
        self, memories: List[MemoryItem], operation: str
    ) -> List[str]:
        ...
        if success_rate < 0.7:
            recommendations.append(
                f"Operation has {success_rate:.1%} success rate - consider reviewing approach"
            )
        ...
        if execution_times:
            ...
            if avg_time > 300:
                recommendations.append(
                    f"Average execution time is {avg_time:.1f}s - consider optimization"
                )
    ...
    async def _enhance_context_with_memory(
        self, context: Dict[str, Any], memory_context: AgentMemoryContext
    ) -> Dict[str, Any]:
        ...
        enhanced_context["memory_insights"] = {
            ...
            "historical_success_rate": self._calculate_success_rate(
    ...
    async def _create_operation_memory(
        ...
        try:
            ...
            pattern = self.pattern_registry.get_pattern(self.wrapped_agent.agent_type, operation)
    ...
    async def _create_error_memory(
        self, operation: str, context: Dict[str, Any], error: str
    ) -> Optional[str]:
    ...
    async def _enhance_result_with_memory(
        self, result: Any, memory_context: AgentMemoryContext, execution_time: float
    ) -> Any:
