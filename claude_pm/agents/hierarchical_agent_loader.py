...
class HierarchicalAgentLoader(BaseService):
        ...
        try:
            ...
            for agent_type in essential_agents:
                if not any(
                    ...
                    for agent in self.hierarchy.system_agents.values()
                ):
                    validation_results["warnings"].append(
                        f"Missing essential system agent: {agent_type}"
                    )
    ...
    async def create_agent_from_template(
        ...
        try:
            ...
            agent_content = self._substitute_template_variables(
                template_content, agent_name, agent_type, tier
            )
    ...
    def _substitute_template_variables(
        self, template_content: str, agent_name: str, agent_type: str, tier: str
    ) -> str:
    ...
    async def get_agent_health_status(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            "loaded_agents": len(self._loaded_agents),
            "tiers": {
                "system": {"count": len(self.hierarchy.system_agents), "agents": {}},
                "user": {"count": len(self.hierarchy.user_agents), "agents": {}},
                "project": {"count": len(self.hierarchy.project_agents), "agents": {}},
            },
        }
