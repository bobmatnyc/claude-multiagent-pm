...
class BasicAgentConfigValidator(ConfigurationValidator):
    ...
    def validate(self, config: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        # Validate agent type
        valid_types = [
            "engineer", "ops", "qa", "security", "architect", 
            "orchestrator", "pm", "data", "ui", "integration",
            "performance", "research", "documentation", "scaffolding"
        ]
...
class AgentConfigurationManager:
    ...
    def __init__(
        ...
        self.validators: Dict[str, ConfigurationValidator] = {
            "basic": BasicAgentConfigValidator()
        }
    ...
    def _ensure_config_directories(self) -> None:
        ...
        for directory in directories:
            ...
            if not default_config_file.exists():
                self._create_default_config_file(default_config_file, self._get_tier_from_path(directory))
    ...
    async def _load_tier_configurations(self, tier: str, config_path: Path, priority: int) -> None:
        ...
        for config_file in config_path.glob("*.yaml"):
            try:
                source = await self._load_configuration_source(
                    config_file, tier, priority
                )
                if source:
                ...
                for source in self.config_sources[tier]:
                    for config_name, config_data in source.config_data.items():
                        if (isinstance(config_data, dict) and 
                            config_data.get("type") == agent_type):
    ...
    def _deep_merge_config(self, base_config: Dict[str, Any], override_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_configuration_status(self) -> Dict[str, Any]:
        ...
        total_sources = sum(len(sources) for sources in self.config_sources.values())
        valid_sources = sum(
            len([s for s in sources if s.valid]) 
            for sources in self.config_sources.values()
        )
        ...
        return {
            ...
            "tiers": {
                tier: {
                    "sources": len(sources),
                    "valid": len([s for s in sources if s.valid])
                }
