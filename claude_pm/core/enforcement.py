...
class FileClassifier:
    ...
    FILE_CATEGORY_PATTERNS = {
        FileCategory.TEST_FILES: [
            ...
            r'\.test\.ts$', r'\.spec\.ts$', r'^test/', r'^tests/',
            r'/test/', r'/tests/', r'__tests__/.*', r'spec/.*'
        ],
        FileCategory.PROJECT_MANAGEMENT: [
            r'^CLAUDE\.md$', r'^BACKLOG\.md$', r'^MILESTONES\.md$', 
            r'trackdown/.*\.md$', r'\.ticket$', r'STATUS.*\.md$'
        ],
        FileCategory.RESEARCH_DOCS: [
            r'research/.*\.md$', r'analysis/.*\.md$', r'docs/research/.*',
            r'investigation/.*\.md$', r'evaluation/.*\.md$'
        ],
        FileCategory.SCAFFOLDING: [
            r'\.template$', r'scaffold/.*', r'templates/.*',
            r'api-spec/.*', r'openapi.*\.yml$', r'swagger.*\.yml$'
        ],
        FileCategory.CONFIGURATION: [
            r'docker.*', r'^Dockerfile$', r'\.yml$', r'\.yaml$', 
            r'\.toml$', r'\.ini$', r'\.conf$', r'\.config$', r'\.env$',
            r'requirements\.txt$', r'package\.json$', r'pyproject\.toml$',
            r'^Makefile$', r'\.sh$', r'\.bat$', r'\.cmd$'
        ],
        FileCategory.DOCUMENTATION: [
            r'\.md$', r'\.rst$', r'\.txt$', r'\.doc$', r'\.docx$',
            r'README.*', r'CHANGELOG.*', r'LICENSE.*', r'CONTRIBUTING.*'
        ],
        FileCategory.SOURCE_CODE: [
            r'\.py$', r'\.js$', r'\.ts$', r'\.jsx$', r'\.tsx$', r'\.java$',
            r'\.cpp$', r'\.c$', r'\.h$', r'\.go$', r'\.rs$', r'\.php$',
            r'\.rb$', r'\.swift$', r'\.kt$', r'\.scala$', r'\.cs$'
        ]
    }
...
class AgentCapabilityManager:
    ...
    def _initialize_agent_permissions(self) -> Dict[AgentType, AgentPermissions]:
        ...
        permissions[AgentType.ORCHESTRATOR] = AgentPermissions(
            ...
            allowed_action_types={ActionType.READ, ActionType.WRITE, ActionType.CREATE, ActionType.MODIFY},
            can_delegate=True,
        ...
        for agent_type in [AgentType.SECURITY_ENGINEER, AgentType.PERFORMANCE_ENGINEER, 
                          AgentType.DEVOPS_ENGINEER, AgentType.DATA_ENGINEER, 
                          AgentType.UI_UX_ENGINEER, AgentType.CODE_REVIEW_ENGINEER]:
    ...
    def get_agent_permissions(self, agent_type: AgentType) -> AgentPermissions:
        ...
        return self.agent_permissions.get(agent_type, AgentPermissions(
            agent_type=agent_type,
    ...
    def validate_agent_action(self, agent: Agent, action: Action) -> ValidationResult:
        ...
        if file_category in permissions.forbidden_file_categories:
            ...
            warnings.append(f"Agent {agent.agent_type.value} accessing {file_category.value} files - verify authorization")
        ...
        for violation in violations:
            if (violation.violation_type == "forbidden_file_access" and 
                agent.agent_type == AgentType.ORCHESTRATOR and 
                file_category == FileCategory.SOURCE_CODE):
    ...
    def detect_circular_delegation(self, delegation_chain: List[Agent]) -> bool:
        ...
        for agent in delegation_chain:
            if agent.agent_type in agent_types_seen:
                logger.error(f"Circular delegation detected: {[a.agent_type.value for a in delegation_chain]}")
                return True
...
class ViolationMonitor:
    ...
    def generate_violation_report(self, start_time: Optional[datetime] = None,
                                end_time: Optional[datetime] = None) -> ViolationReport:
        """
...
class EnforcementEngine:
    ...
    def enforce_file_access(self, agent_type: str, file_path: str, action_type: str = "read") -> bool:
        """
...
def validate_agent_action(agent_type: str, action_type: str, resource_path: str,
                         agent_id: str = None) -> ValidationResult:
    """
    ...
    try:
        ...
        agent = Agent(
            agent_id=agent_id or f"validate-{agent_type}",
            agent_type=agent_enum
        )
        action = Action(
        ...
        dummy_action = Action(action_type=ActionType.READ, resource_path=Path(resource_path), agent=dummy_agent)
        return ValidationResult(
            is_valid=False,
            agent=dummy_agent,
            action=dummy_action,
