...
class MemoryConfigManager:
    ...
    def _initialize_default_configs(self):
        ...
        self.configs["development"] = MemoryEnvironmentConfig(
            ...
            memory_config=ClaudePMConfig(
                host=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_HOST",
                    os.getenv("CLAUDE_PM_MEMORY_HOST", "localhost"),
                ),
                port=int(
                    os.getenv(
                        "CLAUDE_MULTIAGENT_PM_MEMORY_PORT",
                        os.getenv("CLAUDE_PM_MEMORY_PORT", "8002"),
                    )
                ),
                timeout=int(
                    os.getenv(
                        "CLAUDE_MULTIAGENT_PM_MEMORY_TIMEOUT",
                        os.getenv("CLAUDE_PM_MEMORY_TIMEOUT", "10"),
                    )
                ),
                max_retries=2,
                retry_delay=0.5,
                ...
                enable_logging=True,
                api_key=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_API_KEY", os.getenv("CLAUDE_PM_MEMORY_API_KEY")
                ),
                batch_size=50,
        ...
        self.configs["testing"] = MemoryEnvironmentConfig(
            ...
            memory_config=ClaudePMConfig(
                host=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_TEST_HOST",
                    os.getenv("CLAUDE_PM_MEMORY_TEST_HOST", "localhost"),
                ),
                port=int(
                    os.getenv(
                        "CLAUDE_MULTIAGENT_PM_MEMORY_TEST_PORT",
                        os.getenv("CLAUDE_PM_MEMORY_TEST_PORT", "8002"),
                    )
                ),
                timeout=5,
                max_retries=1,
        ...
        self.configs["staging"] = MemoryEnvironmentConfig(
            ...
            memory_config=ClaudePMConfig(
                host=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_STAGING_HOST",
                    os.getenv("CLAUDE_PM_MEMORY_STAGING_HOST", "memory-staging.internal"),
                ),
                port=int(
                    os.getenv(
                        "CLAUDE_MULTIAGENT_PM_MEMORY_STAGING_PORT",
                        os.getenv("CLAUDE_PM_MEMORY_STAGING_PORT", "8002"),
                    )
                ),
                timeout=20,
                max_retries=3,
                ...
                enable_logging=True,
                api_key=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_STAGING_API_KEY",
                    os.getenv("CLAUDE_PM_MEMORY_STAGING_API_KEY"),
                ),
                batch_size=100,
        ...
        self.configs["production"] = MemoryEnvironmentConfig(
            ...
            memory_config=ClaudePMConfig(
                host=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_PROD_HOST",
                    os.getenv("CLAUDE_PM_MEMORY_PROD_HOST", "memory-service.internal"),
                ),
                port=int(
                    os.getenv(
                        "CLAUDE_MULTIAGENT_PM_MEMORY_PROD_PORT",
                        os.getenv("CLAUDE_PM_MEMORY_PROD_PORT", "8002"),
                    )
                ),
                timeout=30,
                max_retries=3,
                ...
                enable_logging=True,
                api_key=os.getenv(
                    "CLAUDE_MULTIAGENT_PM_MEMORY_PROD_API_KEY",
                    os.getenv("CLAUDE_PM_MEMORY_PROD_API_KEY"),
                ),
                batch_size=200,
    ...
    def get_config(self, environment: Optional[str] = None) -> MemoryEnvironmentConfig:
        ...
        if environment not in self.configs:
            raise ValueError(
                f"Unknown environment: {environment}. Available: {list(self.configs.keys())}"
            )
...
def get_environment_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "validation": {
            env: config_manager.validate_environment(env) for env in config_manager.configs.keys()
        },
    }
