...
def alerts():
    ...
    try:
        ...
        result = subprocess.run(
            ["python", str(health_script), "alerts"], capture_output=True, text=True
        )
...
def list():
    ...
    async def run():
        ...
        for project in projects.values():
            compliance_color = (
                "green"
                if project.compliance_score >= 80
                else "yellow" if project.compliance_score >= 60 else "red"
            )
            ...
            table.add_row(
                ...
                (
                    project.last_activity.split("T")[0]
                    if "T" in project.last_activity
                    else project.last_activity[:10]
                ),
            )
...
@click.option(
    "--mode", type=click.Choice(["exact", "fuzzy", "semantic", "hybrid"]), default="hybrid"
)
def search_projects(query, type, tech, limit, mode):
    """Search projects by technology, features, or description."""

    async def run():
        from .services.project_memory_manager import (
        ...
        try:
            ...
            search_query = SearchQuery(
                query=query, mode=SearchMode(mode), filters=filters, limit=limit
            )
            ...
            if not results:
                ...
                console.print(
                    "Try a broader search or run 'claude-pm project-index refresh' to update index"
                )
                return
            ...
            for result in results:
                ...
                match_reasons = (
                    ", ".join(result.match_reasons[:2]) if result.match_reasons else "General match"
                )
...
def recommend_projects(project_name, limit):
    ...
    async def run():
        ...
        try:
            ...
            console.print(
                f"[bold blue]💡 Getting recommendations based on: {project_name}[/bold blue]"
            )
    ...
    async def run():
        ...
        try:
            ...
            if stats["total_projects"] > 5:
                summary_data["achievements"].append(
                    f"Successfully managing {stats['total_projects']} projects"
                )
        ...
        if format == "json":
            ...
            status_color = (
                "green" if summary_data["framework_status"] == "Operational" else "yellow"
            )
...
"""
            console.print(
                Panel(summary_text.strip(), title="Claude PM Framework Executive Summary")
            )
...
@click.option(
    "--status",
    "-s",
    type=click.Choice(["todo", "in_progress", "completed"]),
    ...
    try:
        ...
        for section_name, data in sections.items():
            if data["total"] > 0:
                ...
                progress_bar = "█" * int(completion_rate / 10) + "░" * (
                    10 - int(completion_rate / 10)
                )
...
@click.option(
    "--priority", "-p", type=click.Choice(["critical", "high", "medium", "low"]), default="medium"
)
    ...
    try:
        ...
        if coverage and html:
            ...
            if coverage_dir.exists():
                console.print(
                    f"[green]📊 HTML coverage report generated: {coverage_dir}/index.html[/green]"
                )
...
async def _get_managed_projects_health():
    ...
    try:
        ...
        overall_status = (
            "healthy"
            if healthy_count >= total_projects * 0.8
            else "degraded" if healthy_count >= total_projects * 0.5 else "unhealthy"
        )
        ...
        return {
            ...
            "unhealthy_projects": len(
                [p for p in projects if p["status"] in ["unhealthy", "error"]]
            ),
...
def _display_unified_health_dashboard(
    dashboard, managed_projects_health, detailed=False, verbose=False
):
    ...
    console.print(
        f"Framework Services:        {framework_status['indicator']} {framework_status['status']}"
    )
    ...
    if detailed or verbose:
        for subsystem_name, subsystem in current.subsystems.items():
            ...
            avg_response = (
                f" ({subsystem.avg_response_time_ms:.1f}ms)"
                if subsystem.avg_response_time_ms
                else ""
            )
            console.print(
                f"├─ {subsystem_name:18} {subsystem_indicator} {subsystem.status.value.title()}{avg_response}"
            )
    else:
        ...
        for service_name, service_info in key_services.items():
            console.print(
                f"├─ {service_name:18} {service_info['indicator']} {service_info['status']}"
            )
    ...
    if detailed:
        for project in managed_projects_health["projects"][:10]:  # Show first 10
            ...
            console.print(
                f"├─ {project['name'][:18]:18} {proj_indicator} {project['status'].title()}"
            )
        ...
        console.print(
            f"├─ Degraded:              🟡 {managed_projects_health.get('degraded_projects', 0)}"
        )
        console.print(
            f"└─ Issues:                🔴 {managed_projects_health.get('unhealthy_projects', 0)}"
        )
    ...
    console.print(
        f"├─ Cache Hit Rate:        {cache_hit_rate:.0f}% {'💨' if cache_hit_rate > 70 else '🐌' if cache_hit_rate < 30 else '⚡'}"
    )
    console.print(
        f"├─ Avg Response Time:     {avg_response:.0f}ms {'⚡' if avg_response < 100 else '🐌' if avg_response > 1000 else '✅'}"
    )
        ...
        async with create_mem0ai_integration() as mem0ai:
            if mem0ai.is_connected():
                ...
                if verbose:
                    ...
                    console.print(
                        f"📊 Service Info:          {test_response.get('status', 'Unknown')}"
                    )
            else:
...
def _display_projects_health(managed_projects_health, verbose=False):
    ...
    console.print(
        f"🔴 Unhealthy:             {managed_projects_health.get('unhealthy_projects', 0)}"
    )
...
def _get_key_services_summary(current_report):
    ...
    for service in current_report.services:
        if any(
            keyword in service.name.lower() for keyword in ["health", "memory", "project", "index"]
        ):
            services[service.name] = {
                "indicator": _get_status_indicator(service.status),
                "status": (
                    f"{service.status.value.title()} ({service.response_time_ms:.0f}ms)"
                    if service.response_time_ms
                    else service.status.value.title()
                ),
            }
