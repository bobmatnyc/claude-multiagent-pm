...
class Mem0AIIntegration:
    ...
    def __init__(self, config: Optional[Mem0AIConfig] = None):
        ...
        if self.config.api_key:
            logger.info(
                f"mem0AI integration initialized with authentication: {mask_api_key(self.config.api_key)}"
            )
        else:
    ...
    async def _create_secure_request_headers(
        ...
        try:
            ...
            async def make_request():
                ...
                async with self._session.post(
                    f"{self.base_url}/spaces", json=data, headers=headers
                ) as response:
                    if response.status in [200, 201, 409]:  # 409 = already exists
                        ...
                        logger.error(
                            f"Failed to create project space: {response.status} - {error_text}"
                        )
                        return False
        ...
        try:
            ...
            async def make_request():
                async with self._session.delete(
                    f"{self.base_url}/spaces/{project_name}"
                ) as response:
                    if response.status in [200, 204, 404]:  # 404 = already deleted
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Failed to delete project space: {response.status} - {error_text}"
                        )
                        return False
    ...
    async def store_memory(
        ...
        if category not in self.categories:
            logger.warning(
                f"Unknown memory category: {category}. Known categories: {list(self.categories.keys())}"
            )
            ...
            async def make_request():
                async with self._session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.debug(
                            f"Retrieved {len(memories)} memories for query '{query}' in {project_name}"
                        )
                        return memories
                    else:
                        error_text = await response.text()
            ...
            async def make_request():
                async with self._session.get(f"{self.base_url}/memories/{memory_id}") as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Failed to get memory {memory_id}: {response.status} - {error_text}"
                        )
                        return None
            ...
            async def make_request():
                async with self._session.put(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Failed to update memory {memory_id}: {response.status} - {error_text}"
                        )
                        return False
            ...
            async def make_request():
                async with self._session.delete(
                    ...
                    if response.status in [200, 204, 404]:  # 404 = already deleted
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Failed to delete memory {memory_id}: {response.status} - {error_text}"
                        )
                        return False
...
def create_mem0ai_integration(
    ...
    config = Mem0AIConfig(
        host=host, port=port, timeout=timeout, api_key=api_key, security_config=security_config
    )
