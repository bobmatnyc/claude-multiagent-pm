...
class OrchestrationDetector:
    ...
    ORCHESTRATION_FLAG = "CLAUDE_PM_ORCHESTRATION: ENABLED"
    MAX_PARENT_LEVELS = 3
    ...
    def is_orchestration_enabled(self) -> bool:
        ...
        """
        enabled, claude_md_path = self._detect_orchestration()
        
        if enabled:
            logger.info(f"Orchestration enabled found in: {claude_md_path}")
        else:
            logger.debug("Orchestration not enabled - no valid CLAUDE.md found with enable flag")
            
        return enabled
    
    def _detect_orchestration(self) -> Tuple[bool, Optional[Path]]:
        """
        Search for CLAUDE.md files and check for orchestration flag.
        ...
        while levels_checked <= self.MAX_PARENT_LEVELS:
            ...
            if self._check_claude_md_for_flag(claude_md_path):
    ...
    def _check_claude_md_for_flag(self, file_path: Path) -> bool:
        """
        Check if a CLAUDE.md file exists and contains the orchestration flag.
        ...
        try:
            ...
            if self.ORCHESTRATION_FLAG in content:
                logger.debug(f"Found orchestration flag in: {file_path}")
                return True
    ...
    def get_claude_md_path(self) -> Optional[Path]:
        ...
        """
        _, claude_md_path = self._detect_orchestration()
