...
class PerformanceTestSuite:
    ...
    def stop_resource_monitoring(self) -> Tuple[float, float]:
        ...
        if self.resource_monitor:
            ...
            avg_cpu = (
                statistics.mean(self.resource_monitor["cpu_percent"])
                if self.resource_monitor["cpu_percent"]
                else 0.0
            )
            avg_memory = (
                statistics.mean(self.resource_monitor["memory_mb"])
                if self.resource_monitor["memory_mb"]
                else 0.0
            )
    ...
    def generate_performance_report(self) -> str:
        ...
        report.append(
            "| Test | Operations | Avg Time (ms) | P95 (ms) | Success Rate | Throughput (ops/s) | Target Met |"
        )
        report.append(
            "|------|------------|---------------|----------|--------------|-------------------|------------|"
        )
...
class TestMemoryRecallPerformance(PerformanceTestSuite):
    ...
    async def populated_memory_service(self):
        ...
        config = {"fallback_chain": ["sqlite"], "memory_enabled": True}
        ...
        for i in range(20):
            memory_id = await service.add_memory(
                ...
                tags=[
                    "milestone",
                    "implementation",
                    ["frontend", "backend", "database", "api"][i % 4],
                ],
                metadata={
                    "milestone_id": i,
    ...
    async def test_basic_recall_performance(self, recall_service):
        ...
        target_met = (
            ...
            and percentiles["p95"] <= target_p95_time
            and success_rate >= target_success_rate
            and throughput >= target_throughput
        )
    ...
    async def test_concurrent_recall_performance(self, recall_service):
        ...
        try:
            ...
            tasks = []
            for i in range(50):
                task = self._timed_recall_operation(recall_service, i, operation_times)
                tasks.append(task)
    ...
    async def _timed_recall_operation(self, recall_service, iteration, operation_times):
        ...
        try:
            ...
            result = await recall_service.recall_for_operation(
                ...
                operation_context={"iteration": iteration, "test_type": "concurrent"},
            )
    ...
    async def test_memory_usage_stability(self, recall_service):
        ...
        if self.resource_monitor and self.resource_monitor["memory_mb"]:
            ...
            initial_memory = (
                statistics.mean(memory_values[:5]) if len(memory_values) >= 5 else memory_values[0]
            )
            final_memory = (
                statistics.mean(memory_values[-5:])
                if len(memory_values) >= 5
                else memory_values[-1]
            )
...
class TestSimilarityMatchingPerformance(PerformanceTestSuite):
    ...
    def similarity_matcher(self):
        ...
        config = MatchingConfig(
            min_similarity_threshold=0.1, max_results=50, enable_caching=True, cache_size=10000
        )
    ...
    async def test_cosine_similarity_performance(self, similarity_matcher, sample_memories):
        ...
        assert (
            success_rate >= 0.95
        ), f"Similarity calculation success rate too low: {success_rate:.1%}"
...
class TestRecommendationEnginePerformance(PerformanceTestSuite):
    ...
    async def test_recommendation_generation_performance(
        self, recommendation_engine, enriched_contexts
    ):
        ...
        avg_recommendations = (
            total_recommendations / successful_generations if successful_generations > 0 else 0
        )
        ...
        assert (
            success_rate >= 0.70
        ), f"Recommendation generation success rate too low: {success_rate:.1%}"
        assert avg_time <= 150.0, f"Average generation time too high: {avg_time:.1f}ms"
        assert (
            avg_recommendations >= 1.0
        ), f"Too few recommendations generated on average: {avg_recommendations:.1f}"
