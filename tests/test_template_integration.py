...
"""
        (
            framework_dir / "framework" / "templates" / "projects" / "basic-project.template"
        ).write_text(project_template)
...
"""
        (
            framework_dir / "framework" / "templates" / "tickets" / "basic-ticket.template"
        ).write_text(ticket_template)
...
'''
        (framework_dir / "framework" / "templates" / "agents" / "basic-agent.template").write_text(
            agent_template
        )
...
"""
            ...
            rendered_content = await integration.render_template(
                template_id=template_id, variables=custom_variables
            )
            ...
            updated_version = await integration.update_template(
                template_id=template_id, content=updated_content, variables=updated_variables
            )
            ...
            restore_success = await integration.restore_template(
                template_id=template_id, version="1.0.0"
            )
    ...
    @pytest.mark.asyncio
    async def test_template_discovery_and_registration(
        self, temp_framework_dir, mock_deployment_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_deployment_aware_template_filtering(
        self, temp_framework_dir, mock_deployment_config
    ):
        ...
        with patch.object(
            ...
            deployment_templates = await integration.get_templates_by_deployment_context(
                template_type=TemplateType.PROJECT, include_development=True
            )
    ...
    @pytest.mark.asyncio
    async def test_template_validation_comprehensive(
        self, temp_framework_dir, mock_deployment_config
    ):
    ...
    async def test_template_recommendation_system(self, temp_framework_dir, mock_deployment_config):
        ...
        with patch.object(
            ...
            recommendations = await integration.get_deployment_specific_template_recommendations(
                project_type="project", requirements=["web", "react"]
            )
    ...
    @pytest.mark.asyncio
    async def test_template_backup_restore_workflow(
        self, temp_framework_dir, mock_deployment_config
    ):
        ...
        with patch.object(
            ...
            v2 = await integration.update_template(
                template_id=template_id, content=v2_content, variables=v2_variables
            )
            ...
            v3 = await integration.update_template(
                template_id=template_id, content=v3_content, variables=v3_variables
            )
    ...
    @pytest.mark.asyncio
    async def test_template_deployment_config_integration(
        self, temp_framework_dir, mock_deployment_config
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(ValueError):
                await integration.update_template(
                    template_id="non-existent-template", content="new content"
                )
            ...
            result = await integration.render_template(
                template_id="non-existent-template", variables={"test": "value"}
            )
            assert result is None
            ...
            restore_success = await integration.restore_template(
                template_id="non-existent-template", version="1.0.0"
            )
            assert restore_success is False
