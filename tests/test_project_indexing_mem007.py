...
from claude_pm.services.project_indexer import ProjectIndexer, ProjectMetadata, ProjectType, TechStack
...
class TestProjectIndexer:
    ...
    async def mock_indexer(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            indexer.memory.create_project_memory_space = AsyncMock(return_value=MagicMock(success=True))
            indexer.memory.store_memory = AsyncMock(return_value=MagicMock(success=True, memory_id="test-id"))
...
class TestProjectMemoryManager:
    ...
    async def mock_memory_manager(self):
        ...
        manager.memory.retrieve_memories = AsyncMock(return_value=MagicMock(
            success=True,
            data={
                "memories": [{
                    "id": "test-memory-id",
...
class TestProjectIndexDaemon:
    ...
    def mock_daemon(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            daemon.indexer.scan_and_index_all = AsyncMock(return_value={
                "projects_found": 2,
                "projects_indexed": 1,
                "projects_updated": 1,
                "projects_skipped": 0,
            ...
            daemon.memory_manager.get_performance_stats = AsyncMock(return_value={
...
class TestCLIIntegration:
    ...
    def test_cli_imports(self, mock_memory_manager, mock_indexer):
        ...
        from claude_pm.services.project_memory_manager import create_project_memory_manager, SearchQuery, SearchMode
