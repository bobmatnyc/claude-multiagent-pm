...
from claude_pm.models.health import (
    HealthStatus, HealthReport, HealthDashboard, ServiceHealthReport, SubsystemHealth,
    create_health_report, create_health_dashboard, create_service_health_report
)
from claude_pm.interfaces.health import HealthCollector
from claude_pm.utils.performance import CircuitBreaker, HealthCache, CircuitBreakerOpenError, CacheEntry
...
class TestHealthDashboardOrchestrator:
    ...
    async def test_add_remove_collectors(self):
        ...
        collector = MockHealthCollector("test_collector", "Test Subsystem", ["service1", "service2"])
    ...
    async def test_health_dashboard_generation(self):
        ...
        # Add mock collectors
        collector1 = MockHealthCollector("collector1", "Framework Services", ["service1", "service2"])
    ...
    async def test_collector_failure_handling(self):
        ...
        failing_collector = MockHealthCollector("failing", "Failing Subsystem", ["bad_service"], should_fail=True)
        ...
        error_services = [s for s in dashboard.current_report.services if s.status == HealthStatus.ERROR]
...
class TestHealthMonitorServiceAdapter:
    ...
    async def test_adapter_with_mock_service(self):
        ...
        mock_service.run_health_check = AsyncMock(return_value={
            "status": "completed",
...
async def test_integration_comprehensive_health_command():
    ...
    # Add test collectors
    collector1 = MockHealthCollector("integration_test1", "Test Framework", ["service1", "service2"])
