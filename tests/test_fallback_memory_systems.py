...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("fallback_memory_test.log")],
)
...
class FallbackMemorySystemTester:
    ...
    async def test_sqlite_backend(self) -> Dict[str, Any]:
        ...
        # Test FTS search
        query = MemoryQuery(query="architecture decision", limit=10)
        ...
        query = MemoryQuery(query="", category=MemoryCategory.PATTERN, limit=10)
        ...
        # Test tag filtering
        query = MemoryQuery(query="", tags=["bug"], limit=10)
    ...
    async def test_inmemory_backend(self) -> Dict[str, Any]:
        ...
        inmemory_config = {"max_memory_size": 1000, "enable_expiration": True, "default_ttl": 3600}
    ...
    async def test_circuit_breaker(self) -> Dict[str, Any]:
        ...
        cb_config = CircuitBreakerConfig(
            failure_threshold=3, recovery_timeout=2.0, test_requests=2, success_threshold=2
        )
    ...
    async def test_performance_reliability(self) -> Dict[str, Any]:
        ...
        backend_configs = [
            (
                "sqlite",
                SQLiteBackend,
                {"db_path": os.path.join(self.temp_dir, "perf_sqlite.db"), "enable_fts": True},
            ),
            ("tinydb", TinyDBBackend, {"db_path": os.path.join(self.temp_dir, "perf_tinydb.json")}),
    ...
    async def test_failure_recovery(self) -> Dict[str, Any]:
        ...
        assert (
            metrics["circuit_breaker_activations"] > 0
        ), "Circuit breaker activations not recorded"
        ...
        if "Performance & Reliability" in self.test_results:
            ...
            if perf_data["status"] == "PASSED":
                recommendations.append(
                    "Performance characteristics are acceptable for production use"
                )
