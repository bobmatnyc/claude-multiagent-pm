"""
Unit Tests for Task Tool Helper

Comprehensive test suite for TaskToolHelper covering:
- Subprocess creation and lifecycle management
- Model selection and configuration
- PM Orchestrator integration
- Correction capture integration
...
class TestTaskToolConfiguration:
    """Test TaskToolConfiguration data class."""
    ...
    def test_default_configuration(self):
        ...
        assert config.memory_collection_required is True
        assert config.auto_escalation is True
        assert config.progress_tracking is True
        assert config.integration_validation is True
        assert config.correction_capture_enabled is True
        assert config.correction_capture_auto_hook is True
        assert config.enable_model_selection is True
        assert config.auto_model_optimization is True
    ...
    def test_custom_configuration(self):
        ...
        assert config.memory_collection_required is False
        assert config.model_override == "claude-4-opus"
        assert config.performance_priority == "speed"


class TestTaskToolHelper:
    """Test TaskToolHelper core functionality."""
    ...
    @pytest.mark.asyncio
    async def test_create_agent_subprocess_with_all_parameters(self):
    ...
    @pytest.mark.asyncio
    async def test_create_agent_subprocess_error_handling(self):
        """Test error handling during subprocess creation."""
        with patch('claude_pm.utils.task_tool_helper.PMOrchestrator') as mock_pm, \
             patch('claude_pm.utils.task_tool_helper.collect_pm_orchestrator_memory') as mock_collect:
            ...
            # Should return TaskToolResponse with error
