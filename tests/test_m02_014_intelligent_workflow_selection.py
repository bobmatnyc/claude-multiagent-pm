...
from claude_pm.services.workflow_selection_engine import (
    ...
    WorkflowRecommendation, WorkflowPattern, create_workflow_selection_engine
)
from claude_pm.services.multi_agent_orchestrator import (
    MultiAgentOrchestrator, AgentType, AgentStatus
)
...
class TestWorkflowSelectionEngine:
    ...
    async def test_complex_workflow_selection(self, workflow_engine, mock_dependencies):
        ...
        mock_dependencies["task_planner"].decompose_task = AsyncMock(return_value=mock_decomposition)
    ...
    def test_task_keyword_extraction(self, workflow_engine):
        ...
        test_cases = [
            ("Build a React frontend with API integration", ["implementation", "frontend", "integration"]),
        ...
        for task_description, expected_keywords in test_cases:
            ...
            for expected in expected_keywords:
                assert expected in keywords, f"Expected '{expected}' in keywords for: {task_description}"
    ...
    async def test_workflow_pattern_loading(self, workflow_engine, mock_dependencies):
        ...
        mock_dependencies["memory"].retrieve_memories = AsyncMock(return_value=Mock(
            success=True,
            data={"memories": mock_patterns}
        ))
        
        request = WorkflowSelectionRequest(
            task_description="Simple implementation task",
    ...
    def test_pattern_relevance_checking(self, workflow_engine):
        ...
        relevant_request = WorkflowSelectionRequest(
            task_description="Implement frontend component",
            task_complexity=TaskComplexity.SIMPLE
        )
        ...
        assert not workflow_engine._is_pattern_relevant(pattern, relevant_request, irrelevant_analysis)
    ...
    def test_routing_strategy_determination(self, workflow_engine):
        ...
        test_cases = [
            {
                "request": WorkflowSelectionRequest(
                    task_description="Critical production issue",
                    priority_level="critical"
                ),
        ...
        for case in test_cases:
            strategy = workflow_engine._determine_routing_strategy(case["request"], case["analysis"])
            assert strategy == case["expected"], f"Expected {case['expected']} for: {case['request'].task_description}"
    ...
    async def test_performance_metrics_tracking(self, workflow_engine):
        ...
        request = WorkflowSelectionRequest(
            task_description="Test task for metrics tracking"
        )
    ...
    def test_fallback_recommendation(self, workflow_engine):
        ...
        test_cases = [
            {
                "request": WorkflowSelectionRequest(
                    task_description="Critical emergency task",
                    priority_level="critical"
                ),
            ...
            {
                "request": WorkflowSelectionRequest(
                    task_description="Research new technology options",
                    priority_level="medium"
                ),
...
class TestIntelligentWorkflowOrchestrator:
    ...
    def intelligent_orchestrator(self, mock_memory_service):
        ...
        mock_engine = Mock()
        mock_engine.select_workflow = AsyncMock(return_value=WorkflowRecommendation(
            workflow_type=WorkflowType.SIMPLE_LINEAR,
            confidence=0.8,
        ...
        orchestrator = MultiAgentOrchestrator(base_repo_path="/tmp/test_repo", memory=mock_memory_service)
