...
class TestFrameworkMemoryHooks:
    ...
    async def setup_hooks(self):
        ...
        orchestrator.trigger_memory_creation = AsyncMock(return_value=TriggerResult(success=True, memory_id="test-memory-id"))
...
class TestMemoryTriggerDecorators:
    ...
    async def setup_decorators(self):
        ...
        hooks._enabled = True
        hooks.execute_hook = AsyncMock(return_value=[TriggerResult(success=True, memory_id="test-memory-id")])
