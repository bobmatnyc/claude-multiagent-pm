...
class TestDependencyManager(unittest.TestCase):
    ...
    async def test_install_ai_trackdown_tools(self, mock_run_command):
        ...
        mock_run_command.return_value = Mock(
            returncode=0,
            stdout="+ @bobmatnyc/ai-trackdown-tools@1.1.1",
            stderr=""
        )
    ...
    async def test_install_python_package(self, mock_run_command):
        ...
        mock_run_command.return_value = Mock(
            returncode=0,
            stdout="Successfully installed pydantic-2.5.0",
            stderr=""
        )
    ...
    def test_install_system_binary(self):
        ...
        result = asyncio.run(self.dependency_manager._install_system_binary(
            "test-binary", 
            config, 
    ...
    @patch('claude_pm.services.dependency_manager.DependencyManager._check_ai_trackdown_tools_available')
    @patch('claude_pm.services.dependency_manager.DependencyManager._check_command_available')
    @patch('claude_pm.services.dependency_manager.DependencyManager._run_command')
    ...
    async def test_get_installation_recommendations(self):
        ...
        self.dependency_manager.CORE_DEPENDENCIES["critical-missing"] = {
            "critical": True
        }
        self.dependency_manager.CORE_DEPENDENCIES["optional-missing"] = {
            "critical": False
        }
...
class TestDependencyManagerIntegration(unittest.TestCase):
    ...
    @patch('claude_pm.services.dependency_manager.DependencyManager._initialize_deployment_integration')
    async def test_service_lifecycle(self, mock_init_deployment):
        ...
        with patch.object(self.dependency_manager, '_check_python_available', return_value=True):
            with patch.object(self.dependency_manager, '_check_node_available', return_value=True):
                with patch.object(self.dependency_manager, '_check_npm_available', return_value=True):
                    with patch.object(self.dependency_manager, '_check_git_available', return_value=True):
                        with patch.object(self.dependency_manager, '_check_ai_trackdown_tools_available', return_value=True):
                            with patch.object(self.dependency_manager, '_check_critical_dependencies', return_value=True):
