...
from claude_pm.services.memory.context_builder import (
    ContextBuilder,
    ContextType
)
...
class TestSimilarityMatcher:
    ...
    def test_jaccard_similarity(self, matcher, sample_memories):
        ...
        result = matcher.calculate_similarity(
            query, memory, algorithm=SimilarityAlgorithm.JACCARD
        )
    ...
    def test_hybrid_similarity(self, matcher, sample_memories):
        ...
        result = matcher.calculate_similarity(
            query, memory, algorithm=SimilarityAlgorithm.HYBRID
        )
    ...
    def test_rank_memories_by_similarity(self, matcher, sample_memories):
        ...
        ranked = matcher.rank_memories_by_similarity(
            query, sample_memories, limit=2
        )
...
class TestRecommendationEngine:
    ...
    def test_generate_error_prevention_recommendations(self, recommendation_engine, sample_enriched_context):
...
class TestPerformanceOptimizer:
    ...
    async def mock_recall_service(self):
        ...
        service.get_performance_stats = Mock(return_value={
            "total_recalls": 100,
    ...
    def test_optimization_recommendations(self, optimizer):
        ...
        cache_opt = [r for r in recommendations if r.strategy == OptimizationStrategy.CACHE_OPTIMIZATION]
        assert len(cache_opt) > 0
        
        # Should have query optimization recommendation
        query_opt = [r for r in recommendations if r.strategy == OptimizationStrategy.QUERY_OPTIMIZATION]
