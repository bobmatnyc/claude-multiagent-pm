...
class MockMemory:
    ...
    async def retrieve_memories(self, **kwargs) -> MemoryResponse:
        ...
        return MemoryResponse(success=True, data={"memories": mock_memories}, message="Success")
...
async def test_basic_context_preparation_performance(context_manager):
    ...
    ), f"Context preparation took {preparation_time}ms, should be < 500ms"
...
async def test_parallel_context_preparation_performance(context_manager):
    ...
    assert (
        parallel_time < 1000
    ), f"Parallel context preparation took {parallel_time}ms, should be < 1000ms"
    ...
    assert (
        parallel_time < estimated_sequential_time
...
async def test_context_caching_performance(context_manager):
    ...
    ), f"Cached request ({second_time:.2f}ms) should be much faster than first ({first_time:.2f}ms)"
    assert context1.context_id != context2.context_id, "Cached context should have different ID"
    assert (
        context1.total_memories == context2.total_memories
    ), "Cached context should have same memory count"
...
async def test_large_context_preparation_performance(context_manager):
    ...
    ), f"Large context preparation took {preparation_time}ms, should be < 2000ms"
...
async def test_context_filtering_performance(context_manager):
    ...
    ), f"Context with filtering took {preparation_time}ms, should be < 1000ms"
    assert len(context.context_filters_applied) > 0, "Context should have filters applied"
    assert context.security_level in [
        "public",
        "team_only",
    ], "Context should have security level set"

    print(
        f"✅ Context filtering: {preparation_time:.2f}ms with {len(context.context_filters_applied)} filters"
    )
...
async def test_pattern_recognition_performance(context_manager):
    ...
    assert (
        preparation_time < 800
    ), f"Pattern recognition took {preparation_time}ms, should be < 800ms"
