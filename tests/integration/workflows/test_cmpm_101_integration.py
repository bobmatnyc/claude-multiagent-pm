...
class TestCMPM101Integration(unittest.TestCase):
    ...
    def test_deployment_detection_in_source_repo(self):
        ...
        result = subprocess.run(
            ["node", str(self.claude_pm_bin), "--deployment-info"],
            capture_output=True,
            text=True,
    ...
    def test_deployment_detection_in_deployed_instance(self):
        ...
        result = subprocess.run(
            ["node", str(deployed_bin), "--deployment-info"],
            capture_output=True,
            text=True,
            cwd=self.test_dir,
        )
    ...
    def test_enhanced_help_with_deployment_info(self):
        ...
        result = subprocess.run(
            ["node", str(self.claude_pm_bin), "--help"],
            capture_output=True,
            text=True,
            cwd=self.framework_dir,
        )
    ...
    def test_cli_integration_with_deployment_detection(self):
        ...
        result = subprocess.run(
            ["node", str(self.claude_pm_bin), "--version"],
            capture_output=True,
            text=True,
            cwd=self.framework_dir,
        )
    ...
    def test_error_handling_with_deployment_detection(self):
        ...
        try:
            ...
            test_content = content.replace("path.join(__dirname, '..')", f'"{empty_dir}"')

            with open(test_script, "w") as f:
                f.write(test_content)

            result = subprocess.run(
                ["node", str(test_script), "--deployment-info"],
                capture_output=True,
                text=True,
                cwd=empty_dir,
            )
