...
from claude_pm.services.continuous_learning_engine import (
    ContinuousLearningEngine, TaskOutcome, LearningPattern, LearningMetric,
    OutcomeType, PatternType, LearningMetricType, LearningInsight,
...
class TestContinuousLearningEngine:
    ...
    async def mock_memory(self):
        ...
        memory.store_memory = AsyncMock(return_value=Mock(success=True, memory_id="test_memory_123"))
            ...
            data={"memories": [
                {
                    ...
                    "metadata": {
                        ...
                        "decomposition": {
                            "strategy": "parallel",
                            "subtasks": [
                                {"title": "Design", "complexity": "simple", "estimated_hours": 4},
                                {"title": "Implement", "complexity": "medium", "estimated_hours": 8}
                            ]
                        }
                    }
                }
...
class TestMEM006AcceptanceCriteria:
    ...
    async def test_criteria_3_analyze_failure_patterns(self, learning_engine):
        ...
        prevention_recommendations = [r for r in failure_pattern.recommendations if "prevent" in r.lower() or "mitigate" in r.lower()]
    ...
    async def test_criteria_4_automatic_pattern_recognition(self, learning_engine):
        ...
        outcomes = [
            ...
            TaskOutcome("task_s1", "API implementation", "api_project", "decomp_s1", OutcomeType.SUCCESS, 
                       actual_hours=8, estimated_hours=8, completion_date=datetime.now()),
            TaskOutcome("task_s2", "API development", "api_project", "decomp_s2", OutcomeType.SUCCESS, 
                       actual_hours=9, estimated_hours=8, completion_date=datetime.now()),
            TaskOutcome("task_s3", "API creation", "api_project", "decomp_s3", OutcomeType.SUCCESS, 
                       actual_hours=7, estimated_hours=8, completion_date=datetime.now()),
            
            # Efficiency pattern group
            TaskOutcome("task_e1", "Frontend component", "ui_project", "decomp_e1", OutcomeType.SUCCESS, 
                       actual_hours=4, estimated_hours=6, completion_date=datetime.now()),  # Efficient
            TaskOutcome("task_e2", "UI component build", "ui_project", "decomp_e2", OutcomeType.SUCCESS, 
                       actual_hours=5, estimated_hours=7, completion_date=datetime.now()),  # Efficient
        ]
    ...
    async def test_criteria_6_historical_analysis_effectiveness(self, learning_engine):
        ...
        for week in range(8):  # 8 weeks of data
            for task_num in range(3):  # 3 tasks per week
                ...
                estimated_hours = 10.0
                actual_hours = estimated_hours * (1 + (0.4 - base_accuracy))  # More accurate = closer to estimate
                
                outcome = TaskOutcome(
                    task_id=f"hist_task_{week}_{task_num}",
                    task_description=f"Historical task week {week}",
                    project_name="learning_project",
                    outcome_type=OutcomeType.SUCCESS if base_accuracy > 0.7 else OutcomeType.PARTIAL_SUCCESS,
    ...
    async def test_insight_generation(self, learning_engine):
        ...
        if insights:  # Insights may not always be generated depending on thresholds
            for insight in insights:
                ...
                assert insight.insight_type in ["recommendation", "warning", "optimization", "trend"]
...
async def test_mem006_complete_workflow():
    ...
    mock_memory.retrieve_memories = AsyncMock(return_value=Mock(success=True, data={"memories": []}))
    ...
    outcomes = [
        TaskOutcome("task_1", "Implement login", "auth_project", "decomp_1", OutcomeType.SUCCESS, 
                   actual_hours=8, estimated_hours=8, completion_date=datetime.now()),
        TaskOutcome("task_2", "Implement registration", "auth_project", "decomp_2", OutcomeType.SUCCESS, 
                   actual_hours=6, estimated_hours=8, completion_date=datetime.now()),
        TaskOutcome("task_3", "Complex dashboard", "analytics_project", "decomp_3", OutcomeType.FAILURE, 
                   actual_hours=20, estimated_hours=12, completion_date=datetime.now()),
        TaskOutcome("task_4", "API endpoint", "api_project", "decomp_4", OutcomeType.SUCCESS, 
                   actual_hours=4, estimated_hours=6, completion_date=datetime.now()),
    ]
