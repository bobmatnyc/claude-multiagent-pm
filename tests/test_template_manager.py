...
class TestTemplateManager:
    ...
    async def test_template_update(self, template_manager):
        ...
        await template_manager.create_template(
            template_id=template_id, template_type=TemplateType.PROJECT, content=original_content
        )
    ...
    async def test_template_retrieval(self, template_manager):
        ...
        # Create template
        await template_manager.create_template(
            template_id=template_id, template_type=TemplateType.PROJECT, content=content
        )
    ...
    async def test_template_backup_restore(self, template_manager):
        ...
        version2 = await template_manager.update_template(
            template_id=template_id, content=updated_content
        )
        ...
        success = await template_manager.restore_template(
            template_id=template_id, version=version1.version
        )
        ...
        assert (
            template_manager.template_registry[template_id]["current_version"] == version1.version
        )
    ...
    async def test_template_listing(self, template_manager):
        ...
        await template_manager.create_template(
            template_id="agent-template", template_type=TemplateType.AGENT, content="Agent template"
        )
        ...
        agent_templates = await template_manager.list_templates(template_type=TemplateType.AGENT)
    ...
    async def test_template_history(self, template_manager):
        ...
        # Create template
        await template_manager.create_template(
            template_id=template_id, template_type=TemplateType.PROJECT, content="Version 1"
        )

        # Update template multiple times
        await template_manager.update_template(template_id=template_id, content="Version 2")

        await template_manager.update_template(template_id=template_id, content="Version 3")
    ...
    def test_version_generation(self, template_manager):
        ...
        template_manager.version_registry[template_id] = [Mock(version="1.0.5")]
...
class TestTemplateDeploymentIntegration:
    ...
    async def test_deployment_integration_initialization(self, temp_dir, mock_deployment_config):
        ...
        with patch.object(
            TemplateDeploymentIntegration, "_get_deployment_configuration"
        ) as mock_get_config:
    ...
    async def test_deployment_template_recommendations(self, temp_dir, mock_deployment_config):
        ...
        with patch.object(
            ...
            with patch.object(
                integration, "get_templates_by_deployment_context"
            ) as mock_get_templates:
                mock_get_templates.return_value = mock_templates

                recommendations = (
                    await integration.get_deployment_specific_template_recommendations(
                        project_type="project", requirements=["web", "typescript"]
                    )
                )
...
class TestTemplateManagerEdgeCases:
    ...
    async def test_create_template_with_empty_content(self, template_manager):
        ...
        with pytest.raises(ValueError):
            await template_manager.create_template(
                template_id="empty-template", template_type=TemplateType.PROJECT, content=""
            )
    ...
    async def test_update_nonexistent_template(self, template_manager):
        ...
        with pytest.raises(ValueError):
            await template_manager.update_template(
                template_id="nonexistent-template", content="new content"
            )
    ...
    async def test_render_nonexistent_template(self, template_manager):
        ...
        result = await template_manager.render_template(
            template_id="nonexistent-template", variables={"test": "value"}
        )
        assert result is None
    ...
    async def test_restore_nonexistent_template(self, template_manager):
        ...
        result = await template_manager.restore_template(
            template_id="nonexistent-template", version="1.0.0"
        )
    ...
    async def test_version_cleanup(self, template_manager):
        ...
        for i in range(5):
            await template_manager.update_template(
                template_id=template_id, content=f"Version {i+2}"
            )
