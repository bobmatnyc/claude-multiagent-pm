...
class AgentSelectionResult:
    def __init__(
        self, selected_agent, confidence, reasoning, fallback_agents=None, memory_patterns_used=None
    ):
...
class TestIntelligentTaskPlanner:
    ...
    async def test_strategy_selection_with_memory(self, planner):
        ...
        for task_desc, valid_strategies in strategy_test_cases:
            ...
            assert strategy in [
                s.value for s in valid_strategies
...
class TestMemoryIntegration:
    ...
    def full_system(self, memory_service, context_manager):
        ...
        orchestrator = MultiAgentOrchestrator(
            base_repo_path="/tmp/test_repo", memory=memory_service
        )
