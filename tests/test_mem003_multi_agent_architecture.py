...
class TestMem0ContextManager:
    ...
    async def mock_memory(self):
        ...
        memory.retrieve_memories.return_value = Mock(
            success=True, data={"memories": sample_memories}
        )
...
class TestMultiAgentOrchestrator:
    ...
    async def mock_memory(self):
        ...
        memory.retrieve_memories.return_value = Mock(success=True, data={"memories": []})
        memory.store_memory.return_value = Mock(success=True, memory_id="test_memory_id")
    ...
    async def orchestrator(self, temp_repo, mock_memory):
        ...
        orchestrator = await create_multi_agent_orchestrator(
            base_repo_path=temp_repo, memory=mock_memory, max_parallel=2  # Reduced for testing
        )
        yield orchestrator
...
class TestIntegration:
    ...
    async def test_end_to_end_workflow(self, temp_repo, mock_memory):
        ...
        orchestrator = await create_multi_agent_orchestrator(
            base_repo_path=temp_repo, memory=mock_memory, max_parallel=2
        )
    ...
    async def test_code_review_engineer_workflow(self, temp_repo, mock_memory):
        ...
        orchestrator = await create_multi_agent_orchestrator(
            base_repo_path=temp_repo, memory=mock_memory, max_parallel=1
        )
