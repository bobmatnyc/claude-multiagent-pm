...
from claude_pm.core.enforcement import (
    ...
    FileClassifier, Agent, Action, AgentPermissions, ConstraintViolation,
    ValidationResult, AgentType, ActionType, ViolationSeverity, FileCategory,
    get_enforcement_engine, enforce_file_access, validate_agent_action
)
...
class TestFileClassifier:
    ...
    def test_project_management_classification(self):
        ...
        assert FileClassifier.classify_file("trackdown/issue-001.md") == FileCategory.PROJECT_MANAGEMENT
    ...
    def test_research_docs_classification(self):
        ...
        assert FileClassifier.classify_file("docs/research/tech-eval.md") == FileCategory.RESEARCH_DOCS
        assert FileClassifier.classify_file("investigation/findings.md") == FileCategory.RESEARCH_DOCS
...
class TestAgentCapabilityManager:
    ...
    def test_validate_qa_test_access(self, capability_manager):
        ...
        action = Action(
            action_type=ActionType.WRITE,
            resource_path=Path("test_main.py"),
            agent=qa_agent
        )
...
class TestViolationMonitor:
    ...
    def test_violation_report_generation(self, violation_monitor):
        ...
        for i, severity in enumerate([ViolationSeverity.LOW, ViolationSeverity.HIGH, ViolationSeverity.CRITICAL]):
            violation = ConstraintViolation(
...
class TestIntegrationScenarios:
    ...
    def test_cross_agent_file_access_scenario(self):
        ...
        for agent_type, file_path, action, expected in scenarios:
            ...
            assert result == expected, f"Failed: {agent_type} {action} {file_path} expected {expected}, got {result}"
