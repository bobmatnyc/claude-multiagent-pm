...
from claude_pm.services.claude_pm_memory import (
    ClaudePMMemory, ClaudePMConfig, MemoryCategory, 
    MemoryResponse, create_claude_pm_memory, claude_pm_memory_context
)
...
class TestClaudePMMemoryInit:
    ...
    def test_categories_initialization(self, memory_instance):
        ...
        assert "Architectural decisions" in memory_instance.categories[MemoryCategory.PROJECT]["description"]
        assert "Successful solutions" in memory_instance.categories[MemoryCategory.PATTERN]["description"]
...
class TestConnectionManagement:
    ...
    async def test_ensure_connection_when_disconnected(self, memory_instance):
        ...
        with patch.object(memory_instance, 'is_connected', return_value=False), \
...
class TestSynchronousMethods:
    ...
    def test_store_memory_sync(self, memory_instance):
        ...
        with patch.object(memory_instance, 'store_memory') as mock_async:
            ...
            response = memory_instance.store_memory_sync(
                MemoryCategory.PROJECT, "test content"
            )
...
class TestFactoryFunctions:
    ...
    async def test_claude_pm_memory_context(self):
        ...
        with patch('claude_pm.services.claude_pm_memory.ClaudePMMemory.connect') as mock_connect, \
             patch('claude_pm.services.claude_pm_memory.ClaudePMMemory.disconnect') as mock_disconnect:
