...
var DEFAULT_CONFIG, _ConfigManager, ConfigManager;
var init_config = __esm({
  "src/utils/config.ts"() {
    ...
    _ConfigManager = class _ConfigManager {
      ...
      loadConfigFile(path32) {
        try {
          if (!existsSync(path32)) {
            return {};
          }
          const fileContent = readFileSync(path32, "utf-8");
          const ext = extname(path32);
          if (ext === ".yaml" || ext === ".yml") {
          ...
          console.warn(`Warning: Could not load config from ${path32}, skipping`);
          return {};
        }
      }
      ...
      createDefaultConfigFile(format = "json") {
        ...
        const path32 = join(process.cwd(), filename);
        this.configPath = path32;
        this.config = { ...DEFAULT_CONFIG };
        this.saveConfig();
        return path32;
      }
...
var _PathResolver, PathResolver;
var init_path_resolver = __esm({
  "src/utils/path-resolver.ts"() {
    ...
    _PathResolver = class _PathResolver {
       ...
       */
      getActiveDir() {
        return join2(this.getRootDirectory(), "active");
      }
      /**
       * Get the completed tasks directory
       */
      getCompletedDir() {
        return join2(this.getRootDirectory(), "completed");
      }
      /**
       * Get the templates directory
       */
      getTemplatesDir() {
        return join2(this.getRootDirectory(), "templates");
      }
      /**
       * Get the exports directory
       */
      getExportsDir() {
        return join2(this.getRootDirectory(), "exports");
      }
      /**
       * Get the documentation directory
       */
      getDocsDir() {
        return join2(this.getRootDirectory(), "docs");
      }
      /**
       * Get the archived tasks directory
       */
      getArchivedDir() {
        return join2(this.getRootDirectory(), "archived");
      }
      /**
      ...
      shouldMigrate() {
        const config = this.configManager.getConfig();
        const hasLegacyDir = existsSync2(join2(process.cwd(), "trackdown"));
      ...
      getLegacyTrackdownDir() {
        return join2(process.cwd(), "trackdown");
      }
      /**
      ...
      getTemplateDirectories(templateType) {
        ...
        const templates = {
          standard: [
            { path: join2(rootDir, "active"), type: "directory" },
            { path: join2(rootDir, "completed"), type: "directory" },
            { path: join2(rootDir, "templates"), type: "directory" },
            { path: join2(rootDir, "exports"), type: "directory" },
            { path: join2(rootDir, "docs"), type: "directory" }
          ],
          cli: [
            { path: join2(rootDir, "features"), type: "directory" },
            { path: join2(rootDir, "bugs"), type: "directory" },
            { path: join2(rootDir, "releases"), type: "directory" },
            { path: join2(rootDir, "documentation"), type: "directory" }
          ],
          web: [
            { path: join2(rootDir, "frontend"), type: "directory" },
            { path: join2(rootDir, "backend"), type: "directory" },
            { path: join2(rootDir, "testing"), type: "directory" },
            { path: join2(rootDir, "deployment"), type: "directory" }
          ],
          api: [
            { path: join2(rootDir, "endpoints"), type: "directory" },
            { path: join2(rootDir, "schemas"), type: "directory" },
            { path: join2(rootDir, "testing"), type: "directory" },
            { path: join2(rootDir, "documentation"), type: "directory" }
          ],
          mobile: [
            { path: join2(rootDir, "features"), type: "directory" },
            { path: join2(rootDir, "ui-ux"), type: "directory" },
            { path: join2(rootDir, "testing"), type: "directory" },
            { path: join2(rootDir, "releases"), type: "directory" }
          ]
        };
...
${bottom}`);
      ...
      static path(path32) {
        return chalk.cyan.underline(path32);
      }
      // Code formatting
      static code(code) {
...
var _ValidationError, ValidationError;
...
import { existsSync as existsSync4, mkdirSync as mkdirSync2, readdirSync, readFileSync as readFileSync2, writeFileSync as writeFileSync3, statSync } from "fs";
...
`
    ...
    async (options) => {
      ...
      try {
        ...
        const trackdownDir = join3(process.cwd(), pathResolver.getRootDirectory());
        if (!existsSync4(trackdownDir)) {
        ...
        try {
          ...
          if (exportConfig.output) {
            ...
            writeFileSync3(outputPath, exportedData);
...
function resolveOutputPath(outputPath, trackdownDir, pathResolver) {
  if (outputPath.startsWith("/") || outputPath.includes(":")) {
    ...
    return join3(process.cwd(), pathResolver.getExportsDir(), outputPath);
  }
}
...
function collectItems(trackdownDir, includeCompleted, pathResolver) {
  ...
  for (const dir of directories) {
    const fullPath = join3(process.cwd(), dir);
    if (existsSync4(fullPath)) {
      const files = readdirSync(fullPath).filter((file) => file.endsWith(".md"));
      for (const file of files) {
        try {
          const item = parseTrackdownFile(join3(fullPath, file));
          if (item) {
            items.push(item);
          }
        } catch (_error) {
    ...
    _UnifiedPathResolver = class _UnifiedPathResolver {
      ...
      getUnifiedPaths() {
        ...
        return {
          ...
          configDir: join4(this.projectRoot, ".ai-trackdown"),
          tasksRoot: join4(this.projectRoot, tasksRoot),
          epicsDir: join4(this.projectRoot, tasksRoot, this.config.structure.epics_dir),
          issuesDir: join4(this.projectRoot, tasksRoot, this.config.structure.issues_dir),
          tasksDir: join4(this.projectRoot, tasksRoot, this.config.structure.tasks_dir),
          prsDir: join4(this.projectRoot, tasksRoot, this.config.structure.prs_dir || "prs"),
          templatesDir: join4(this.projectRoot, tasksRoot, this.config.structure.templates_dir)
        };
      }
      /**
      ...
      detectLegacyStructure() {
        ...
        const potentialLegacyDirs = [
          join4(this.projectRoot, "epics"),
          join4(this.projectRoot, "issues"),
          join4(this.projectRoot, "tasks"),
          join4(this.projectRoot, "prs"),
          join4(this.projectRoot, "trackdown")
...
var DEFAULT_CONFIG_DIR, DEFAULT_CONFIG_FILE, _ConfigManager2, ConfigManager2;
...
var _AITrackdownIdGenerator, AITrackdownIdGenerator;
  ...
  "src/utils/id-generator.ts"() {
    ...
    _AITrackdownIdGenerator = class _AITrackdownIdGenerator {
      ...
      constructor(config, projectRoot) {
        this.config = config;
        this.countersPath = path4.join(projectRoot, ".ai-trackdown", "counters.json");
        this.loadCounters();
      }
      /**
      ...
      autoDetectCounters(projectRoot) {
        const epicsDir = path4.join(projectRoot, this.config.structure.epics_dir);
        const issuesDir = path4.join(projectRoot, this.config.structure.issues_dir);
        const tasksDir = path4.join(projectRoot, this.config.structure.tasks_dir);
      ...
      saveCounters() {
        try {
          const dir = path4.dirname(this.countersPath);
...
var FRONTMATTER_DELIMITER, FRONTMATTER_REGEX, _FrontmatterParser, FrontmatterParser;
  ...
  "src/utils/frontmatter-parser.ts"() {
    ...
    _FrontmatterParser = class _FrontmatterParser {
      ...
      writeEpic(filePath, data, content) {
        ...
        this.ensureDirectoryExists(path5.dirname(filePath));
...
`;
      ...
      parseDirectory(dirPath, itemType) {
        ...
        const files = fs4.readdirSync(dirPath).filter((file) => file.endsWith(".md")).map((file) => path5.join(dirPath, file));
        const results = [];
...
`
    ...
    try {
      ...
      const projectPath = path6.resolve(process.cwd(), projectNameValue);
      ...
      const spinner = ora2("Initializing AI-Trackdown project...").start();
      try {
  ...
  try {
    const answers = await inquirer2.prompt([
      ...
      },
      {
        ...
        when: /* @__PURE__ */ __name((answers2) => fs5.existsSync(path6.resolve(process.cwd(), answers2.name)), "when")
      }
    ]);
...
`;
  fs5.writeFileSync(path6.join(projectPath, "README.md"), readmeContent, "utf8");
}
...
Thumbs.db
`;
  fs5.writeFileSync(path6.join(projectPath, ".gitignore"), gitignoreContent, "utf8");
}
var init_init = __esm({
  "src/commands/init.ts"() {
    "use strict";
...
import { existsSync as existsSync11, readdirSync as readdirSync6, readFileSync as readFileSync7, statSync as statSync2 } from "fs";
...
function createStatusCommand() {
  ...
  command.description("Display comprehensive project status with advanced filtering and analytics").option("-v, --verbose", "show detailed item information with descriptions").option("-c, --compact", "compact output format for quick overview").option("--table", "display results in table format").option("--stats", "show detailed project statistics and analytics").option("--summary", "show concise project summary with key metrics").option("--filter <expr>", 'advanced filter expression (e.g., "status=todo,priority=high")').option("-s, --status <status>", "filter by status (todo, in-progress, done, blocked)").option("-p, --priority <priority>", "filter by priority (low, medium, high, critical)").option("-a, --assignee <name>", "filter by assignee name or email").option("-t, --tags <tags>", "filter by tags (comma-separated)").option("-i, --id <id>", "filter by specific item ID").option("--created-after <date>", "show items created after date (YYYY-MM-DD)").option("--created-before <date>", "show items created before date (YYYY-MM-DD)").option("--updated-after <date>", "show items updated after date (YYYY-MM-DD)").option("--updated-before <date>", "show items updated before date (YYYY-MM-DD)").option("--estimate-min <points>", "minimum story points").option("--estimate-max <points>", "maximum story points").option(
    "--sort <field>",
    "sort by field (created, updated, priority, status, title)",
    "updated"
...
`
    ...
    async (options) => {
      ...
      try {
        ...
        const config = configManager.getConfig();
        const pathResolver = new PathResolver(configManager, rootDirOption);
        const trackdownDir = join10(process.cwd(), pathResolver.getRootDirectory());
        if (!existsSync11(trackdownDir)) {
          if (pathResolver.shouldMigrate()) {
            pathResolver.showMigrationWarning();
            console.log("\nMigration commands:");
            pathResolver.getMigrationCommands().forEach((cmd) => {
        ...
        const spinner = ora3("Analyzing project status...").start();
        try {
...
async function getAllItemsEnhanced(trackdownDir, pathResolver) {
  ...
  for (const dir of directories) {
    const fullPath = join10(process.cwd(), dir);
    if (existsSync11(fullPath)) {
      const files = readdirSync6(fullPath).filter((file) => file.endsWith(".md"));
      for (const file of files) {
        try {
          const item = parseTrackdownFileEnhanced(join10(fullPath, file));
          if (item) {
            items.push(item);
          }
        } catch (_error) {
...
function parseFilters(options) {
  ...
  if (options == null ? void 0 : options.tags) {
    filters.tags = validateTags2(options.tags);
  }
...
async function exportResults(items, exportFile, config) {
  const spinner = ora3("Exporting filtered results...").start();
  try {
...
function displaySummaryView(trackdownDir, allItems, filteredItems, config, pathResolver) {
  ...
  console.log(Formatter.subheader("\u{1F4CB} Project Summary"));
  ...
  console.log("");
  console.log(Formatter.subheader("\u{1F3AF} Status Breakdown"));
  console.log(`   \u{1F4DD} Todo: ${allItems.filter((item) => item.status === "todo").length}`);
  console.log(`   \u{1F504} In Progress: ${inProgressItems.length}`);
  console.log(`   \u{1F6AB} Blocked: ${blockedItems.length}`);
  console.log(`   \u2705 Done: ${completedItems.length}`);
  console.log("");
  console.log(Formatter.subheader("\u26A1 Priority Distribution"));
  console.log(`   \u{1F534} Critical: ${priorityBreakdown.critical || 0}`);
  console.log(`   \u{1F7E0} High: ${priorityBreakdown.high || 0}`);
  console.log(`   \u{1F7E1} Medium: ${priorityBreakdown.medium || 0}`);
  console.log(`   \u{1F7E2} Low: ${priorityBreakdown.low || 0}`);
  console.log("");
  console.log(Formatter.subheader("\u{1F504} Recent Activity"));
  console.log(`   Items updated in last 7 days: ${Formatter.highlight(recentlyUpdated.toString())}`);
  if (filtered !== total) {
    console.log("");
    console.log(Formatter.info(`\u{1F4CA} ${filtered} of ${total} items match current filters`));
  }
...
var init_status = __esm({
  "src/commands/status.ts"() {
    ...
    __name(displaySummaryView, "displaySummaryView");
...
var readFile2, writeFile2, stat2, readdir2, access2, INDEX_FILE_NAME, INDEX_VERSION, MAX_CONCURRENT_READS, _TrackdownIndexManager, TrackdownIndexManager;
var init_trackdown_index_manager = __esm({
  "src/utils/trackdown-index-manager.ts"() {
    ...
    init_frontmatter_parser();
    init_unified_path_resolver();
    readFile2 = promisify(fs6.readFile);
    writeFile2 = promisify(fs6.writeFile);
    stat2 = promisify(fs6.stat);
    readdir2 = promisify(fs6.readdir);
    access2 = promisify(fs6.access);
    ...
    _TrackdownIndexManager = class _TrackdownIndexManager {
      ...
      constructor(config, projectPath, cliTasksDir) {
        ...
        this.indexPath = path7.join(this.tasksDir, INDEX_FILE_NAME);
      }
      /**
      ...
      async findItemFile(type, id) {
        ...
        try {
          ...
          for (const file of mdFiles) {
            if (file.includes(id)) {
              return path7.join(typeDir, file);
            }
          }
        } catch {
        }
...
function createStatusEnhancedCommand() {
  const command = new Command4("status-enhanced");
...
function createBacklogCommand() {
  const cmd = new Command5("backlog");
...
function createBacklogEnhancedCommand() {
  const command = new Command6("backlog-enhanced");
...
function createPortfolioCommand() {
  const cmd = new Command7("portfolio");
...
async function showPortfolio(options) {
  ...
  const scanDirectory = path8.resolve(options.directory || ".");
  ...
  function scanDir(dir, depth = 0) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const itemPath = path8.join(dir, item);
        try {
...
async function analyzeProject(projectPath) {
  ...
  return {
    name: (config == null ? void 0 : config.project_name) || path8.basename(projectPath),
    path: projectPath,
    config,
    epics,
    issues,
...
function createTrackCommand() {
  const command = new Command8("track");
...
`
    ...
    async (title, options) => {
      ...
      try {
        ...
        const tags = taskData.tags ? validateTags(taskData.tags) : void 0;
        const labels = taskData.labels ? validateTags(taskData.labels) : void 0;
        ...
        const spinner = ora4("Creating trackdown task...").start();
        try {
          const item = {
            id: itemId,
          ...
          const activeDir = join13(process.cwd(), pathResolver.getActiveDir());
          if (!existsSync13(activeDir)) {
            ...
            mkdirSync9(activeDir, { recursive: true });
          }
          const filename = `${itemId}-${sanitizeFilename(validatedTitle)}.md`;
          const filePath = join13(activeDir, filename);
...
var _VersionManager, VersionManager;
  ...
  "src/utils/version.ts"() {
    ...
    _VersionManager = class _VersionManager {
      ...
      static getProjectRoot() {
        ...
        while (cliRoot !== path9.dirname(cliRoot)) {
          const packageJsonPath = path9.join(cliRoot, "package.json");
          ...
          cliRoot = path9.dirname(cliRoot);
        }
        let current = process.cwd();
        while (current !== path9.dirname(current)) {
          if (fs8.existsSync(path9.join(current, "package.json"))) {
            return current;
          }
      ...
      static getVersion() {
        ...
        const versionFile = path9.join(projectRoot, this.VERSION_FILE);
        let versionString;
        if (fs8.existsSync(versionFile)) {
          versionString = fs8.readFileSync(versionFile, "utf8").trim();
        } else {
          const packageJsonPath = path9.join(projectRoot, this.PACKAGE_JSON);
          if (fs8.existsSync(packageJsonPath)) {
            const packageJson = JSON.parse(fs8.readFileSync(packageJsonPath, "utf8"));
      ...
      static setVersion(version) {
        ...
        const projectRoot = this.getProjectRoot();
        const versionFile = path9.join(projectRoot, this.VERSION_FILE);
        fs8.writeFileSync(versionFile, version);
      }
      /**
...
function createShowCommand() {
  const command = new Command9("show");
...
var _ChangelogManager, ChangelogManager;
    ...
    _ChangelogManager = class _ChangelogManager {
      ...
      static getProjectRoot() {
        ...
        while (current !== path10.dirname(current)) {
          if (fs9.existsSync(path10.join(current, "package.json"))) {
            return current;
          }
      ...
      static generateChangelog(version, since) {
        const projectRoot = this.getProjectRoot();
        const changelogPath = path10.join(projectRoot, this.CHANGELOG_FILE);
        ...
        if (fs9.existsSync(changelogPath)) {
          const existingContent = fs9.readFileSync(changelogPath, "utf8");
        ...
        fs9.writeFileSync(changelogPath, changelogContent);
      }
      /**
...
`;
      ...
      static getLatestChangelogVersion() {
        ...
        const content = fs9.readFileSync(changelogPath, "utf8");
  ...
  "src/utils/git.ts"() {
    ...
    _GitManager = class _GitManager {
      ...
      static isGitRepository() {
        try {
          execSync2("git rev-parse --git-dir", { stdio: "ignore" });
          return true;
        } catch {
          return false;
        }
      ...
      static hasUncommittedChanges() {
        try {
          const output = execSync2("git status --porcelain", { encoding: "utf8" });
      ...
      static getCurrentBranch() {
        try {
          return execSync2("git branch --show-current", { encoding: "utf8" }).trim();
        } catch {
      ...
      static createTag(version, message) {
        ...
        try {
          execSync2(`git tag -l ${tagName}`, { stdio: "ignore" });
          throw new Error(`Tag ${tagName} already exists`);
        } catch (error) {
        }
        try {
          execSync2(`git tag -a ${tagName} -m "${tagMessage}"`);
        } catch (error) {
      ...
      static getTags() {
        ...
        try {
          const output = execSync2("git tag -l", { encoding: "utf8" });
      ...
      static getLastTag() {
        ...
        try {
          const output = execSync2("git describe --tags --abbrev=0", { encoding: "utf8" });
          return output.trim();
        } catch {
          return null;
        }
      ...
      static getCommitsSince(since) {
        ...
        try {
          const output = execSync2(`git log ${since}..HEAD --oneline`, { encoding: "utf8" });
      ...
      static tagExists(tag) {
        ...
        try {
          execSync2(`git tag -l ${tag}`, { stdio: "ignore" });
          return true;
        } catch {
          return false;
        }
      ...
      static getRepositoryUrl() {
        ...
        try {
          const output = execSync2("git config --get remote.origin.url", { encoding: "utf8" });
          return output.trim();
        } catch {
          return null;
        }
...
function createBumpCommand() {
  const command = new Command10("bump");
...
function createChangelogCommand() {
  const command = new Command11("changelog");
...
function createTagCommand() {
  const command = new Command12("tag");
    ...
    try {
      ...
      try {
        ...
        if (options.force && error instanceof Error && error.message.includes("already exists")) {
          ...
          try {
            execSync3(`git tag -d ${tagName}`, { stdio: "ignore" });
            GitManager.createTag(versionInfo.version, tagMessage);
...
function createSyncCommand() {
  const command = new Command13("sync");
...
function createReleaseCommand() {
  const command = new Command14("release");
...
function createVersionCommand() {
  const command = new Command15("version");
...
var init_simple_id_generator = __esm({
  "src/utils/simple-id-generator.ts"() {
    ...
    _IdGenerator = class _IdGenerator {
      ...
      constructor() {
        this.countersPath = path12.join(process.cwd(), ".ai-trackdown", "counters.json");
        this.loadCounters();
      }
      /**
       * Generate unique Epic ID
      ...
      saveCounters() {
        try {
          const dir = path12.dirname(this.countersPath);
...
function createEpicCreateCommand() {
  const cmd = new Command16("create");
...
async function createEpic(title, options) {
  ...
  const filePath = path13.join(paths.epicsDir, filename);
  if (options.dryRun) {
...
var init_relationship_manager = __esm({
  "src/utils/relationship-manager.ts"() {
    ...
    _RelationshipManager = class _RelationshipManager {
      ...
      findCircularDependencies() {
        ...
        for (const item of allItems) {
          ...
          if (!visited.has(itemId)) {
            const path32 = [];
            this.dfsForCycles(itemId, visited, recursionStack, path32, cycles);
          }
        }
        return cycles;
      }
       ...
       */
      dfsForCycles(itemId, visited, recursionStack, path32, cycles) {
        visited.add(itemId);
        ...
        if (item && item.dependencies) {
          for (const depId of item.dependencies) {
            if (!visited.has(depId)) {
              this.dfsForCycles(depId, visited, recursionStack, path32, cycles);
            } else if (recursionStack.has(depId)) {
              const cycleStart = path32.indexOf(depId);
              const cycle = path32.slice(cycleStart).concat([depId]);
              cycles.push(cycle);
            }
          }
        }
...
function createEpicUpdateCommand() {
  const cmd = new Command19("update");
...
function createEpicDeleteCommand() {
  const cmd = new Command20("delete");
...
function createEpicCompleteCommand() {
  const cmd = new Command21("complete");
...
function createIssueCreateCommand() {
  const cmd = new Command23("create");
...
async function createIssue(title, options) {
  ...
  const filePath = path14.join(paths.issuesDir, filename);
  if (options.dryRun) {
...
function createIssueShowCommand() {
  const cmd = new Command25("show");
  cmd.description("Show detailed information about an issue").argument("<issue-id>", "issue ID to show").option("-f, --format <type>", "output format (detailed|json|yaml)", "detailed").option("--show-tasks, --with-tasks", "show related tasks").option("--show-prs", "show related pull requests").option("--show-content", "show issue content/description").option("--show-related", "show related issues and dependencies").action(async (issueId, options) => {
    try {
...
async function showIssue(issueId, options) {
  const configManager = new ConfigManager2();
  const config = configManager.getConfig();
  const cliTasksDir = process.env.CLI_TASKS_DIR;
  const paths = configManager.getAbsolutePaths(cliTasksDir);
...
async function displayIssueDetailed(issue, tasks, prs, epic, options, relationshipManager) {
  console.log(Formatter.success(`
=== ISSUE: ${issue.title} ===`));
  ...
  console.log(`  Status: ${getStatusDisplay5(issue.status)}`);
  console.log(`  Priority: ${getPriorityDisplay5(issue.priority)}`);
  ...
  if (issue.completion_percentage !== void 0) {
    console.log(`  Progress: ${issue.completion_percentage}%`);
  }
  console.log("");
  console.log(Formatter.success("Tracking Information:"));
  console.log(`  Created: ${formatDateTime2(issue.created_date)}`);
  console.log(`  Updated: ${formatDateTime2(issue.updated_date)}`);
  ...
  if (options.showTasks || tasks.length > 0) {
    ...
    if (tasks.length === 0) {
      ...
      for (const task of tasks) {
        const statusIcon = getStatusIcon4(task.status);
        const priorityColor = getPriorityDisplay5(task.priority);
  ...
  if (options.showRelated) {
    ...
    if (related.siblings.length > 0) {
      ...
      for (const sibling of related.siblings) {
        console.log(`  \u2022 ${getItemId3(sibling)}: ${sibling.title}`);
      }
      console.log("");
    }
    if (related.dependencies.length > 0) {
      console.log(Formatter.success("Dependencies:"));
      for (const dep of related.dependencies) {
        console.log(`  \u2022 ${getItemId3(dep)}: ${dep.title}`);
      }
      console.log("");
    }
    if (related.dependents.length > 0) {
...
function getStatusIcon4(status) {
  const icons = {
    "planning": "\u23F3",
    "active": "\u{1F504}",
    "completed": "\u2705",
...
function getPRStatusIcon(status) {
  const icons = {
    "draft": "\u{1F4DD}",
    "open": "\u{1F513}",
    "review": "\u{1F440}",
...
var init_show3 = __esm({
  "src/commands/issue/show.ts"() {
    ...
    __name(showIssue, "showIssue");
    __name(displayIssueDetailed, "displayIssueDetailed");
    __name(getStatusDisplay5, "getStatusDisplay");
    __name(getPriorityDisplay5, "getPriorityDisplay");
    __name(getStatusIcon4, "getStatusIcon");
    __name(getPRStatusIcon, "getPRStatusIcon");
...
function createIssueUpdateCommand() {
  const cmd = new Command26("update");
...
function createIssueDeleteCommand() {
  const cmd = new Command27("delete");
...
function createIssueCompleteCommand() {
  const cmd = new Command28("complete");
...
function createIssueAssignCommand() {
  const cmd = new Command29("assign");
...
function createTaskCreateCommand() {
  const cmd = new Command31("create");
...
async function createTask(title, options) {
  ...
  const filePath = path15.join(paths.tasksDir, filename);
  if (options.dryRun) {
...
async function displayTasksTable(tasks, options) {
  ...
  for (const task of tasks) {
    const row = [
      ...
      getStatusDisplay6(task.status),
      getPriorityDisplay6(task.priority),
      task.issue_id,
...
function getStatusDisplay6(status) {
  const statusColors = {
    "planning": /* @__PURE__ */ __name((text) => Formatter.info(text), "planning"),
    "active": /* @__PURE__ */ __name((text) => Formatter.success(text), "active"),
    "completed": /* @__PURE__ */ __name((text) => Formatter.success(text), "completed"),
...
function getPriorityDisplay6(priority) {
  const priorityColors = {
    "critical": /* @__PURE__ */ __name((text) => Formatter.error(text), "critical"),
    "high": /* @__PURE__ */ __name((text) => Formatter.warning(text), "high"),
    "medium": /* @__PURE__ */ __name((text) => Formatter.info(text), "medium"),
...
var init_list3 = __esm({
  "src/commands/task/list.ts"() {
    ...
    __name(getStatusDisplay6, "getStatusDisplay");
    __name(getPriorityDisplay6, "getPriorityDisplay");
...
function createTaskUpdateCommand() {
  const cmd = new Command34("update");
...
function createTaskDeleteCommand() {
  const cmd = new Command35("delete");
...
function createTaskCompleteCommand() {
  const cmd = new Command36("complete");
...
function createPRCreateCommand() {
  const cmd = new Command38("create");
...
async function createPR(title, options) {
  ...
  const filePath = path16.join(paths.prsDir, filename);
  if (options.dryRun) {
...
function createPRReviewCommand() {
  const cmd = new Command41("review");
...
`;
  ...
  const reviewFilePath = path17.join(reviewsDir, reviewFileName);
...
function createPRApproveCommand() {
  const cmd = new Command42("approve");
...
async function approvePR(prId, options) {
  ...
  const reviewsDir = path18.join(paths.prsDir, "reviews");
...
`;
  ...
  const approvalFilePath = path18.join(reviewsDir, `${approvalReviewId}.md`);
...
function createPRUpdateCommand() {
  const cmd = new Command43("update");
...
async function handleStatusTransition3(pr, newStatus, paths) {
  const statusDirs = {
    draft: path19.join(paths.prsDir, "draft"),
    open: path19.join(paths.prsDir, "active"),
    review: path19.join(paths.prsDir, "active"),
    approved: path19.join(paths.prsDir, "active"),
    merged: path19.join(paths.prsDir, "merged"),
    closed: path19.join(paths.prsDir, "closed")
  };
...
async function logPRUpdate(updateLogEntry, paths) {
  try {
    const logDir = path19.join(paths.prsDir, "logs");
    if (!fs19.existsSync(logDir)) {
      fs19.mkdirSync(logDir, { recursive: true });
    }
    const logFile = path19.join(logDir, "pr-updates.log");
...
var _PRStatusManager, PRStatusManager;
    ...
    _PRStatusManager = class _PRStatusManager {
      ...
      getStatusDirectory(status, basePRsDir) {
        const statusDirs = {
          ...
          open: path20.join(basePRsDir, "active"),
          review: path20.join(basePRsDir, "active"),
          approved: path20.join(basePRsDir, "active"),
      ...
      async moveToStatusDirectory(pr, newStatus, basePRsDir) {
        const currentDir = path20.dirname(pr.file_path);
        ...
        const fileName = path20.basename(pr.file_path);
        const newFilePath = path20.join(targetDir, fileName);
...
var _PRFileManager, PRFileManager;
    ...
    _PRFileManager = class _PRFileManager {
      ...
      initializePRDirectories(basePRsDir) {
        ...
        directories.forEach((dir) => {
          const dirPath = path21.join(basePRsDir, dir);
        ...
        activeDirs.forEach((subdir) => {
          const subDirPath = path21.join(basePRsDir, "active", subdir);
      ...
      getPRDirectory(status, basePRsDir) {
        const statusDirectories = {
          draft: path21.join(basePRsDir, "draft"),
          open: path21.join(basePRsDir, "active", "open"),
          review: path21.join(basePRsDir, "active", "review"),
          approved: path21.join(basePRsDir, "active", "approved"),
          merged: path21.join(basePRsDir, "merged"),
          closed: path21.join(basePRsDir, "closed")
        };
      ...
      async movePRToStatusDirectory(pr, newStatus, basePRsDir) {
        ...
        const fileName = path21.basename(currentPath);
        const newPath = path21.join(targetDir, fileName);
      ...
      generateUniqueFilename(filePath) {
        ...
        while (fs21.existsSync(uniquePath)) {
          uniquePath = path21.join(dir, `${base}-${counter}${ext}`);
          counter++;
        }
        return uniquePath;
      }
      ...
      async archiveOldPRs(basePRsDir, olderThanDays = 90) {
        const archiveDir = path21.join(basePRsDir, "archive");
        ...
        const errors = [];
        const dirsToCheck = [
          path21.join(basePRsDir, "merged"),
          path21.join(basePRsDir, "closed")
        ];
        for (const dir of dirsToCheck) {
          ...
          for (const file of files) {
            ...
            if (stats.mtime < cutoffDate) {
              try {
                const archivePath = path21.join(archiveDir, file);
      ...
      cleanupEmptyDirectories(basePRsDir) {
        ...
        const subDirs = [
          path21.join(basePRsDir, "active", "open"),
          path21.join(basePRsDir, "active", "review"),
          path21.join(basePRsDir, "active", "approved")
        ];
      ...
      validatePRDirectoryStructure(basePRsDir) {
        ...
        prFiles.forEach((file) => {
          const relativePath = path21.relative(basePRsDir, file);
      ...
      findPRFiles(basePRsDir) {
        ...
        const searchDir = /* @__PURE__ */ __name((dir) => {
          ...
          items.forEach((item) => {
            const itemPath = path21.join(dir, item);
...
async function updateTaskForClose(taskId, prId, options, configManager) {
  try {
    ...
    const taskFile = taskFiles[0];
    const taskPath = path23.join(tasksDir, taskFile);
...
async function updateIssueForClose(issueId, prId, options, configManager) {
  try {
    ...
    const issueFile = issueFiles[0];
    const issuePath = path23.join(issuesDir, issueFile);
...
async function addToClosureReport(prData, options, configManager) {
  ...
  const reportPath = path23.join(reportsDir, "closure-report.json");
  let reports = [];
...
async function createBatchReport(result, options, configManager) {
  ...
  const reportsDir = path24.join(configManager.getPRsDirectory(), "reports");
  ...
  const reportPath = path24.join(reportsDir, reportFileName);
...
function findCircularDependencies(dependencies) {
  ...
  const visited = /* @__PURE__ */ new Set();
  const recursionStack = /* @__PURE__ */ new Set();
  function dfs(node, path32) {
    if (recursionStack.has(node)) {
      const cycleStart = path32.indexOf(node);
      if (cycleStart !== -1) {
        cycles.push([...path32.slice(cycleStart), node]);
      }
      return;
    }
    if (visited.has(node)) {
...
async function loadDependencies(configManager) {
  const depsPath = path25.join(configManager.getPRsDirectory(), "dependencies.json");
...
async function loadTaskData(taskId, configManager) {
  try {
    ...
    const taskFile = taskFiles[0];
    const taskPath = path26.join(tasksDir, taskFile);
...
function createPRArchiveCommand() {
  const cmd = new Command49("archive");
...
async function performArchive(options, statusManager, fileManager, configManager) {
  ...
  try {
    const basePRsDir = configManager.getPRsDirectory();
    const archiveDir = path27.join(basePRsDir, "archive");
    const timestamp = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const archivePath = path27.join(archiveDir, `archive-${timestamp}`);
    if (!options.dryRun) {
...
async function archivePR(pr, archivePath, options, configManager) {
  ...
  try {
    ...
    const fileName = path27.basename(pr.file_path);
    let targetPath;
    if (options.preserveStructure) {
      const relativePath = path27.relative(configManager.getPRsDirectory(), pr.file_path);
      targetPath = path27.join(archivePath, relativePath);
      const targetDir = path27.dirname(targetPath);
      if (!fs27.existsSync(targetDir)) {
        fs27.mkdirSync(targetDir, { recursive: true });
      }
    } else {
      targetPath = path27.join(archivePath, fileName);
    }
...
async function archiveReviewFiles(archivePath, options, configManager) {
  try {
    const reviewsDir = path27.join(configManager.getPRsDirectory(), "reviews");
    if (!fs27.existsSync(reviewsDir)) {
      return 0;
    }
    const reviewFiles = fs27.readdirSync(reviewsDir);
    const archiveReviewsDir = path27.join(archivePath, "reviews");
    ...
    for (const file of reviewFiles) {
      const sourcePath = path27.join(reviewsDir, file);
      const targetPath = path27.join(archiveReviewsDir, file);
      try {
        if (!options.dryRun) {
...
async function createArchiveIndex(prs, archivePath, result, configManager) {
  ...
  for (const pr of prs) {
    const entry = {
      ...
      archivePath: path27.relative(archivePath, ((_a = result.details.find((d) => d.prId === pr.pr_id)) == null ? void 0 : _a.archivePath) || ""),
  ...
  const indexPath = path27.join(archivePath, "index.json");
...
async function listArchivedPRs(configManager, searchQuery) {
  const archiveDir = path27.join(configManager.getPRsDirectory(), "archive");
  if (!fs27.existsSync(archiveDir)) {
    return [];
  }
  const archiveDirs = fs27.readdirSync(archiveDir).filter((dir) => {
    const dirPath = path27.join(archiveDir, dir);
    return fs27.statSync(dirPath).isDirectory();
  });
  const allPRs = [];
  for (const dir of archiveDirs) {
    const indexPath = path27.join(archiveDir, dir, "index.json");
...
async function restorePR(prId, targetStatus, statusManager, fileManager, configManager) {
  ...
  try {
    ...
    const fileName = path27.basename(archivedPR.archivePath);
    const restorePath = path27.join(targetDir, fileName);
    const archiveDir = path27.join(basePRsDir, "archive");
    const archiveDirs = fs27.readdirSync(archiveDir).filter((dir) => {
      const dirPath = path27.join(archiveDir, dir);
      return fs27.statSync(dirPath).isDirectory();
    ...
    let sourcePath;
    for (const dir of archiveDirs) {
      const candidatePath = path27.join(archiveDir, dir, archivedPR.archivePath);
...
function calculateDirectorySize(dirPath) {
  ...
  for (const item of items) {
    const itemPath = path27.join(dirPath, item);
...
function createAiGenerateLlmsCommand() {
  const cmd = new Command51("generate-llms-txt");
...
async function generateLlmsTxt(options) {
  ...
  const outputPath = path28.resolve(options.output || "llms.txt");
...
function createAiTrackTokensCommand() {
  const cmd = new Command52("track-tokens");
...
function createAiContextCommand() {
  const cmd = new Command53("context");
    ...
    switch (format) {
      ...
      default:
        for (const item of itemsWithContext) {
          console.log(Formatter.info(`${getItemId5(item)}: ${item.title}`));
          for (const context of item.ai_context) {
            console.log(`  \u2022 ${context}`);
          }
          console.log("");
...
var _GitHubClient, GitHubClient;
...
function createSyncSetupCommand() {
  const command = new Command55("setup");
...
var _GitHubSyncEngine, GitHubSyncEngine;
  ...
  "src/integrations/github-sync.ts"() {
    ...
    _GitHubSyncEngine = class _GitHubSyncEngine {
      ...
      async getLocalIssues() {
        ...
        for (const file of files) {
          try {
            const filePath = path29.join(issuesDir, file);
...
\`\`\``;
      ...
      async createLocalIssueFromGitHub(githubIssue) {
        ...
        const filePath = path29.join(issuesDir, filename);
        const newIssue = {
          issue_id: issueId,
          epic_id: "",
      ...
      getSyncMetaPath() {
        const paths = this.configManager.getAbsolutePaths();
        return path29.join(paths.configDir, "sync-metadata.json");
      }
      /**
...
function createSyncPushCommand() {
  const command = new Command56("push");
  command.description("Push local changes to GitHub").option("--dry-run", "Show what would be pushed without making changes").option("--force", "Force push even with conflicts").option("--verbose", "Show detailed progress information").action(async (options) => {
    ...
    try {
      ...
      const spinner = ora5("Testing GitHub connection...").start();
      ...
      const pushSpinner = ora5("Pushing local changes to GitHub...").start();
      try {
...
function createSyncPullCommand() {
  const command = new Command57("pull");
  command.description("Pull changes from GitHub to local").option("--dry-run", "Show what would be pulled without making changes").option("--force", "Force pull even with conflicts").option("--verbose", "Show detailed progress information").action(async (options) => {
    ...
    try {
      ...
      console.log("");
      const syncEngine = new GitHubSyncEngine(configManager);
      const spinner = ora6("Testing GitHub connection...").start();
      const testResult = await syncEngine.testConnection();
      if (!testResult.success) {
        spinner.fail("GitHub connection failed");
        console.log(Formatter.error(testResult.message));
      ...
      spinner.succeed("GitHub connection verified");
      const pullSpinner = ora6("Pulling changes from GitHub...").start();
      try {
...
function createSyncStatusCommand() {
  const command = new Command58("status");
  command.description("Show sync status and conflicts").option("--verbose", "Show detailed status information").option("--conflicts-only", "Show only conflicts").action(async (options) => {
    ...
    try {
      ...
      console.log("");
      const syncEngine = new GitHubSyncEngine(configManager);
      const spinner = ora7("Checking GitHub connection...").start();
      const testResult = await syncEngine.testConnection();
      if (!testResult.success) {
        spinner.fail("GitHub connection failed");
        console.log(Formatter.error(testResult.message));
      ...
      spinner.succeed("GitHub connection verified");
      const statusSpinner = ora7("Fetching sync status...").start();
      try {
        ...
        if (status.conflicts > 0) {
          ...
          if (options.verbose) {
            const conflictSpinner = ora7("Analyzing conflicts...").start();
            try {
        ...
        if (options.verbose && !options.conflictsOnly) {
          const rateLimitSpinner = ora7("Checking rate limit...").start();
          try {
...
function createSyncAutoCommand() {
  const command = new Command59("auto");
...
function createSyncBidirectionalCommand() {
  const command = new Command60("bidirectional");
  command.description("Perform full bidirectional sync between local and GitHub").option("--dry-run", "Show what would be synced without making changes").option("--force", "Force sync even with conflicts").option("--verbose", "Show detailed progress information").option("--conflict-resolution <strategy>", "Override conflict resolution strategy", "config").action(async (options) => {
    ...
    try {
      ...
      console.log("");
      const syncEngine = new GitHubSyncEngine(configManager);
      const spinner = ora8("Testing GitHub connection...").start();
      const testResult = await syncEngine.testConnection();
      if (!testResult.success) {
        spinner.fail("GitHub connection failed");
        console.log(Formatter.error(testResult.message));
      ...
      spinner.succeed("GitHub connection verified");
      const syncSpinner = ora8("Performing bidirectional sync...").start();
      try {
...
function createSyncCommand2() {
  const command = new Command61("sync");
...
function createMigrateCommand() {
  return new Command62("migrate").description("Migrate from legacy trackdown to ai-trackdown structure").option("-d, --dry-run", "Preview migration without making changes").option("-v, --verbose", "Show detailed migration progress").option("-b, --backup", "Create backup before migration").option("--source <dir>", "Source directory to migrate from", process.cwd()).action(async (options) => {
    const migrationTool = new MigrationTool();
    try {
      console.log(chalk2.bold.blue("AI-Trackdown Migration Tool"));
      console.log(chalk2.gray("Converting legacy structure to new ai-trackdown format\n"));
      ...
      console.error(chalk2.red(`Migration failed: ${error instanceof Error ? error.message : "Unknown error"}`));
      process.exit(1);
    }
  });
}
var _MigrationTool, MigrationTool;
  ...
  "src/commands/migrate.ts"() {
    ...
    _MigrationTool = class _MigrationTool {
      ...
      async migrate(sourceDir = process.cwd(), options = {}) {
        const spinner = ora9("Starting AI-Trackdown migration...").start();
        try {
          if (options.backup) {
      ...
      async migrateConfiguration(sourceDir, dryRun = false) {
        const legacyConfigPath = path30.join(sourceDir, ".trackdownrc.json");
        if (!fs30.existsSync(legacyConfigPath)) {
          console.log(chalk2.yellow("No legacy .trackdownrc.json found, creating default config"));
          return;
        }
        try {
          ...
          if (!dryRun) {
            const configDir = path30.join(sourceDir, ".ai-trackdown");
            if (!fs30.existsSync(configDir)) {
              fs30.mkdirSync(configDir, { recursive: true });
            }
            const newConfigPath = path30.join(configDir, "config.yaml");
            fs30.writeFileSync(newConfigPath, YAML5.stringify(newConfig, { indent: 2 }));
            console.log(chalk2.green(`\u2713 Configuration migrated to ${newConfigPath}`));
          } else {
            console.log(chalk2.blue(`[DRY RUN] Would migrate config to .ai-trackdown/config.yaml`));
          }
      ...
      async createDirectoryStructure(sourceDir, dryRun = false) {
        const aiTrackdownDir = path30.join(sourceDir, ".ai-trackdown");
        const directories = [
          path30.join(aiTrackdownDir, "epics"),
          path30.join(aiTrackdownDir, "issues"),
          path30.join(aiTrackdownDir, "tasks"),
          path30.join(aiTrackdownDir, "templates")
        ];
        if (!dryRun) {
          for (const dir of directories) {
            if (!fs30.existsSync(dir)) {
              fs30.mkdirSync(dir, { recursive: true });
              console.log(chalk2.green(`\u2713 Created directory: ${dir}`));
            }
          }
        } else {
          console.log(chalk2.blue(`[DRY RUN] Would create directories: ${directories.join(", ")}`));
        }
      }
      /**
      ...
      async migrateFiles(sourceDir, dryRun = false, verbose = false) {
        const legacyDirs = [
          path30.join(sourceDir, "active"),
          path30.join(sourceDir, "completed"),
          path30.join(sourceDir, "trackdown")
      ...
      async processLegacyDirectory(legacyDir, sourceDir, dryRun = false, verbose = false) {
        ...
        for (const file of files) {
          const filePath = path30.join(legacyDir, file);
          try {
            ...
            this.stats.errors.push(errorMsg);
            if (verbose) {
              console.log(chalk2.red(`\u2717 ${errorMsg}`));
            }
          }
        }
      }
      ...
      async migrateFile(filePath, sourceDir, dryRun = false, verbose = false) {
        ...
        if (!dryRun) {
          const targetDir = path30.dirname(targetPath);
          ...
          if (verbose) {
            console.log(chalk2.green(`\u2713 Migrated ${type}: ${filePath} \u2192 ${targetPath}`));
          }
        } else {
          console.log(chalk2.blue(`[DRY RUN] Would migrate ${type}: ${filePath} \u2192 ${targetPath}`));
        }
      }
      /**
      ...
      analyzeAndPrepareFile(filename, content, sourceDir) {
        ...
        switch (type) {
          case "epic":
            ...
            targetPath = path30.join(aiTrackdownDir, "epics", `${id}.md`);
            break;
          case "issue":
            ...
            targetPath = path30.join(aiTrackdownDir, "issues", `${id}.md`);
            break;
          case "task":
            ...
            targetPath = path30.join(aiTrackdownDir, "tasks", `${id}.md`);
            break;
        }
      ...
      async createBackup(sourceDir) {
        const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
        const backupDir = path30.join(sourceDir, `backup-${timestamp}`);
        this.copyDirectory(sourceDir, backupDir, [
          "node_modules",
          ".git",
          "backup-*"
        ]);
        console.log(chalk2.green(`\u2713 Backup created at ${backupDir}`));
      }
      /**
      ...
      copyDirectory(src, dest, exclude = []) {
        ...
        for (const item of items) {
          const srcPath = path30.join(src, item);
          const destPath = path30.join(dest, item);
      ...
      async cleanupLegacyFiles(sourceDir, dryRun = false) {
        const legacyFiles = [
          path30.join(sourceDir, ".trackdownrc.json")
        ];
        const legacyDirs = [
          path30.join(sourceDir, "active"),
          path30.join(sourceDir, "completed")
        ...
        if (!dryRun) {
          for (const file of legacyFiles) {
            if (fs30.existsSync(file)) {
              ...
              console.log(chalk2.green(`\u2713 Removed legacy file: ${file}`));
            }
          }
          for (const dir of legacyDirs) {
            if (fs30.existsSync(dir)) {
              const archiveDir = path30.join(sourceDir, `${path30.basename(dir)}-legacy`);
              fs30.renameSync(dir, archiveDir);
              console.log(chalk2.green(`\u2713 Archived legacy directory: ${dir} \u2192 ${archiveDir}`));
            }
          }
        } else {
          console.log(chalk2.blue(`[DRY RUN] Would cleanup legacy files and directories`));
        }
      }
      /**
       ...
       */
      displaySummary() {
        console.log("\n" + chalk2.bold.green("Migration Summary:"));
        console.log(chalk2.green(`\u2713 Configuration migrated: ${this.stats.configMigrated}`));
        console.log(chalk2.green(`\u2713 Files processed: ${this.stats.filesProcessed}`));
        console.log(chalk2.green(`\u2713 Epics migrated: ${this.stats.epicsMigrated}`));
        console.log(chalk2.green(`\u2713 Issues migrated: ${this.stats.issuesMigrated}`));
        console.log(chalk2.green(`\u2713 Tasks migrated: ${this.stats.tasksMigrated}`));
        if (this.stats.errors.length > 0) {
          console.log(chalk2.red(`\u2717 Errors encountered: ${this.stats.errors.length}`));
          this.stats.errors.forEach((error) => {
            console.log(chalk2.red(`  - ${error}`));
          });
        }
        console.log("\n" + chalk2.bold.blue("Next Steps:"));
...
function createMigrateStructureCommand() {
  const cmd = new Command63("migrate-structure");
...
async function migrateStructure(options) {
  ...
  try {
    ...
    legacy.legacyDirs.forEach((dir) => {
      console.log(Formatter.info(`   \u2022 ${path31.relative(projectRoot, dir)}`));
    });
    const targetPaths = pathResolver.getUnifiedPaths();
    const targetTasksRoot = path31.relative(projectRoot, targetPaths.tasksRoot);
    console.log(Formatter.info(`
...
\u{1F4C1} Files to migrate: ${migrationPlan.length}`));
    if (options.verbose || options.dryRun) {
      migrationPlan.forEach((item) => {
        const sourcePath = path31.relative(projectRoot, item.source);
        const targetPath = path31.relative(projectRoot, item.target);
    ...
    for (const item of migrationPlan) {
      try {
        const targetDir = path31.dirname(item.target);
...
function planMigration(projectRoot, legacyDirs, pathResolver) {
  ...
  for (const legacyDir of legacyDirs) {
    ...
    const dirName = path31.basename(legacyDir);
    let targetDir;
    switch (dirName) {
      case "epics":
      ...
      case "trackdown":
        ...
        for (const subdir of subdirs) {
          const sourceSubdir = path31.join(legacyDir, subdir);
          let targetSubdir;
          switch (subdir) {
            case "active":
            case "epics":
          ...
          if (fs31.existsSync(sourceSubdir)) {
            ...
            for (const file of files) {
              plan.push({
                source: path31.join(sourceSubdir, file),
                target: path31.join(targetSubdir, file),
                type: subdir
              });
            }
          }
    ...
    if (fs31.existsSync(legacyDir)) {
      ...
      for (const file of files) {
        plan.push({
          source: path31.join(legacyDir, file),
          target: path31.join(targetDir, file),
          type: dirName
        });
      }
    }
...
async function createBackup(projectRoot, legacyDirs) {
  const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
  const backupDir = path31.join(projectRoot, `.backup-${timestamp}`);
  console.log(Formatter.info(`\u{1F4E6} Creating backup in ${path31.relative(projectRoot, backupDir)}...`));
  fs31.mkdirSync(backupDir, { recursive: true });
  for (const legacyDir of legacyDirs) {
    if (fs31.existsSync(legacyDir)) {
      const targetBackupDir = path31.join(backupDir, path31.basename(legacyDir));
      await copyDirectory(legacyDir, targetBackupDir);
    }
  }
  console.log(Formatter.success(`\u2705 Backup created: ${path31.relative(projectRoot, backupDir)}`));
}
async function copyDirectory(source, target) {
  ...
  for (const item of items) {
    const sourcePath = path31.join(source, item.name);
    const targetPath = path31.join(target, item.name);
    if (item.isDirectory()) {
  ...
  for (const dir of legacyDirs) {
    try {
      if (fs31.existsSync(dir)) {
        ...
        if (items.length === 0) {
          fs31.rmdirSync(dir);
          console.log(Formatter.success(`\u2705 Removed empty directory: ${path31.basename(dir)}`));
        }
      }
    } catch (error) {
    }
...
async function main() {
  ...
  program.on("--help", () => {
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F680} AI-Trackdown CLI - Comprehensive Project Management Tool"));
    console.log("");
    console.log(chalk3.bold.yellow("\u{1F4CB} HIERARCHICAL STRUCTURE:"));
    console.log("  Epics \u2192 Issues \u2192 Tasks \u2192 PRs (Pull Requests)");
    console.log("  Each level tracks tokens, progress, and relationships");
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F3D7}\uFE0F HIERARCHICAL COMMANDS:"));
    ...
    console.log("    $ aitrackdown sync auto --enable");
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F916} AI-SPECIFIC COMMANDS:"));
    ...
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F3AF} ANYWHERE-SUBMIT FUNCTIONALITY:"));
    ...
    console.log("");
    console.log(chalk3.bold.cyan("\u2699\uFE0F CORE PROJECT COMMANDS:"));
    ...
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F504} MIGRATION COMMANDS:"));
    ...
    console.log("");
    console.log(chalk3.bold.cyan("\u26A1 ALIASES & SHORTCUTS:"));
    console.log("  atd = aitrackdown (shorter command)");
    console.log("  issue = issues, task = tasks, pr = prs");
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F527} GLOBAL OPTIONS:"));
    ...
    console.log("");
    console.log(chalk3.bold.cyan("\u{1F4D6} LEARN MORE:"));
