...
class HealthDashboard:
    ...
    def calculate_trends(self) -> None:
        ...
        if len(health_percentages) >= 2:
            trend = "improving" if health_percentages[-1] > health_percentages[0] else \
                   "declining" if health_percentages[-1] < health_percentages[0] else "stable"
        else:
            trend = "stable"
    ...
    def calculate_performance_metrics(self) -> None:
        ...
        service_distribution = {
            ...
            "degraded_percentage": (current.degraded_services / max(current.total_services, 1)) * 100,
            "unhealthy_percentage": (current.unhealthy_services / max(current.total_services, 1)) * 100,
            "down_percentage": (current.down_services / max(current.total_services, 1)) * 100
        }
        
        # Response time analysis
        service_response_times = [s.response_time_ms for s in current.services if s.response_time_ms is not None]
        avg_service_response = sum(service_response_times) / len(service_response_times) if service_response_times else 0
