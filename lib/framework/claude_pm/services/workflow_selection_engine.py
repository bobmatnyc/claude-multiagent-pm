...
from .intelligent_task_planner import (
    IntelligentTaskPlanner, TaskComplexity, DecompositionStrategy, 
...
class WorkflowSelectionEngine:
    ...
    def _initialize_selection_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "research_discovery": {
                "task_keywords": ["research", "investigate", "analyze", "explore", "poc", "feasibility"],
                "preferred_workflows": [
                    WorkflowType.RESEARCH_DISCOVERY,
                    WorkflowType.ITERATIVE_REFINEMENT
                ],
    ...
    async def _analyze_task_characteristics(self, request: WorkflowSelectionRequest) -> Dict[str, Any]:
    ...
    def _analyze_task_type(self, task_description: str, keywords: List[str]) -> Dict[str, Any]:
        ...
        # Research intensity
        research_indicators = ["research", "investigate", "analyze", "explore", "poc", "feasibility"]
        analysis["research_required"] = any(indicator in task_lower for indicator in research_indicators)
        ...
        analysis["critical_path_dependencies"] = any(indicator in task_lower for indicator in dep_indicators)
    ...
    def _assess_urgency_and_resources(self, request: WorkflowSelectionRequest) -> Dict[str, Any]:
        ...
        if request.resource_constraints:
            if request.resource_constraints.get("cpu_intensive", False) or \
               request.resource_constraints.get("memory_intensive", False):
    ...
    async def _load_workflow_patterns(self, 
                                    ...
                                    task_analysis: Dict[str, Any]) -> List[WorkflowPattern]:
    ...
    def _is_pattern_relevant(self, 
        ...
        try:
            ...
            complexity_order = [
                ...
                TaskComplexity.MEDIUM, TaskComplexity.COMPLEX, TaskComplexity.EPIC
            ]
            ...
            if task_keywords and pattern_keywords:
                overlap_ratio = len(task_keywords.intersection(pattern_keywords)) / len(task_keywords.union(pattern_keywords))
    ...
    def _determine_routing_strategy(self, 
                                  request: WorkflowSelectionRequest,
                                  task_analysis: Dict[str, Any]) -> RoutingStrategy:
        ...
        if "experiment" in request.task_description.lower() or \
           "poc" in request.task_description.lower():
            return RoutingStrategy.LEARNING_OPTIMIZED
        
        # Default to balanced approach
        return RoutingStrategy.BALANCED
    
    async def _match_workflow_patterns(self,
                                     request: WorkflowSelectionRequest,
                                     task_analysis: Dict[str, Any],
                                     patterns: List[WorkflowPattern],
                                     routing_strategy: RoutingStrategy) -> Dict[WorkflowType, List[WorkflowPattern]]:
        ...
        for workflow_type, pattern_list in matches.items():
            pattern_list.sort(
                key=lambda p: self._score_pattern_for_strategy(p, routing_strategy),
                reverse=True
            )
    ...
    def _apply_selection_rules(self, 
                             request: WorkflowSelectionRequest,
                             task_analysis: Dict[str, Any]) -> List[WorkflowType]:
        ...
        for rule_name, rule in self.selection_rules.items():
            ...
            if "priority_levels" in rule:
                if request.priority_level in rule["priority_levels"] or urgency in rule["priority_levels"]:
                    rule_applies = True
    ...
    def _score_pattern_for_strategy(self, 
                                   pattern: WorkflowPattern,
                                   strategy: RoutingStrategy) -> float:
        """Score a pattern based on the routing strategy."""
        weights = self.routing_weights.get(strategy.value, self.routing_weights[RoutingStrategy.BALANCED.value])
    ...
    async def _predict_workflow_outcomes(self,
                                       request: WorkflowSelectionRequest,
                                       pattern_matches: Dict[WorkflowType, List[WorkflowPattern]],
                                       task_analysis: Dict[str, Any]) -> Dict[WorkflowType, Dict[str, Any]]:
        ...
        for workflow_type, patterns in pattern_matches.items():
            ...
            if total_weight > 0:
                predicted_success_rate = sum(sr * w for sr, w in zip(success_rates, weights)) / total_weight
                predicted_duration = sum(et * w for et, w in zip(execution_times, weights)) / total_weight
                predicted_quality = sum(qs * w for qs, w in zip(quality_scores, weights)) / total_weight
            else:
                predicted_success_rate = sum(success_rates) / len(success_rates) if success_rates else 0.5
                predicted_duration = sum(execution_times) / len(execution_times) if execution_times else 30
                predicted_quality = sum(quality_scores) / len(quality_scores) if quality_scores else 0.7
            
            # Adjust predictions based on task analysis
            adjusted_predictions = self._adjust_predictions_for_task(
                workflow_type, task_analysis, predicted_success_rate, predicted_duration, predicted_quality
            )
    ...
    def _adjust_predictions_for_task(self,
                                   workflow_type: WorkflowType,
                                   task_analysis: Dict[str, Any],
                                   base_success_rate: float,
                                   base_duration: float,
                                   base_quality: float) -> Dict[str, Any]:
        ...
        multiplier = complexity_multipliers.get(complexity, complexity_multipliers[TaskComplexity.MEDIUM])
    ...
    def _calculate_prediction_confidence(self,
        ...
        if len(success_rates) > 1:
            import statistics
            consistency_confidence = 1.0 - (statistics.stdev(success_rates) / 0.5)  # Normalize by max possible stdev
        else:
    ...
    def _select_optimal_workflow(self,
                               predictions: Dict[WorkflowType, Dict[str, Any]],
                               routing_strategy: RoutingStrategy,
                               request: WorkflowSelectionRequest) -> Tuple[WorkflowType, Dict[str, Any]]:
        ...
        workflow_scores = {}
        weights = self.routing_weights.get(routing_strategy.value, self.routing_weights[RoutingStrategy.BALANCED.value])
        
        for workflow_type, prediction in predictions.items():
            # Normalize scores
            success_score = prediction["success_rate"]
            time_score = 1.0 - min(prediction["duration_minutes"] / 180.0, 1.0)  # Normalize to 3 hours
            ...
            score = (
                ...
                weights.get("execution_time", 0) * time_score +
                weights.get("quality", 0) * quality_score +
                weights.get("resource_efficiency", 0) * (1.0 - len(prediction["risk_factors"]) / 10.0) +
    ...
    async def _generate_workflow_recommendation(self,
                                              selected_workflow: Tuple[WorkflowType, Dict[str, Any]],
                                              request: WorkflowSelectionRequest,
                                              task_analysis: Dict[str, Any],
                                              pattern_matches: Dict[WorkflowType, List[WorkflowPattern]],
                                              routing_strategy: RoutingStrategy) -> WorkflowRecommendation:
        ...
        all_predictions = {wf: pred for wf, pred in 
                          [(k, v) for k, v in pattern_matches.items() if k != workflow_type]}
        fallback_workflows = sorted(all_predictions.keys(), 
                                  key=lambda wf: all_predictions[wf][0]["success_rate"] if all_predictions[wf] else 0,
                                  reverse=True)[:2]
        
        # Resource requirements
        resource_requirements = self._estimate_resource_requirements(workflow_type, task_analysis, prediction)
    ...
    def _estimate_resource_requirements(self,
        ...
        base_requirements = {
            WorkflowType.SIMPLE_LINEAR: {"agents": 1, "cpu": "low", "memory": "low", "coordination": "none"},
            WorkflowType.PARALLEL_MULTI_AGENT: {"agents": 3, "cpu": "medium", "memory": "medium", "coordination": "high"},
            WorkflowType.HIERARCHICAL_REVIEW: {"agents": 4, "cpu": "medium", "memory": "medium", "coordination": "high"},
            WorkflowType.ITERATIVE_REFINEMENT: {"agents": 2, "cpu": "medium", "memory": "low", "coordination": "medium"},
            WorkflowType.RESEARCH_DISCOVERY: {"agents": 2, "cpu": "low", "memory": "low", "coordination": "low"},
            WorkflowType.CRITICAL_PATH: {"agents": 4, "cpu": "high", "memory": "medium", "coordination": "high"},
            WorkflowType.EMERGENCY_FAST_TRACK: {"agents": 1, "cpu": "medium", "memory": "low", "coordination": "none"}
        }
        
        requirements = base_requirements.get(workflow_type, base_requirements[WorkflowType.SIMPLE_LINEAR]).copy()
    ...
    async def _store_workflow_selection(self,
                                      request: WorkflowSelectionRequest,
        ...
        try:
            selection_data = {
                "request": {
                    ...
                    "complexity": task_analysis.get("complexity", {}).value if task_analysis.get("complexity") else "medium",
            ...
            await self.memory.store_memory(
                ...
                tags=["workflow_selection", "routing", recommendation.workflow_type.value, 
    ...
    def _update_selection_metrics(self, recommendation: WorkflowRecommendation, execution_time: float) -> None:
    ...
    def _create_fallback_recommendation(self, request: WorkflowSelectionRequest) -> WorkflowRecommendation:
...
def create_workflow_selection_engine(memory: ClaudePMMemory,
                                   context_manager: Mem0ContextManager,
                                   agent_memory_manager: Any,  # Replaced AgentMemoryManager with mem0AI
                                   task_planner: IntelligentTaskPlanner) -> WorkflowSelectionEngine:
    """
