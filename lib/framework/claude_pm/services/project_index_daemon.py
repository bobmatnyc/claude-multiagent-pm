...
class DaemonConfig:
    ...
    watch_extensions: Set[str] = field(default_factory=lambda: {
        '.md', '.json', '.toml', '.txt', '.yml', '.yaml', '.py', '.js', '.ts'
    })
...
class ProjectIndexDaemon:
    ...
    def _should_perform_health_check(self, now: datetime) -> bool:
        ...
        return (now - self.last_health_check).total_seconds() >= (self.config.health_check_interval_minutes * 60)
    ...
    def _should_perform_full_scan(self, now: datetime) -> bool:
        ...
        return (now - self.last_full_scan).total_seconds() >= (self.config.scan_interval_minutes * 60)
    ...
    def _should_perform_quick_scan(self, now: datetime) -> bool:
        ...
        return (now - self.last_quick_scan).total_seconds() >= (self.config.quick_scan_interval_minutes * 60)
    ...
    async def _perform_full_scan(self) -> None:
        ...
        try:
            ...
            logger.info(f"Full scan completed: {results.get('projects_found', 0)} projects found, "
    ...
    async def _perform_quick_scan(self) -> None:
        ...
        try:
            ...
            for project_dir in project_dirs:
                ...
                if (project_name not in self.project_checksums or 
                    self.project_checksums[project_name] != current_checksum):
...
if __name__ == "__main__":
    async def example_usage():
        ...
        config = create_daemon_config(
            scan_interval_minutes=10,  # More frequent for testing
