...
class HealthMonitorServiceAdapter(HealthCollector):
    ...
    def __init__(self, health_service: Optional[HealthMonitorService] = None, timeout_seconds: float = 10.0):
        ...
        self._health_report_path = Path.home() / "Projects" / "Claude-PM" / "logs" / "health-report.json"
        
        # Alternative path for current project structure
        if not self._health_report_path.exists():
            self._health_report_path = Path(__file__).parent.parent.parent / "logs" / "health-report.json"
        ...
        try:
            ...
            if health_status:
                service_report = ServiceHealthReport(
                    ...
                    status=self._map_health_status("healthy" if health_status.get("overall_health", 0) > 50 else "unhealthy"),
            ...
            for service_name, service_data in services.items():
                if isinstance(service_data, dict):
                    ...
                    service_report = ServiceHealthReport(
                        ...
                        timestamp=self._parse_timestamp(service_data.get("last_check", service_data.get("timestamp"))),
            ...
            if projects:
                ...
                project_report = ServiceHealthReport(
                    ...
                    metrics={
                        ...
                        "warning_projects": sum(1 for p in projects.values() if p.get("status") == "warning"),
                        "critical_projects": sum(1 for p in projects.values() if p.get("status") == "critical"),
                        "overall_health_percentage": health_data.get("summary", {}).get("overall_health_percentage", 0)
                    }
                )
