...
class SecurityEventLogger:
    ...
    def get_recent_failures(self, host: str, minutes: int = 60) -> int:
        ...
        failures = [
            ...
            if (event.service_host == host and 
...
class Mem0AIAuthenticator:
        ...
        try:
            ...
            async with session.get(f"{base_url}/health", headers=headers) as response:
                if response.status == 200:
                    ...
                    self.security_logger.log_event(AuthenticationEvent(
                    ...
                    # Other error
                    self.logger.warning(f"Unexpected response during auth validation: {response.status}")
                    return False
    ...
    async def _handle_auth_failure(self, host: str, status_code: Optional[int] = None, error: Optional[str] = None):
...
def create_security_config() -> SecurityConfig:
    ...
    return SecurityConfig(
        ...
        auth_failure_lockout_minutes=int(os.getenv("MEM0AI_AUTH_LOCKOUT_MINUTES", str(AUTH_FAILURE_LOCKOUT_MINUTES)))
    )
