...
def status(output_format: str):
    ...
    try:
        ...
        stats_table.add_row(
            ...
            "Unacknowledged violation alerts"
        )
        stats_table.add_row(
            "Critical Violations",
...
@click.option('--action', default='read', type=click.Choice(['read', 'write', 'execute']),
        ...
        if verbose or not result.is_valid:
            ...
            if result.violations:
                ...
                for i, violation in enumerate(result.violations, 1):
                    console.print(f"  {i}. [red]{violation.violation_type}[/red] ({violation.severity.value})")
                    console.print(f"     {violation.description}")
                    if violation.resolution_guidance:
                        console.print(f"     [yellow]💡 Resolution: {violation.resolution_guidance}[/yellow]")
                    console.print()
...
def alerts(severity: Optional[str], output_format: str):
    ...
    try:
        ...
        if severity:
            ...
            alerts_list = [a for a in alerts_list 
                          if severity_order[a.violation.severity] >= min_level]
...
def report(hours: int, output_format: str, output: Optional[str]):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            lines.append(f"Period: {start_time.strftime('%Y-%m-%d %H:%M')} - {end_time.strftime('%Y-%m-%d %H:%M')}")
